...
// { num, title, body } sections which, once explicitly saved, replaces
// the standard Master Services Agreement body wherever that blueprint's
// terms are shown (sign modal, print modes). Nothing is saved here
// automatically — the admin GET below pre-fills the standard text purely
// so the editor never looks blank; a blueprint's live rendering is
// unaffected until an admin actually clicks Save.
function sanitizeTosSections(raw) {
  ...
  for (const item of raw) {
    ...
    const body = (item.body || '').toString().slice(0, 20_000);
