...
function brandName() { return (window.__brand && window.__brand.name) || 'Recycle Away'; }
function brandHandle() { return brandName().toLowerCase().replace(/[^a-z0-9]+/g, ''); }
function BrandMark({ fontSize = 11, color = 'var(--fg-1)', family = 'var(--font-display)', weight = 700, letterSpacing = '-0.01em', height, onDark }) {
  const b = window.__brand || {};
  const logo = onDark ? (b.logoSrcDark || b.logoSrc) : b.logoSrc;
  if (logo) return <img src={logo} alt={brandName()} style={{ height: height || Math.round(fontSize * 1.7), width: 'auto', display: 'block', objectFit: 'contain' }}/>;
  return <span style={{ fontFamily: family, fontWeight: weight, fontSize, letterSpacing, color }}>{brandName()}</span>;
}
...
function BPIntro() {
  ...
  const coName = (bpCo && bpCo.name) || 'Recycle Away';
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 024 · PREPARED SEPTEMBER 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              A plan to replatform {brandName()}&rsquo;s commerce operations to Shopify, consolidate
...
function BPApproach() {
  const steps = [
    ...
    { n: 'Phase 2', t: 'Build', d: 'Storefront, B2B, system integration. Fixed scope, senior team, no surprises.', wk: 'Weeks 16+' },
...
const raBadge = (b) => (b==='NEW'||b==='PRO') ? { background:'var(--uc-black)', color:'#fff' } : { background:'var(--ra-green)', color:'var(--uc-black)' };
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>NET 30 TERMS · BULK PRICING · MADE IN USA OPTIONS</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Recycling Bins','Trash Cans','Compost Bins','Custom'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Recycling Bins', items: ['Indoor','Outdoor','Deskside','Combination stations'] },
    { h: 'Trash & Compost', items: ['Trash cans','Compost bins','Dog waste stations','Can liners'] },
    { h: 'Customize', items: ['Logo & colors','Lids','Labels & signs','Decals'] },
    { h: 'Featured', items: ['Made in USA','NYC compliant','Trade accounts'] }
  ];
  return (
    <GfxChrome>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:1, background:'var(--line-1)' }}>
        {cols.map((c,ci)=>(
          <div key={ci} style={{ background:'var(--uc-paper)', padding:'12px 13px 16px', display:'flex', flexDirection:'column', gap:7 }}>
            ...
            {c.items.map((it,ri)=>(<span key={ri} style={{ fontFamily:'var(--font-display)', fontWeight:ci===3&&ri===0?700:500, fontSize:10.5, letterSpacing:'-0.01em', color: ci===0&&ri===0?'var(--uc-black)':'var(--fg-2)', display:'flex', alignItems:'center', gap:5 }}>{ci===0&&ri===0 && <span style={{ width:14, height:3, background:'var(--ra-green)' }}/>}{it}</span>))}
          </div>
        ))}
      </div>
    </GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['14-Qt Recycling Bin','$12.95','each','#3B6FA8','BEST','4.9','412'],
    ['28-Qt Recycling Bin','$19.95','each','#7DAD1D',null,'4.8','236'],
    ['Deskside Sorter 7 gal','$34.00','each','#2B2B2B','NEW','4.9','88'],
    ['23-Gal Slim Bin','$64.00','each','#3B6FA8',null,'4.7','301'],
    ['2-Stream Station','$489','each','#8A8F8A',null,'4.8','57'],
    ['3-Stream Station','$1,249','each','#8B6B3D','USA','4.9','122']
  ];
  const facets = [
    { h: 'Category', items: [['Recycling Bins', true],['Trash Cans', false],['Compost Bins', false],['Combination Stations', true],['Lids & Signs', false]] },
    { h: 'Setting', items: [['Office', true],['Schools', false],['Outdoor', false]] },
    { h: 'Availability', items: [['In stock', true],['Made in USA', false],['NYC compliant', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Recycling Bins','Trash Cans','Compost Bins','Custom'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / RECYCLING BINS / INDOOR</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Indoor Recycling Bins</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>312 results</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Indoor ✕','2-Stream ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--ra-green-deep)', fontWeight:700 }}>Clear all</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'104px minmax(0,1fr)', borderTop:'1px solid var(--line-1)' }}>
        ...
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:11 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$12</span><span>$1,200</span></div>
          </div>
        </div>

        {/* product grid */}
        <div style={{ padding:10, display:'grid', gridTemplateColumns:'repeat(3, minmax(0,1fr))', gap:8, minWidth:0 }}>
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)', minWidth:0 }}>
              <div style={{ aspectRatio:'1/1', background:`linear-gradient(140deg, ${p[3]}, var(--uc-bone))`, borderRadius:2, position:'relative' }}>
                {p[4] && <span style={{ position:'absolute', top:4, left:4, padding:'1px 5px', ...raBadge(p[4]), borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800 }}>{p[4]}</span>}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:3 }}><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'var(--ra-green-deep)' }}>★★★★★</span><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'var(--fg-3)' }}>{p[5]} ({p[6]})</span></div>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'var(--ra-green-deep)' }}>Bulk pricing ✓</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:'9px 14px', borderTop:'1px solid var(--line-1)', display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
        {['‹','1','2','3','…','18','›'].map((n,i)=>(<span key={i} style={{ minWidth:15, textAlign:'center', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:n==='1'?'var(--uc-paper)':'var(--fg-2)', background:n==='1'?'var(--uc-black)':'transparent', borderRadius:2, padding:'2px 4px' }}>{n}</span>))}
      </div>
    </div>
  );
}
function GfxCart() {
  const lines=[['14-Qt Recycling Bin · Blue','250+','$10.95','$2,738'],['Deskside Sorter 7 gal','50+','$29.50','$1,475'],['Stream Decals · 6pk','80+','$6.10','$488']];
  const grid=[['14-Qt Recycling Bin','$12.95','#3B6FA8','BEST'],['28-Qt Recycling Bin','$19.95','#7DAD1D',null],['Deskside Sorter','$34.00','#2B2B2B','NEW'],['23-Gal Slim Bin','$64.00','#3B6FA8',null],['2-Stream Station','$489.00','#8A8F8A',null],['3-Stream Station','$1,249','#8B6B3D','USA']];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ padding:'9px 13px', borderBottom:'1px solid var(--line-1)', background:'var(--uc-cream)' }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-1)', marginBottom:5 }}>✓ You&rsquo;ve unlocked <span style={{ color:'var(--ra-green-deep)', fontWeight:700 }}>bulk pricing</span> · 250+ units</div>
          <div style={{ height:4, background:'var(--line-1)', borderRadius:999, overflow:'hidden' }}><span style={{ display:'block', width:'100%', height:'100%', background:'var(--ra-green)' }}/></div>
        </div>
        {/* lines */}
        {lines.map((l,i)=>(
          <div key={i} style={{ padding:'8px 13px', borderBottom:'1px solid var(--line-1)', display:'grid', gridTemplateColumns:'26px 1fr auto', gap:9, alignItems:'center' }}>
            ...
            <div><div style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:10, color:'var(--fg-1)' }}>{l[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>qty {l[1]} · <span style={{ color:'var(--ra-green-deep)', fontWeight:700 }}>{l[2]}</span></div></div>
        ...
        <div style={{ marginTop:'auto', padding:'10px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$4,701</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--ra-green-deep)', marginBottom:7 }}><span>Bulk savings</span><span>−$420</span></div>
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'var(--uc-black)', color:'var(--uc-paper)', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · NET 30 · $4,281</span>
        </div>
      </div>
    </div>
  );
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / RECYCLING BINS / COMBINATION STATIONS</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#3B6FA8','#7DAD1D','#1F1F1F','#8A8F8A'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #3B6FA8, #23446E 60%, #14181F)', position:'relative', overflow:'hidden' }}>
            <span style={{ position:'absolute', top:8, left:8, padding:'2px 7px', background:'var(--ra-green)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>BEST SELLER</span>
            <svg viewBox="0 0 100 100" style={{ position:'absolute', bottom:'8%', left:'10%', width:'80%', opacity:0.92 }}><g fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth="1.7"><rect x="6" y="36" width="26" height="54" rx="3"/><rect x="37" y="36" width="26" height="54" rx="3"/><rect x="68" y="36" width="26" height="54" rx="3"/><path d="M3 36 H97 M3 30 Q50 20 97 30 V36"/></g><g fill="rgba(255,255,255,0.35)"><circle cx="19" cy="52" r="5.5"/><rect x="43" y="49" width="14" height="4.5" rx="2"/><rect x="74" y="46" width="14" height="11" rx="2"/></g></svg>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'var(--ra-green-deep)' }}>IN STOCK · MADE IN USA · SHIPS FROM FACTORY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>3-STREAM<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Recycling Station</span></div>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--ra-green-deep)' }}>★★★★★</span><span style={{ color:'var(--fg-3)' }}>4.9 · 128 reviews</span></div>
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#3B6FA8','#7DAD1D','#1F1F1F','#8A8F8A'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Made in USA','✓ Net 30 terms','✓ Ships to 48 states'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['3-Stream Station','$1,249','#3B6FA8'],['Stream Decals 3pk','$24','#7DAD1D'],['Can Liners 100ct','$38','#1F1F1F']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$1,311</span></span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Capacity','3 × 23 gal'],['Material','Powder-coated steel'],['Footprint','54 × 20 in'],['Openings','Round · slot · full']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
        <div style={{ background:'var(--uc-cream)', padding:'11px 14px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['Three streams, one footprint — recycling, compost, landfill.','Bulk pricing and Net 30 terms unlock with a trade account.','Your logo, colors and messaging — mockups from our graphics team.'].map((t,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'12px 1fr', gap:7, alignItems:'start' }}><span style={{ width:10, height:10, borderRadius:999, background:'var(--ra-green)', marginTop:1, display:'flex', alignItems:'center', justifyContent:'center' }}><svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="var(--uc-black)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/></svg></span><span style={{ fontFamily:'var(--font-serif)', fontSize:9, lineHeight:1.35, color:'var(--fg-2)' }}>{t}</span></div>))}</div>
        </div>
      </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--ra-green-deep)' }}>★★★★★</div>
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“Outfitted every floor of our HQ. People actually sort now.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Stream Decals 3pk','$24','#7DAD1D'],['Can Liners 100ct','$38','#1F1F1F'],['Swing Lid','$18','#3B6FA8'],['Floor Sign','$42','#8A8F8A']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxPortal() {
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>TRADE ACCOUNT PORTAL · NET 30 TERMS ACTIVE</div>
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <BrandMark fontSize={11}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>HARBORVIEW HOSPITALITY</span>
          <span style={{ width:18, height:18, borderRadius:999, background:'linear-gradient(135deg,#7DAD1D,#2F4A12)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800, color:'#fff' }}>H</span>
        </span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'112px 1fr', gap:1, background:'var(--line-1)' }}>
        ...
        <div style={{ background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:3 }}>
          {nav.map((n,i)=>(<div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'5px 7px', borderRadius:3, background:n[2]?'var(--uc-black)':'transparent' }}><span style={{ fontSize:9, color:n[2]?'var(--ra-green)':'var(--fg-3)' }}>{n[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:n[2]?'var(--uc-paper)':'var(--fg-2)' }}>{n[1]}</span></div>))}
        ...
        <div style={{ background:'var(--uc-paper)', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
            ...
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'var(--uc-black)', background:'var(--ra-green)', padding:'4px 9px', borderRadius:3 }}>+ New order</span>
          </div>
          {/* stat tiles */}
          ...
          <div style={{ padding:'0 14px 12px' }}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'70px 1fr auto auto', gap:8, alignItems:'center', padding:'6px 0', borderTop:'1px solid var(--line-1)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'var(--uc-black)', background:'var(--ra-green)', padding:'2px 7px', borderRadius:2 }}>Reorder</span>
            </div>))}
          </div>
        </div>
      </div>
...
function GfxHomepage() {
  const nav = ['Shop','Industries','Custom','USA-Made','Trade'];
  const mega = [
    { h: 'By Category', items: ['Recycling Bins','Trash Cans','Compost Bins','Combination Stations','Outdoor Bins','Lids, Labels & Signs'] },
    { h: 'By Setting', items: ['Office','Schools & Campus','Stadiums & Arenas','Cities & Parks'] },
    { h: 'Quick Links', items: ['Made in USA','NYC Compliant','California Compost','Custom Bins'] }
  ];
  const brands = ['OFFICES','SCHOOLS','STADIUMS','AIRPORTS','CITIES','HOTELS'];
  const feat = [['14-Qt Recycling Bin','$12.95','#3B6FA8','BEST'],['Deskside Sorter','$34.00','#2B2B2B','NEW'],['3-Stream Station','$1,249','#8B6B3D','USA'],['Dog Waste Station','$389.00','#7DAD1D',null]];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>NET 30 TERMS · BULK PRICING · MADE IN USA OPTIONS</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7, minWidth:0, flex:'0 1 auto' }}>
          <span style={{ flex:'1 1 70px', minWidth:0, maxWidth:120, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search bins, lids & signs…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #0A0A0A 0%, #1E2A10 55%, #4E7012 135%)', overflow:'hidden' }}>
        
        {/* hero copy (right 1/3) */}
        <div style={{ position:'absolute', left:'66%', right:14, top:0, bottom:0, display:'flex', flexDirection:'column', justifyContent:'center', gap:7, minWidth:0 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'var(--ra-green)' }}>CUSTOM RECYCLING BINS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Showcase<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>yourself.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'var(--ra-green)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Get a mockup →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--ra-green)', whiteSpace:'nowrap' }}>FIND THE RIGHT BIN</span>
        {['Setting','Stream','Capacity'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:'var(--ra-green)', color:'var(--uc-black)', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Find Bins →</span>
      </div>
      ...
      <div style={{ padding:'0 14px 14px', display:'grid', gridTemplateColumns:'repeat(4, minmax(0,1fr))', gap:9 }}>
        {feat.map((p,i)=>(
          <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
            <div style={{ aspectRatio:'1/0.9', borderRadius:4, background:`linear-gradient(150deg, ${p[2]}, var(--uc-bone))`, position:'relative' }}>{p[3] && <span style={{ position:'absolute', top:5, left:5, padding:'1px 6px', ...raBadge(p[3]), borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800 }}>{p[3]}</span>}<span style={{ position:'absolute', bottom:5, right:5, width:16, height:16, borderRadius:999, background:'var(--uc-paper)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'var(--fg-1)', boxShadow:'0 1px 3px rgba(0,0,0,0.18)' }}>＋</span></div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #4E7012, #14200A)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>BELLOWS FALLS, VT</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Decades of hands-on experience building recycling programs — from Vermont to your facility.</span>
          <span style={{ alignSelf:'flex-start', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', borderBottom:'1.5px solid var(--ra-green)', paddingBottom:1 }}>Read our story →</span>
        </div>
      </div>
    </div>
  );
  ...
  const groups = [
    {
      ...
      points: [
        ...
        { t: 'Information Architecture', d: 'Mega navigation with multi-level menus built around how facility buyers actually browse.' },
        { t: 'Product Finding', d: 'Guided bin finder by setting, stream and capacity, quizzes, and creative ways to find products.' }
      ],
      gfx: <GfxHomepage/>
    },
    {
...
function GfxPhone({ kind }) {
  // Mobile recycling-supply storefront. kind: 'home' | 'about' | 'blog'
  return (
    <div style={{
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px' }}>
            ...
            <span style={{ display:'inline-flex', gap:6, alignItems:'center' }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, height:22, padding:'0 9px', background:'var(--uc-black)', borderRadius:5, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)' }}>⛒ Cart<span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', minWidth:11, height:11, padding:'0 2px', background:'var(--ra-green)', color:'var(--uc-black)', borderRadius:999, fontSize:6, fontWeight:800 }}>6</span></span>
            </span>
          </div>
          {/* big open search */}
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search bins, lids & signs…</span>
              <span style={{ alignSelf:'stretch', display:'flex', alignItems:'center', padding:'0 9px', margin:'3px 0', background:'var(--ra-green)', color:'var(--uc-black)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>GO</span>
            </div>
          </div>
        </div>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            {/* announcement */}
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>NET 30 TERMS · BULK PRICING · MADE IN USA OPTIONS</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #0A0A0A 0%, #1E2A10 55%, #4E7012 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--ra-green)' }}>CUSTOM RECYCLING BINS</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Showcase<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>yourself.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'var(--ra-green)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Get a mockup →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['OFFICES','SCHOOLS','STADIUMS','CITIES','HOTELS'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            <div style={{ display:'flex', gap:13, padding:'10px 12px 8px' }}>{['Best Sellers','New','Made in USA'].map((t,i)=>(<span key={t} style={{ paddingBottom:4, fontFamily:'var(--font-display)', fontWeight:700, fontSize:9, color:i===0?'var(--fg-1)':'var(--fg-3)', borderBottom:i===0?'2px solid var(--uc-black)':'2px solid transparent' }}>{t}</span>))}</div>
            {/* product grid */}
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['14-Qt Recycling Bin','$12.95','#3B6FA8','BEST'],['Deskside Sorter','$34.00','#2B2B2B','NEW'],['3-Stream Station','$1,249','#8B6B3D','USA'],['Dog Waste Station','$389.00','#7DAD1D',null]].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
                  <div style={{ aspectRatio:'1/0.86', borderRadius:5, background:`linear-gradient(150deg, ${p[2]}, var(--uc-bone))`, position:'relative' }}>{p[3] && <span style={{ position:'absolute', top:5, left:5, padding:'1px 6px', ...raBadge(p[3]), borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5, fontWeight:800 }}>{p[3]}</span>}<span style={{ position:'absolute', bottom:5, right:5, width:16, height:16, borderRadius:999, background:'var(--uc-paper)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'var(--fg-1)' }}>＋</span></div>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Recycling','#3B6FA8'],['Trash','#2B2B2B'],['Compost','#8B6B3D'],['Custom','#7DAD1D']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--ra-green)' }}/>BELLOWS FALLS, VERMONT</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Recycling programs<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>you can be proud of.</span></span>
            </div>
            {/* overlapping image cards */}
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              <div style={{ position:'absolute', left:0, top:8, width:'58%', aspectRatio:'4/3', borderRadius:5, background:'linear-gradient(150deg, #3B6FA8, var(--uc-bone))', transform:'rotate(-4deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.45)' }}/>
              <div style={{ position:'absolute', right:6, top:0, width:'52%', aspectRatio:'3/4', borderRadius:5, background:'linear-gradient(150deg, #4E7012, var(--uc-bone))', transform:'rotate(6deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.4)' }}/>
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>GREEN MOUNTAIN STATE</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“Call us and a real person answers — and stays with you from first mockup to final delivery.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['48','States shipped'],['250','Bins in 3 weeks'],['Net 30','Account terms']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>Case Studies</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #7DAD1D, #2F4A12)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:'var(--ra-green)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>STADIUMS · 4 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>250 compost bins to Yankee Stadium in 3 weeks.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['How the MBTA keeps bottles riding','5 min','#3B6FA8'],['2-stream vs 3-stream stations','6 min','#8A8F8A'],['NYC-compliant bins, explained','4 min','#7DAD1D']].map((a,i)=>(
...
function BPContent() {
  const screens = [
    { kind: 'home',  label: 'Landing Pages', d: 'Short hero, industry wall, live product tabs, and a category grid — a merchandising surface that gets facility buyers to the full catalog fast.' },
    { kind: 'about', label: 'Brand Story', d: 'Vermont roots and proof — real people on the phone, Net 30 terms, and case studies that win procurement trust and rank for branded search.' },
    { kind: 'blog',  label: 'Case Studies & Guides', d: 'Case studies and how-to content engineered for SEO, GEO, and paid landing — each story a destination that routes to product.' }
  ];
  return (
    <BPSection id="content" n="07" label="Content" paper tail="STORY · SEO · GEO · PAID">
      ...
      <p style={{
        ...
        We build the full mobile content surface for Recycle Away — homepage,
        brand story, and case studies. Each screen is structured for SEO and generative
  ...
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            ...
            { tier:'03', name:'Experience & Engagement', note:'Customer-facing', tools:[['Shopify Search & Discovery','launch'],['Shopify Checkout Blocks','launch'],['Shopify Bundles','launch'],['Shopify Messaging','launch'],['Uncap Quotes','start']] },
            { tier:'02', name:'Data & Integration', note:'System of record', tools:[['Avalara','start'],['Matrixify','start']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        {/* Header */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) minmax(120px,auto)', gap:16, padding:'12px 18px', background:'#0F0F0F', borderBottom:'1px solid #1F1F1F', fontFamily:'var(--font-mono)', fontSize:10, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--uc-stone-500)' }}>
        ...
        {rows.map((r, i) => {
          ...
          return (
            <div key={i} style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) minmax(120px,auto)', gap:16, padding:'14px 18px', borderBottom: i < rows.length-1 ? '1px solid #1F1F1F' : 'none', alignItems:'center' }}>
        ...
        {/* Footer total */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) minmax(120px,auto)', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span style={{ gridColumn:'3 / 5', fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(18px,1.8vw,24px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right', whiteSpace:'nowrap' }}>~$2,440/mo</span>
        </div>
      </div>
...
function GfxQuotes() {
  ...
  const rows = [
    { id:'Q-2087', dt:'2026-05-28', co:'Harborview Hospitality',   cu:'Dale Wojcik',     it:'509',  tot:'$2,406.06', exp:'2026-06-12', st:'Viewed',   cm:true },
    { id:'Q-2086', dt:'2026-05-28', co:'Green Mountain Schools',  cu:'Stan Petryk',     it:'20',   tot:'$2,182.80', exp:'2026-06-15', st:'Draft' },
    { id:'Q-2085', dt:'2026-05-27', co:'City of Burlington Parks', cu:'Sandra Liu',      it:'1230', tot:'$5,074.55', exp:'2026-06-22', st:'Approved' },
    { id:'Q-2084', dt:'2026-05-27', co:'Northeast Transit Auth.', cu:'Carl Bregović',   it:'12',   tot:'$1,536.52', exp:'2026-06-20', st:'Viewed',   cm:true },
    { id:'Q-2083', dt:'2026-05-27', co:'Lakeside Medical Center', cu:'Erik Lindqvist',  it:'4',    tot:'$1,060.25', exp:'2026-06-09', st:'Paid',     arch:true },
    { id:'Q-2082', dt:'2026-05-08', co:'Summit Convention Ctr.',  cu:'Jen Park',        it:'4',    tot:'$2,708.16', exp:'2026-05-20', st:'Expired',  arch:true },
    { id:'Q-2081', dt:'2026-05-27', co:'Pinecrest University',   cu:'Travis McCready', it:'259',  tot:'$1,244.44', exp:'2026-06-14', st:'Paid',     arch:true }
  ];
...
function BPAgentic() {
  const caps = [
    { t: 'AI storefront search', d: 'Shoppers ask in plain language — "3-stream station for a hotel lobby" — and land on the right SKU.' },
  ...
  const chat = [
    { who: 'buyer', text: 'I need recycling for a 3-floor office — 60 desks and two break rooms.' },
    { who: 'agent', text: 'Here are the right bins at your trade-account pricing:' },
    { who: 'cards', items: [
      { t: 'Deskside Sorter · 7 gal', price: '$29.50', c: '#3B6FA8', qty: '×60', icon: <g stroke="#fff" strokeWidth="2.4" fill="none" strokeLinecap="round" strokeLinejoin="round"><path d="M12 14 H36 L33 42 H15 Z"/><path d="M9 14 H39"/><path d="M24 14 V42"/><path d="M18 22 H21 M27 22 H30"/></g> },
      { t: '3-Stream Station · Break room', price: '$1,149', c: '#7DAD1D', qty: '×2', icon: <g stroke="#fff" strokeWidth="2.4" fill="none" strokeLinecap="round" strokeLinejoin="round"><rect x="6" y="16" width="10" height="26" rx="1.5"/><rect x="19" y="16" width="10" height="26" rx="1.5"/><rect x="32" y="16" width="10" height="26" rx="1.5"/><path d="M4 16 H44 M6 11 Q24 6 42 11 V16"/></g> }
    ] },
    { who: 'buyer', text: 'Add both with our logo, ship to the Boston office, Net 30.' }
  ];
  return (
      ...
      <div style={{ marginTop: 'clamp(32px, 4vw, 56px)', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1.15fr)', gap: 'clamp(28px, 4vw, 64px)', alignItems: 'center' }}>
        ...
        <div style={{ border: '1px solid #1F1F1F', borderRadius: 12, background: '#121212', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '11px 16px', borderBottom: '1px solid #1F1F1F' }}>
            <span style={{ width: 18, height: 18, borderRadius: 999, background: 'var(--ra-green)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 800, color: 'var(--uc-black)' }}>✦</span>
            ...
            <span style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--uc-stone-500)' }}><span style={{ width: 5, height: 5, borderRadius: 999, background: 'var(--ra-green)' }}/>Online</span>
          </div>
          <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {m.items.map((p, j) => (
                    <div key={j} style={{ border: '1px solid #2B2B2B', borderRadius: 10, background: '#1A1A1A', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                      <div style={{ height: 96, background: `linear-gradient(140deg, ${p.c}, #14181F)`, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      ...
                      <div style={{ padding: '9px 10px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, color: 'var(--ra-green)' }}>{p.price}</span>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8.5, fontWeight: 800, color: 'var(--uc-black)', background: 'var(--ra-green)', borderRadius: 3, padding: '3px 9px' }}>ADD</span>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4, padding: '8px 12px', border: '1px solid #2B2B2B', borderRadius: 999 }}>
              ...
              <span style={{ width: 22, height: 22, borderRadius: 999, background: 'var(--ra-green)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: 'var(--uc-black)' }}>↑</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', borderTop: '1px solid #1F1F1F' }}>
          {caps.map((x, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: 'minmax(0,40px) minmax(0,1fr)', gap: 16, alignItems: 'baseline', padding: 'clamp(14px,1.7vw,20px) 0', borderBottom: '1px solid #1F1F1F', position: 'relative' }}>
              <span aria-hidden="true" style={{ position: 'absolute', top: 0, left: 0, width: 16, height: 2, background: 'var(--ra-green)' }}/>
...
function BPIntegrations() {
  ...
  const activity = [
    ['Order #SO-48217 pushed to connected system', '2 min ago'],
    ['Inventory delta · 1,284 items reconciled', '9 min ago'],
    ['Customer Harborview Hospitality created in connected system', '14 min ago']
  ];
  ...
  return (
    <BPSection id="integrations" n="11" label="Integrated" tail="HOW IT CONNECTS">
      ...
      <p style={{ marginTop:'clamp(18px, 2.2vw, 26px)', maxWidth:640, fontFamily:'var(--font-serif)', fontSize:'clamp(15px, 1.3vw, 18px)', lineHeight:1.5, color:'var(--fg-2)' }}>
        Uncap Connect runs as a Shopify-embedded app — a real-time, bidirectional system
        ...
        <div style={{ display:'flex', background:'#F1F1F1' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:14, flexWrap:'wrap' }}>
            <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
              ...
              <span style={{ fontFamily:pFont, fontSize:12.5, color:'#616161' }}>System integration</span>
            </div>
            <div style={{ display:'flex', gap:8 }}>
          ...
          <Card>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
              <div style={{ display:'flex', alignItems:'center', gap:11 }}>
                <span style={{ width:34, height:34, borderRadius:8, background:'#101A2B', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:pFont, fontSize:13, fontWeight:800, color:'#9FB4FF' }}>⇄</span>
                <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
                  <span style={{ fontFamily:pFont, fontSize:13.5, fontWeight:700, color:'#1A1A1A' }}>Connected System</span>
                  <span style={{ fontFamily:pFont, fontSize:11.5, color:'#616161' }}>Production · Last sync 2 min ago</span>
                </div>
              </div>
          ...
          <Card pad={false}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.3fr) clamp(48px,6vw,72px) minmax(0,1.3fr) minmax(0,90px)', gap:'clamp(8px,1.2vw,16px)', padding:'8px 16px', background:'#FAFAFA', borderBottom:'1px solid #E3E3E3' }}>
              {['Shopify','','Connected system','Status'].map((h,i)=>(<span key={i} style={{ fontFamily:pFont, fontSize:10.5, fontWeight:600, color:'#616161', textAlign:i===3?'right':'left' }}>{h}</span>))}
            </div>
            {maps.map((m,i)=>(
          ...
          <div style={{ marginTop:'clamp(30px, 4vw, 48px)', border:'1px solid var(--line-2)', borderRadius:10, overflow:'hidden', background:'var(--uc-paper)', boxShadow:'0 24px 60px -34px rgba(10,10,10,0.4)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:cols, alignItems:'center', borderBottom:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px' }}>
                <span style={{ width:26, height:26, borderRadius:6, background:'#121118', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-hero)', fontSize:12, fontWeight:800, color:'#8FA0FF' }}>S4</span>
                <div style={{ display:'flex', flexDirection:'column', gap:1 }}>
                  <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:14, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Shift4Shop</span>
...
function BPApproveButton() {
  const APPROVED_KEY = 'recycleaway_approved_v1';
