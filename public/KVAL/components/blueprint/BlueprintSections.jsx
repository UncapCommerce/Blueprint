    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 019 · PREPARED AUGUST 2026 · CONFIDENTIAL</span>
        </div>
    ...
    <BPSection id="summary" n="01" label="Executive Summary" paper tail="TL;DR">
      ...
      <div style={{
        ...
        {[
          ...
          { v: 'Unified', l: 'B2C, B2B, POS on Shopify integrated with Salesforce' },
...
function BPApproach() {
  const steps = [
    ...
    { n: 'Phase 2', t: 'Build', d: 'Storefront, B2B, Salesforce sync. Fixed scope, senior team, no surprises.', wk: '20 weeks' },
...
function BPScope() {
  const layers = [
    ...
    { code: '05', t: 'SF Integration',   d: 'Bidirectional, real-time sync native to Shopify Plus.' },
...
function BPGantt() {
  const groups = [
    ...
    { name: 'Production', range: 'WK 06–18', tasks: [
      ...
      { n: '06', t: 'Data Migration',             tag: 'Migrate',  s: 11, e: 15, range: 'W11–15' },
      { n: '07', t: 'Integration',                tag: 'Connect',  s: 6,  e: 16, range: 'W6–16' },
...
function GfxChrome({ children }) {
  return (
    <div style={{ ...KV_VARS, background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>GENUINE KVAL PARTS · FREE FREIGHT OVER $999</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <span style={{ display:'flex', alignItems:'center', gap:6 }}><WireLogo h={15}/></span>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Machines','Parts','Saws','Merch'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Machines', items: ['Handler','Commander','Architect','Feeder / Stacker'] },
    { h: 'Parts', items: ['Electrical','Pneumatics','Overhead Section','Back Section'] },
    { h: 'Machinery Lines', items: ['Entry Door Line','Jamb Line','Automatic Saws','Bifold & Patio'] },
    { h: 'Featured', items: ['Parts Catalog','New · Q3','Kval Merchandise'] }
  ];
  return (
    <GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Prox Sensor M18','$64.00','ea','#7E8C8A','OEM','4.8','212'],
    ['Solenoid Valve','$112.00','24V','#A9B7B2',null,'4.6','158'],
    ['Vacuum Cup 40mm','$18.50','2 pk','#CDD8D5','NEW','4.9','64'],
    ['Drive Belt 92"','$86.00','ea','#8A9494',null,'4.7','309'],
    ['Servo Motor','$1,240','750 W','#C8A14E','PRO','4.9','47'],
    ['Saw Blade 14"','$210.00','60 T','#93A19E',null,'4.5','521']
  ];
  const facets = [
    { h: 'Section', items: [['Electrical', true],['Pneumatics', true],['Overhead', false],['Back Section', false],['Feeder', false]] },
    { h: 'Machine', items: [['Handler', true],['Commander', false],['Architect', false]] },
    { h: 'Availability', items: [['In stock', true],['Ships today', false],['Made to order', false]] }
  ];
  return (
    <div style={{ ...KV_VARS, background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <WireLogo h={14}/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Machines','Parts','Saws','Merch'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:110, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search parts…</span>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / PARTS BY MACHINE / HANDLER</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Handler Parts</span>
        </div>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Electrical ✕','Handler ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'104px 1fr', borderTop:'1px solid var(--line-1)' }}>
        ...
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:11 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$5</span><span>$2,400</span></div>
          </div>
        </div>
        ...
        <div style={{ padding:10, display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:8 }}>
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#3F8B5D' }}>Trade price ✓</span>
            </div>
          ))}
        </div>
      </div>
...
function GfxCart() {
  const lines=[['Prox Sensor M18','10+','$58.90','$589'],['Drive Belt 92"','5+','$82.00','$410'],['Vacuum Cup 40mm','40+','$16.20','$648']];
  const grid=[['Prox Sensor M18','$64.00','#7E8C8A','OEM'],['Solenoid Valve','$112.00','#A9B7B2',null],['Vacuum Cup 40mm','$18.50','#CDD8D5','NEW'],['Drive Belt 92"','$86.00','#8A9494',null],['Servo Motor','$1,240','#C8A14E',null],['Saw Blade 14"','$210.00','#93A19E',null]];
  return (
    <div style={{ ...KV_VARS, position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* Full CATEGORY page behind */}
      <div>
        <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>GENUINE KVAL PARTS · FREE FREIGHT OVER $999</div>
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          <WireLogo h={14}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Machines','Parts','Saws','Merch'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
        ...
        <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
          <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / PARTS / ELECTRICAL</span>
            <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Electrical Parts</span>
          </div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, color:'var(--fg-3)' }}>48 products · Sort ▾</span>
        </div>
        {/* filter chips */}
        <div style={{ display:'flex', gap:6, padding:'0 14px 11px' }}>{['Sensors','Relays','Drives','In stock'].map((f,i)=>(<span key={f} style={{ padding:'3px 9px', borderRadius:999, border:'1px solid', borderColor:i===0?'var(--uc-black)':'var(--line-1)', background:i===0?'var(--uc-black)':'transparent', color:i===0?'var(--uc-paper)':'var(--fg-2)', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700 }}>{f}</span>))}</div>
        {/* product grid */}
      ...
      <div style={{
        ...
        {/* lines */}
        {lines.map((l,i)=>(
          <div key={i} style={{ padding:'6px 13px', borderBottom:'1px solid var(--line-1)', display:'grid', gridTemplateColumns:'26px 1fr auto', gap:9, alignItems:'center' }}>
        ...
        <div style={{ padding:'7px 13px', background:'var(--uc-cream)', borderBottom:'1px solid var(--line-1)' }}>
          ...
          <div style={{ display:'flex', gap:6 }}>{[['Relay 24V','$22'],['Cable 5m','$34'],['Fuse Kit','$18']].map((u,i)=>(<div key={i} style={{ flex:1, border:'1px solid var(--line-1)', borderRadius:3, padding:5, display:'flex', flexDirection:'column', gap:3, background:'var(--uc-paper)' }}><div style={{ aspectRatio:'1/0.7', background:'var(--uc-stone-200)', borderRadius:2 }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7, color:'var(--fg-1)' }}>{u[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-black)' }}>+ {u[1]}</span></div>))}</div>
        </div>
        {/* featured upsell product */}
        <div style={{ padding:'7px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          <span style={{ width:18, height:18, borderRadius:3, background:'linear-gradient(135deg, #8FB0AE, #4E6A6B)', flexShrink:0 }}/>
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>PM Service Kit <span style={{ color:'var(--uc-signal)' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>$189 · belts, filters, fittings</div></div>
        ...
        <div style={{ marginTop:'auto', padding:'10px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$1,647</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'#3F8B5D', marginBottom:7 }}><span>Tier savings</span><span>−$142</span></div>
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'var(--uc-black)', color:'var(--uc-paper)', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · NET-30 · $1,505</span>
        </div>
      </div>
    </div>
  );
...
function GfxPDP() {
  return (
    <div style={{ ...KV_VARS, background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / PARTS / HANDLER</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#27484a','#2F3B3C','#8A9494','#C8A14E'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #27484a, #3E6B6D 60%, #132022)', position:'relative', overflow:'hidden' }}>
            ...
            <svg viewBox="0 0 100 100" style={{ position:'absolute', bottom:'4%', right:'2%', width:'80%', opacity:0.92 }}><rect x="16" y="36" width="42" height="30" rx="3" fill="none" stroke="rgba(255,255,255,0.55)" strokeWidth="1.7"/><path d="M58 46 L76 46 L76 56 L58 56" fill="none" stroke="rgba(255,255,255,0.55)" strokeWidth="1.7"/><path d="M22 36 V29 M30 36 V29 M38 36 V29 M46 36 V29 M54 36 V29" stroke="rgba(255,255,255,0.4)" strokeWidth="1.4"/><circle cx="78" cy="51" r="4.5" fill="rgba(255,255,255,0.18)"/></svg>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>GENUINE KVAL · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>SERVO MOTOR<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>750W · Handler line</span></div>
          ...
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#27484a','#1F1F1F','#C8A14E','#8A9494'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['Servo Motor 750W','$1,240','#27484a'],['Drive Cable 5m','$96','#1F1F1F'],['Mount Kit','$54','#8A9494']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$1,390</span></span>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“Dropped in, wired up — line was back up within the hour.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Drive Cable 5m','$96','#27484a'],['Encoder Unit','$210','#8A9494'],['Mount Kit','$54','#2F3B3C'],['Contactor 24V','$48','#C8A14E']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ ...KV_VARS, background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <WireLogo h={14}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>PACIFIC DOOR & MILLWORK</span>
          <span style={{ width:18, height:18, borderRadius:999, background:'linear-gradient(135deg,#8FB0AE,#33565A)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800, color:'#fff' }}>P</span>
        </span>
      </div>
...
function GfxHomepage() {
  const nav = ['Machines','Parts','Saws','Merch','Trade'];
  const mega = [
    { h: 'By Machine', items: ['Handler','Commander','Architect','Feeder / Stacker','Automatic Saws','All machines'] },
    { h: 'By Section', items: ['Electrical','Pneumatics','Overhead Section','Back Section'] },
    { h: 'Quick Links', items: ['New Arrivals','Parts Catalog','Trade Pricing','Kval Merchandise'] }
  ];
  const brands = ['HANDLER','COMMANDER','ARCHITECT','BIFOLD','SAWS'];
  const feat = [['Prox Sensor M18','$64.00','#7E8C8A','OEM'],['Solenoid Valve 24V','$112.00','#A9B7B2',null],['Vacuum Cup 40mm','$18.50','#CDD8D5','NEW'],['Servo Motor 750W','$1,240','#C8A14E','PRO']];
  return (
    <div style={{ ...KV_VARS, background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>GENUINE KVAL PARTS · FREE FREIGHT OVER $999</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:120, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search parts…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #142223 0%, #1F3638 55%, #3E6B6D 135%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>OEM PARTS · SHIP SAME-DAY</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>The right part,<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>first time.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Shop parts →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--uc-signal)', whiteSpace:'nowrap' }}>PARTS BY MACHINE</span>
        {['Machine','Model','Section'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
      ...
      {/* brand strip */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 16px', borderBottom:'1px solid var(--line-1)' }}>{brands.map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:10, letterSpacing:'-0.02em', color:'var(--fg-3)', whiteSpace:'nowrap' }}>{b}</span>))}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #33565A, #16282A)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EST. 1947</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Machinery built in Petaluma since 1947 — and every part that keeps it running.</span>
...
function GfxGarage() {
  const machines=[['HANDLER 990','SN 48217 · 2019','#27484a'],['COMMANDER','SN 31205 · 2021','#3E6B6D']];
  const parts=[['Drive Belt 92"','Fits HANDLER 990','$86.00'],['Prox Sensor M18','Fits HANDLER 990','$64.00'],['PM Service Kit','Fits both machines','$189.00']];
  const nav=[['◧','Dashboard',false],['▤','Orders',false],['⚙','My Garage',true],['♡','Saved lists',false],['❏','Quotes',false]];
  return (
    <div style={{ ...KV_VARS, background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>CUSTOMER GARAGE · PARTS MATCHED TO YOUR MACHINES</div>
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <WireLogo h={14}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>PACIFIC DOOR & MILLWORK</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'112px minmax(0,1fr)', gap:1, background:'var(--line-1)' }}>
        <div style={{ background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:3 }}>
          {nav.map((n,i)=>(<div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'5px 7px', borderRadius:3, background:n[2]?'var(--uc-black)':'transparent' }}><span style={{ fontSize:9, color:n[2]?'var(--uc-signal)':'var(--fg-3)' }}>{n[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:n[2]?'var(--uc-paper)':'var(--fg-2)' }}>{n[1]}</span></div>))}
          <div style={{ marginTop:'auto', padding:'8px 7px 0' }}><div style={{ fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)', lineHeight:1.5 }}>REGISTERED<br/><span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:13, letterSpacing:'-0.02em', color:'var(--fg-1)' }}>2</span> machines</div></div>
        </div>
        <div style={{ background:'var(--uc-paper)', display:'flex', flexDirection:'column' }}>
          <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
            <div>
              <div style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>YOUR EQUIPMENT</div>
              <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:17, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>My Garage</div>
            </div>
          </div>
          {/* machine cards */}
          <div style={{ padding:'0 14px 10px', display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)', gap:8 }}>
            {machines.map((m,i)=>(<div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:4, padding:'6px 10px', display:'flex', alignItems:'center', gap:8, minWidth:0 }}>
              <span style={{ width:22, height:22, borderRadius:4, background:`linear-gradient(140deg, ${m[2]}, var(--uc-bone))`, flexShrink:0 }}/>
              <div style={{ minWidth:0, overflow:'hidden' }}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:11, letterSpacing:'-0.02em', color:'var(--fg-1)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{m[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{m[1]}</div></div>
              <span style={{ marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'#3F8B5D', whiteSpace:'nowrap' }}>ACTIVE ✓</span>
            </div>))}
          </div>
          {/* machine diagram */}
          <div style={{ margin:'0 14px 10px', border:'1px solid var(--line-1)', borderRadius:4, background:'var(--uc-bone)', position:'relative', overflow:'hidden', height:60 }}>
            <svg viewBox="0 0 220 70" preserveAspectRatio="xMidYMid meet" style={{ position:'absolute', inset:0, width:'100%', height:'100%', padding:6, boxSizing:'border-box' }}>
              <rect x="18" y="22" width="96" height="34" rx="2" fill="none" stroke="#27484a" strokeWidth="1.6"/>
              <path d="M18 30 H114 M40 22 V56 M88 22 V56" stroke="#27484a" strokeWidth="0.9" opacity="0.55"/>
              <rect x="48" y="10" width="32" height="12" rx="2" fill="none" stroke="#27484a" strokeWidth="1.2"/>
              <path d="M114 34 H150 M114 46 H150" stroke="#27484a" strokeWidth="1.2"/>
              <rect x="150" y="26" width="28" height="28" rx="2" fill="none" stroke="#27484a" strokeWidth="1.4"/>
              <circle cx="164" cy="40" r="6.5" fill="none" stroke="#27484a" strokeWidth="1.2"/>
              <path d="M26 56 V62 M106 56 V62 M158 54 V62 M170 54 V62" stroke="#27484a" strokeWidth="1.4"/>
              <circle cx="64" cy="39" r="2.2" fill="#27484a"/><circle cx="96" cy="48" r="2.2" fill="#27484a"/><circle cx="164" cy="40" r="2" fill="#27484a"/>
              <path d="M64 39 L64 14 L128 14" stroke="#27484a" strokeWidth="0.8" strokeDasharray="2 2"/>
              <path d="M96 48 L96 66 L128 66" stroke="#27484a" strokeWidth="0.8" strokeDasharray="2 2"/>
            </svg>
            <span style={{ position:'absolute', top:6, left:8, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HANDLER 990 · SCHEMATIC</span>
            <span style={{ position:'absolute', bottom:6, right:8, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#3F8B5D' }}>3 WEAR POINTS TRACKED</span>
          </div>
          {/* assigned parts */}
          <div style={{ padding:'0 14px 12px' }}>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', paddingBottom:6 }}>PARTS FOR YOUR MACHINES</div>
            {parts.map((r,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) auto auto auto', gap:8, alignItems:'center', padding:'6px 0', borderTop:'1px solid var(--line-1)' }}>
              <span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:9, color:'var(--fg-1)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{r[0]}</span>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>{r[1]}</span>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, color:'var(--fg-1)' }}>{r[2]}</span>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'var(--uc-black)', background:'var(--uc-signal)', padding:'2px 7px', borderRadius:2 }}>Add</span>
            </div>))}
          </div>
        </div>
      </div>
...
function BPPerformance() {
  const groups = [
    ...
    {
      kpi: 'Customer Garage',
      sub: 'Every machine they own, with its parts one click away.',
      points: [
        { t: 'Machine Registry', d: 'Your team assigns each machine — model and serial — to the customer’s account at purchase; the garage remembers.' },
        { t: 'Assigned Parts Catalog', d: 'A catalog filtered to their equipment: only the parts, belts, and kits that fit what they own.' },
        { t: 'One-Click Restock', d: 'Wear parts and service kits reordered per machine, without hunting through the full catalog.' }
      ],
      gfx: <GfxGarage/>
    }
  ];
...
function GfxPhone({ kind }) {
  ...
  return (
    <div style={{
      ...KV_VARS, background:'var(--uc-paper)', border:'1px solid var(--line-2)', borderRadius:18,
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>GENUINE KVAL PARTS · FREE FREIGHT OVER $999</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #142223 0%, #1F3638 55%, #3E6B6D 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>OEM PARTS · SHIP SAME-DAY</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>The right part,<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>first time.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Shop parts →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['HANDLER','COMMANDER','ARCHITECT','BIFOLD','SAWS'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Prox Sensor M18','$64.00','#7E8C8A','OEM'],['Solenoid Valve','$112.00','#A9B7B2',null],['Vacuum Cup 40mm','$18.50','#CDD8D5','NEW'],['Servo Motor 750W','$1,240','#C8A14E','DEAL']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              <div style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:6 }}>SHOP BY SECTION</div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Electrical','#A9B7B2'],['Pneumatics','#93A19E'],['Belts & Blades','#C8A14E'],['Motors','#27484a']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--uc-signal)' }}/>EST. 1947 · PETALUMA, CA</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Door machinery<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>since 1947.</span></span>
            </div>
            {/* overlapping image cards */}
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              <div style={{ position:'absolute', left:0, top:8, width:'58%', aspectRatio:'4/3', borderRadius:5, background:'linear-gradient(150deg, #A9B7B2, var(--uc-bone))', transform:'rotate(-4deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.45)' }}/>
              <div style={{ position:'absolute', right:6, top:0, width:'52%', aspectRatio:'3/4', borderRadius:5, background:'linear-gradient(150deg, #3E6B6D, var(--uc-bone))', transform:'rotate(6deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.4)' }}/>
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>PETALUMA · 1947</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“Since 1947 we've built the machines that hang America's doors — and we stock every part that keeps them running.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['1947','Founded'],['79yr','In trade'],['OEM','Parts']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>Service Guides</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #A9B7B2, #33565A)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>MAINTENANCE · 7 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Preventive maintenance, by the book.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['Servo alignment in 20 minutes','5 min','#7E8C8A'],['Reading pneumatic schematics','6 min','#8A9494'],['When to replace drive belts','8 min','#C8A14E']].map((a,i)=>(
...
function BPContent() {
  const screens = [
    { kind: 'home',  label: 'Home Page', d: 'Short hero, brand wall, live product tabs, and a department grid — a merchandising surface that gets pros to the full catalog fast.' },
    { kind: 'about', label: 'Info Pages', d: 'Supplier story and proof — branch network and credentials that win contractor trust and rank for branded search.' },
    { kind: 'blog',  label: 'Policy Pages', d: 'How-to and spec content engineered for SEO, GEO, and paid landing — each guide a destination that routes to product.' }
  ];
  return (
...
function BPTechStack() {
  const rows = [
    ...
    { name: 'Uncap Connect',               fn: 'Salesforce ↔ Shopify integration',    pri: 'start',  cost: '$390/mo' },
    { name: 'KVAL My Garage',              fn: 'Custom app for Garage functionality', pri: 'start',  cost: '–' },
    ...
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            ...
            { tier:'03', name:'Experience & Engagement', note:'Customer-facing', tools:[['Shopify Search & Discovery','launch'],['Shopify Checkout Blocks','launch'],['Shopify Bundles','launch'],['Uncap Quotes','start'],['KVAL My Garage','start']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) 120px', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontSize:13, color:'var(--uc-stone-500)' }}>Platform, apps, and Salesforce integration</span>
          <span/>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(15px,1.4vw,19px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right', whiteSpace:'nowrap' }}>~$2,830/mo</span>
        </div>
      </div>
...
function BPAgentic() {
  ...
  const chat = [
    ...
    { who: 'cards', items: [
      { t: '½" #4 Rebar · 20 ft', price: '$8.40', c: '#7E8C8A', qty: '×24', icon: <g stroke="#fff" strokeWidth="2.4" strokeLinecap="round"><path d="M10 14 L40 14 M8 24 L38 24 M10 34 L40 34"/><path d="M10 14 q3 5 0 10 q-3 5 0 10 M24 14 q3 5 0 10 q-3 5 0 10 M38 14 q3 5 0 10 q-3 5 0 10"/></g> },
      { t: '16-ga Tie Wire · 3.5 lb', price: '$18.00', c: '#8A9494', qty: '×2', icon: <g stroke="#fff" strokeWidth="2.4" fill="none"><circle cx="24" cy="24" r="14"/><circle cx="24" cy="24" r="8"/><path d="M24 10 L24 4 M38 24 q5 -1 6 -4" strokeLinecap="round"/></g> }
    ] },
                ...
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {m.items.map((p, j) => (
                    <div key={j} style={{ border: '1px solid #2B2B2B', borderRadius: 10, background: '#1A1A1A', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                      <div style={{ height: 96, background: `linear-gradient(140deg, ${p.c}, #132022)`, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
function BPIntegrations() {
  ...
  const maps = [
    ...
    { l:'Fulfillment', d:'←', r:'Fulfillment Order' }
  ];
  const activity = [
        ...
        <div style={{ display:'flex', background:'#F1F1F1' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:14, flexWrap:'wrap' }}>
            <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
              ...
              <span style={{ fontFamily:pFont, fontSize:12.5, color:'#616161' }}>Salesforce integration</span>
            </div>
            <div style={{ display:'flex', gap:8 }}>
          ...
          <Card>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
              <div style={{ display:'flex', alignItems:'center', gap:11 }}>
                <span style={{ width:34, height:34, borderRadius:8, background:'#032D60', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:pFont, fontSize:13, fontWeight:800, color:'#9FD2FF' }}>SF</span>
...
function BPDelivery() {
  const milestones = [
    ...
    { wk: 'WK 08', t: 'Storefront alpha', s: 'Core theme + catalog in staging.' },
    { wk: 'WK 14', t: 'Integrations live', s: 'Salesforce sync + B2B flows validated.' },
    { wk: 'WK 16', t: 'Migration + QA', s: 'Data moved, redirects, full QA pass.' },
...
function BPRiskAssessment() {
  ...
  const risks = [
    ...
    { t: 'Salesforce integration complexity', sev: 'High',
      d: 'Bidirectional Salesforce sync is the hardest dependency — mismatches can block orders.',
...
function BPInvestment() {
  const work = [
    ...
    'Shopify-side Salesforce integration guidance',
  ...
  const schedule = [
    { n: '01', amount: '$19,200', due: 'Month 1' },
    { n: '02', amount: '$19,200', due: 'Month 2' },
    { n: '03', amount: '$19,200', due: 'Month 3' },
    { n: '04', amount: '$19,200', due: 'Month 4' },
    { n: '05', amount: '$19,200', due: 'Month 5' }
  ];
  return (
    <BPSection id="investment" n="15" label="Investment" dark tail="FIXED · NO SURPRISES">
      ...
      <div style={{
        ...
        <div style={{ padding: 'clamp(28px, 3.4vw, 52px)', display: 'flex', flexDirection: 'column', gap: 'clamp(18px, 2vw, 26px)' }}>
          ...
          <div style={{ padding: 'clamp(18px, 2vw, 26px) clamp(20px, 2.2vw, 28px)', background: 'var(--uc-signal)', borderRadius: 12, boxShadow: '0 22px 60px -26px rgba(232,255,82,0.45)' }}>
            ...
            <div style={{ marginTop: 4, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(52px, 6vw, 88px)', letterSpacing: '-0.05em', lineHeight: 0.85, color: 'var(--uc-black)' }}>$56k</div>
          </div>
      ...
      <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
        {[['Salesforce Integration', 'Bidirectional sync via Uncap Connect', '$28k'], ['My Garage App', 'Custom app for Garage functionality', '$12k']].map((a, i) => (
          <div key={i} style={{ position: 'relative', overflow: 'hidden', background: '#0F0F0F', border: '1px solid #1F1F1F', borderRadius: 12, padding: 'clamp(20px, 2.2vw, 30px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--uc-stone-500)', marginBottom: 7 }}>Add-on {String(i + 1).padStart(2, '0')}</div>
              <div style={{ fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(20px, 2.2vw, 30px)', letterSpacing: '-0.035em', lineHeight: 0.95, color: 'var(--uc-paper)' }}>{a[0]}</div>
              <div style={{ marginTop: 6, fontFamily: 'var(--font-serif)', fontSize: 'clamp(13.5px, 1.1vw, 16px)', color: 'var(--uc-stone-300)' }}>{a[1]}</div>
            </div>
            <div style={{ fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(34px, 3.6vw, 54px)', letterSpacing: '-0.05em', lineHeight: 0.85, color: 'var(--uc-signal)', whiteSpace: 'nowrap' }}>{a[2]}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginTop: 'clamp(28px, 3.2vw, 44px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 'clamp(16px, 1.8vw, 22px)' }}>
          <span aria-hidden="true" style={{ width: 16, height: 2, background: 'var(--uc-signal)' }}/>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--uc-stone-500)' }}>Payment schedule by monthly installments</span>
        </div>
          ...
          {schedule.map((s, i) => (
            <div key={i} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10 }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--fg-2)' }}>{s.due}</span>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.1em', color: 'var(--fg-3)' }}>{s.n} / 05</span>
              </div>
              <div style={{ fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(20px, 2vw, 28px)', letterSpacing: '-0.035em', lineHeight: 0.9, color: 'var(--fg-1)' }}>{s.amount}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ marginTop: 6, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(34px, 3.6vw, 52px)', letterSpacing: '-0.045em', lineHeight: 0.9, color: 'var(--uc-paper)' }}>$16k</div>
          <div style={{ marginTop: 6, fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', color: 'var(--uc-stone-500)' }}>120 HRS</div>
        </div>
      </div>
    </BPSection>
  );
    ...
    <BPSection id="growth" n="16" label="Growth" paper tail="AFTER LAUNCH">
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 15, color: 'var(--fg-1)' }}>
          Uncap Growth — Core Plan · <span style={{ fontFamily: 'var(--font-hero)', fontWeight: 800 }}>$2.5k / month</span>
        </div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'kvalinc_approved_v1';
