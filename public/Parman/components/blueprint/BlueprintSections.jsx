...
const QP = '#1844a2';
function brandName() { return (window.__brand && window.__brand.name) || 'Parman'; }
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 018 · PREPARED AUGUST 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              { k: 'Client lead',  v: 'Colin Hockenberger',  s: 'Chief Operating Officer' },
              { k: 'Company', v: 'Parman Tractor & Equipment', s: '3570 Dickerson Pike, Nashville, TN 37207' },
              { k: 'Valid through', v: 'Aug 31, 2026',    s: '14 days' }
            ].map((c, i) => (
              <div key={i} style={{
    ...
    <BPSection id="summary" n="01" label="Executive Summary" paper tail="TL;DR">
      ...
      <div style={{
        ...
        {[
          ...
          { v: 'Unified', l: 'B2C, B2B, POS on Shopify integrated with RIMSS' },
...
function BPWhere() {
  const items = [
    { n: '01', t: 'No ecommerce storefront', tag: 'Storefront', impact: 'High', pct: 82, d: 'Custom-built, patched for years, costing more every renewal cycle.' },
...
function BPObjectives() {
  const items = [
    { n: '01', t: 'Seamless new build', d: 'Build new on Shopify without losing what works — while optimizing for growth.', metric: 'Zero',   unit: 'disruption',       tag: 'Build' },
...
function BPApproach() {
  const steps = [
    ...
    { n: 'Phase 2', t: 'Build', d: 'Storefront, B2B, RIMSS sync. Fixed scope, senior team, no surprises.', wk: '16 weeks' },
    { n: 'Phase 3', t: 'Grow', d: 'Import data, go live, then optimize conversion, AOV, and automation.', wk: '12 months' }
  ];
  return (
...
function BPScope() {
  const layers = [
    ...
    { code: '05', t: 'RIMSS Integration', d: 'Bidirectional, real-time sync native to Shopify Plus.' },
...
function BPGantt() {
  const groups = [
    ...
    { name: 'Production', range: 'WK 06–14', tasks: [
      ...
      { n: '06', t: 'Data Import',                tag: 'Data',     s: 9,  e: 12, range: 'W9–12' },
      { n: '07', t: 'Integration',                tag: 'Connect',  s: 5,  e: 13, range: 'W5–13' },
...
function ProSolLogo({ h = 12 }) { return <img src="assets/parman-logo.svg" alt="Parman" style={{ height: h, width: 'auto', display: 'block' }}/>; }
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>EMPLOYEE-OWNED · BULK & PACKAGED DELIVERY</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <span style={{ display:'flex', alignItems:'center', gap:6 }}><ProSolLogo h={13}/></span>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Lubricants','Fuels','Coolants','DEF'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Lubricants', items: ['Engine oils','Hydraulic fluids','Gear oils','Greases'] },
    { h: 'Fuels', items: ['On-road diesel','Off-road diesel','Gasoline','Tanks & delivery'] },
    { h: 'Coolants & DEF', items: ['Antifreeze','ELC 50/50','BlueDEF','Handling equipment'] },
    { h: 'Featured', items: ['Full Catalog','Tankeye™ Monitoring','Bulk program'] }
  ];
  return (
    <GfxChrome>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:1, background:'var(--line-1)' }}>
        {cols.map((c,ci)=>(
          <div key={ci} style={{ background:'var(--uc-paper)', padding:'12px 13px 16px', display:'flex', flexDirection:'column', gap:7 }}>
            ...
            {c.items.map((it,ri)=>(<span key={ri} style={{ fontFamily:'var(--font-display)', fontWeight:ci===3&&ri===0?700:500, fontSize:10.5, letterSpacing:'-0.01em', color: ci===0&&ri===0?'var(--uc-black)':'var(--fg-2)', display:'flex', alignItems:'center', gap:5 }}>{ci===0&&ri===0 && <span style={{ width:14, height:3, background:'#af3429' }}/>}{it}</span>))}
          </div>
        ))}
      </div>
    </GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Delo 400 15W-40','$18.40','/gal','#af3429','BULK','4.8','212'],
    ['AW Hydraulic 46','$12.10','5 gal','#C8A14E',null,'4.6','158'],
    ['BlueDEF 2.5 Gal','$14.80','2.5 gal','#3D6FA8','NEW','4.9','64'],
    ['EP2 Grease 10pk','$24.00','10 ct','#8A8780',null,'4.7','309'],
    ['ELC Coolant 50/50','$89.00','55 gal','#3F8B8B','PRO','4.9','47'],
    ['Gear Oil 80W-90','$16.20','/gal','#9A938A',null,'4.5','521']
  ];
  const facets = [
    { h: 'Category', items: [['Engine Oils', true],['Hydraulic', true],['Coolants', false],['DEF', false],['Greases', false]] },
    { h: 'Brand', items: [['Chevron', true],['Citgo', false],['Fuchs', false]] },
    { h: 'Availability', items: [['In stock', true],['Bulk delivery', false],['Will-call', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <ProSolLogo h={12}/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Lubricants','Fuels','Coolants','DEF'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / LUBRICANTS & COOLANTS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Lubricants & Coolants</span>
        </div>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Engine Oils ✕','Hydraulic ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'#af3429', fontWeight:700 }}>Clear all</span>
      </div>
          ...
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              <div style={{ aspectRatio:'1/1', background:`linear-gradient(140deg, ${p[3]}, var(--uc-bone))`, borderRadius:2, position:'relative' }}>
                {p[4] && <span style={{ position:'absolute', top:4, left:4, padding:'1px 5px', background:p[4]==='NEW'?'#af3429':(p[4]==='PRO'?'var(--uc-black)':'#3F8B5D'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800 }}>{p[4]}</span>}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:3 }}><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'#af3429' }}>★★★★★</span><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'var(--fg-3)' }}>{p[5]} ({p[6]})</span></div>
...
function GfxCart() {
  const lines=[['Delo 400 15W-40 · Case','40+','$22.80','$912'],['ELC Coolant · Drum','5+','$298','$1,490'],['BlueDEF 2.5 Gal','80+','$6.10','$488']];
  const grid=[['Delo 400 15W-40','$18.40','#af3429','BULK'],['BlueDEF 2.5 Gal','$14.80','#3D6FA8',null],['AW Hydraulic 46','$12.10','#C8A14E','NEW'],['EP2 Grease 10pk','$24.00','#8A8780',null],['ELC Coolant 50/50','$89.00','#3F8B8B',null],['Gear Oil 80W-90','$16.20','#9A938A',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          ...
          <ProSolLogo h={12}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Lubricants','Fuels','Coolants','DEF'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{
        ...
        {lines.map((l,i)=>(
          <div key={i} style={{ padding:'6px 13px', borderBottom:'1px solid var(--line-1)', display:'grid', gridTemplateColumns:'26px 1fr auto', gap:9, alignItems:'center' }}>
            ...
            <div><div style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:10, color:'var(--fg-1)' }}>{l[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>qty {l[1]} · <span style={{ color:'#af3429', fontWeight:700 }}>{l[2]}</span></div></div>
        ...
        <div style={{ padding:'9px 13px', background:'var(--uc-cream)', borderBottom:'1px solid var(--line-1)' }}>
          ...
          <div style={{ display:'flex', gap:6 }}>{[['Drum Pump','$68'],['Spill Kit','$42'],['Funnel Set','$18']].map((u,i)=>(<div key={i} style={{ flex:1, border:'1px solid var(--line-1)', borderRadius:3, padding:5, display:'flex', flexDirection:'column', gap:3, background:'var(--uc-paper)' }}><div style={{ aspectRatio:'1/0.55', background:'var(--uc-stone-200)', borderRadius:2 }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7, color:'var(--fg-1)' }}>{u[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-black)' }}>+ {u[1]}</span></div>))}</div>
        </div>
        {/* StorePro subscription upgrade */}
        <div style={{ padding:'8px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          <span style={{ width:18, height:18, borderRadius:999, border:'2px solid #F0A9A1', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}><span style={{ width:7, height:7, borderRadius:999, background:'#af3429' }}/></span>
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Add Tankeye™ Monitoring <span style={{ color:'#F0A9A1' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>$49/mo · tank telemetry · auto-replenish</div></div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'#fff', background:'#af3429', padding:'4px 8px', borderRadius:3 }}>ADD</span>
        </div>
        {/* totals + checkout */}
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / LUBRICANTS / ENGINE OILS</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#af3429','#3A3632','#3D6FA8','#3F8B8B'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #af3429, #6E1812 60%, #221110)', position:'relative', overflow:'hidden' }}>
            ...
            <svg viewBox="0 0 100 100" style={{ position:'absolute', bottom:'4%', right:'2%', width:'80%', opacity:0.92 }}><rect x="30" y="26" width="40" height="52" rx="5" fill="none" stroke="rgba(255,255,255,0.55)" strokeWidth="1.7"/><path d="M30 42 H70 M30 62 H70" stroke="rgba(255,255,255,0.4)" strokeWidth="1.4"/><ellipse cx="50" cy="26" rx="20" ry="5" fill="none" stroke="rgba(255,255,255,0.55)" strokeWidth="1.7"/><circle cx="58" cy="23" r="2.5" fill="rgba(255,255,255,0.3)"/></svg>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>CHEVRON · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>DELO 400 SDE<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>15W-40 Heavy Duty</span></div>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'#af3429' }}>★★★★★</span><span style={{ color:'var(--fg-3)' }}>4.9 · 128 reviews</span></div>
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#af3429','#1F1F1F','#3D6FA8','#3F8B8B'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Bulk & packaged','✓ Same-day delivery','✓ Tech support'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['Delo 400 Drum','$894','#af3429'],['ELC Coolant 50/50','$129','#3F8B8B'],['BlueDEF 2.5 Gal','$15','#3D6FA8']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$998</span></span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Viscosity','15W-40'],['Spec','API CK-4'],['Format','55-gal drum'],['Case pack','4 × 1 gal']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
        <div style={{ background:'var(--uc-cream)', padding:'11px 14px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['Tankeye™ monitoring — never run out of any lubricant.','Trade pricing and NET terms unlock at bulk volume.','A real person answers, every time you call.'].map((t,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'12px 1fr', gap:7, alignItems:'start' }}><span style={{ width:10, height:10, borderRadius:999, background:'#af3429', marginTop:1, display:'flex', alignItems:'center', justifyContent:'center' }}><svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="#fff" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/></svg></span><span style={{ fontFamily:'var(--font-serif)', fontSize:9, lineHeight:1.35, color:'var(--fg-2)' }}>{t}</span></div>))}</div>
        </div>
      </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'#af3429' }}>★★★★★</div>
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“Parman is part of our maintenance team. We never run out.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['BlueDEF 2.5 Gal','$15','#3D6FA8'],['Drum Pump','$68','#8A8780'],['Spill Kit','$54','#3A3632'],['EP2 Grease 10pk','$42','#C8A14E']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <ProSolLogo h={12}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>MILLER TRANSPORTATION</span>
          <span style={{ width:18, height:18, borderRadius:999, background:'linear-gradient(135deg,#af3429,#5E1611)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800, color:'#fff' }}>M</span>
        </span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'112px 1fr', gap:1, background:'var(--line-1)' }}>
        ...
        <div style={{ background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:3 }}>
          {nav.map((n,i)=>(<div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'5px 7px', borderRadius:3, background:n[2]?'var(--uc-black)':'transparent' }}><span style={{ fontSize:9, color:n[2]?'#F0A9A1':'var(--fg-3)' }}>{n[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:n[2]?'var(--uc-paper)':'var(--fg-2)' }}>{n[1]}</span></div>))}
        ...
        <div style={{ background:'var(--uc-paper)', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
            ...
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'#fff', background:'#af3429', padding:'4px 9px', borderRadius:3 }}>+ New order</span>
          </div>
          {/* stat tiles */}
          ...
          <div style={{ padding:'0 14px 12px' }}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'70px 1fr auto auto', gap:8, alignItems:'center', padding:'6px 0', borderTop:'1px solid var(--line-1)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'#fff', background:'#af3429', padding:'2px 7px', borderRadius:2 }}>Reorder</span>
            </div>))}
          </div>
        </div>
      </div>
...
function GfxHomepage() {
  const nav = ['Shop','Brands','Bulk','Tankeye','Trade'];
  const mega = [
    { h: 'By Department', items: ['Engine Oils','Hydraulic Fluids','Coolants & Antifreeze','Diesel Exhaust Fluid','Greases','Fuels'] },
    { h: 'By Industry', items: ['Transportation','Construction','Manufacturing','Agriculture'] },
    { h: 'Quick Links', items: ['Bulk Delivery','Tankeye™ Monitoring','Request a Quote','Clearance'] }
  ];
  const brands = ['CHEVRON','CITGO','BLUEDEF','FUCHS','DELO','HAVOLINE'];
  const feat = [['Delo 400 15W-40','$18.40','#af3429','BULK'],['BlueDEF 2.5 Gal','$14.80','#3D6FA8',null],['AW Hydraulic 46','$12.10','#C8A14E','NEW'],['ELC Coolant 50/50','$89.00','#3F8B8B','PRO']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>EMPLOYEE-OWNED · BULK & PACKAGED DELIVERY</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:120, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search oils, DEF, coolants…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #1B0F0D 0%, #55150F 55%, #af3429 135%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'#F0A9A1' }}>TOTALLY OUTRAGEOUS CUSTOMER SERVICE</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Lubricants & fuels,<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>delivered further.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'#af3429', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Request a quote →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'#F0A9A1', whiteSpace:'nowrap' }}>MATCH YOUR EQUIPMENT</span>
        ...
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:'#af3429', color:'#fff', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Find Fluids →</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #af3429, #2A1210)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EMPLOYEE-OWNED</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>We exist to create opportunities where they may not be available otherwise.</span>
          <span style={{ alignSelf:'flex-start', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', borderBottom:'1.5px solid #af3429', paddingBottom:1 }}>Our ESOP story →</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px' }}>
            ...
            <span style={{ display:'inline-flex', gap:6, alignItems:'center' }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, height:22, padding:'0 9px', background:'var(--uc-black)', borderRadius:5, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)' }}>⛒ Cart<span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', minWidth:11, height:11, padding:'0 2px', background:'#af3429', color:'#fff', borderRadius:999, fontSize:6, fontWeight:800 }}>6</span></span>
            </span>
          </div>
          {/* big open search */}
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search oils, DEF, coolants…</span>
              <span style={{ alignSelf:'stretch', display:'flex', alignItems:'center', padding:'0 9px', margin:'3px 0', background:'#af3429', color:'#fff', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>GO</span>
            </div>
          </div>
        </div>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>EMPLOYEE-OWNED · BULK & PACKAGED DELIVERY</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #1B0F0D 0%, #55150F 55%, #af3429 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'#F0A9A1' }}>TOTALLY OUTRAGEOUS CUSTOMER SERVICE</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Lubricants & fuels,<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>delivered further.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'#af3429', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Request a quote →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['CHEVRON','CITGO','BLUEDEF','FUCHS','DELO'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            <div style={{ display:'flex', gap:13, padding:'10px 12px 8px' }}>{['Best Sellers','New','Bulk'].map((t,i)=>(<span key={t} style={{ paddingBottom:4, fontFamily:'var(--font-display)', fontWeight:700, fontSize:9, color:i===0?'var(--fg-1)':'var(--fg-3)', borderBottom:i===0?'2px solid var(--uc-black)':'2px solid transparent' }}>{t}</span>))}</div>
            {/* product grid */}
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Delo 400 15W-40','$18.40','#af3429','BULK'],['BlueDEF 2.5 Gal','$14.80','#3D6FA8',null],['AW Hydraulic 46','$12.10','#C8A14E','NEW'],['ELC Coolant 50/50','$89.00','#3F8B8B','DEAL']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
                  <div style={{ aspectRatio:'1/0.86', borderRadius:5, background:`linear-gradient(150deg, ${p[2]}, var(--uc-bone))`, position:'relative' }}>{p[3] && <span style={{ position:'absolute', top:5, left:5, padding:'1px 6px', background:p[3]==='DEAL'?'#af3429':(p[3]==='NEW'?'var(--uc-black)':'#3F8B5D'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5, fontWeight:800 }}>{p[3]}</span>}<span style={{ position:'absolute', bottom:5, right:5, width:16, height:16, borderRadius:999, background:'var(--uc-paper)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'var(--fg-1)' }}>＋</span></div>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Engine Oils','#af3429'],['Hydraulic','#C8A14E'],['Coolants','#3F8B8B'],['DEF','#3D6FA8']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'#af3429' }}/>ESOP · EMPLOYEE-OWNED</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Owned by the<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>people who serve you.</span></span>
            </div>
            ...
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              ...
              <div style={{ position:'absolute', right:6, top:0, width:'52%', aspectRatio:'3/4', borderRadius:5, background:'linear-gradient(150deg, #af3429, var(--uc-bone))', transform:'rotate(6deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.4)' }}/>
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>ESOP · EST. 2015</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“We exist to create opportunities where they may not be available otherwise.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['100%','Employee-owned'],['2015','ESOP est.'],['4','Core values']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>Resources</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #af3429, #4A1712)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:'#af3429', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>FLEET · 7 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Never run out: Tankeye™ monitoring.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['Choosing the right coolant','5 min','#3F8B8B'],['DEF handling & storage','6 min','#3D6FA8'],['Grease compatibility 101','8 min','#C8A14E']].map((a,i)=>(
    ...
    <BPSection id="content" n="07" label="Content" paper tail="STORY · SEO · GEO · PAID">
      ...
      {(() => {
        const seo = [
          ...
          ['Title Tags', 'Carry over every title tag as-is unless there&rsquo;s a proven issue.'],
          ...
          ['Alt Text', 'Carry over all image alt text without loss.'],
...
function BPTechStack() {
  const rows = [
    ...
    { name: 'Uncap Connect',               fn: 'RIMSS ↔ Shopify integration',           pri: 'start',  cost: '$290/mo' },
    { name: 'Uncap Diagrams',              fn: 'Interactive parts diagrams',          pri: 'start',  cost: '$90/mo' },
    { name: 'Matrixify',                   fn: 'Bulk import / export',                pri: 'dev', cost: '$50/mo' }
  ];
  const priMeta = {
    ...
    dev:    { l: 'During development', c: 'transparent',     fg: 'var(--fg-3)', outline: true }
  };
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            ...
            { tier:'02', name:'Data & Integration', note:'System of record', tools:[['Uncap Connect','start'],['Uncap Diagrams','start'],['Matrixify','dev']] },
            { tier:'01', name:'Commerce Core', note:'Foundation', tools:[['Shopify','start']], core:true }
          ];
          const chip = { start:{ bg:'var(--uc-signal)', fg:'var(--uc-black)' }, launch:{ bg:'#FF8B37', fg:'var(--uc-black)' }, future:{ bg:'transparent', fg:'var(--uc-stone-300)', outline:true }, dev:{ bg:'transparent', fg:'var(--uc-stone-300)', outline:true } };
          return (
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        {/* Footer total */}
        <div style={{ display:'flex', alignItems:'center', gap:16, flexWrap:'wrap', padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontSize:13, color:'var(--uc-stone-500)' }}>Platform + apps + RIMSS integration</span>
          <span style={{ marginLeft:'auto', fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(18px,1.8vw,24px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right', whiteSpace:'nowrap' }}>~$2,820/mo</span>
        </div>
      </div>
...
function BPIntegrations() {
  ...
  const maps = [
    ...
    { l:'Products',    d:'→', r:'SO Lines' },
    { l:'Inventory',   d:'←', r:'Stock on hand' },
    { l:'Fulfillment', d:'←', r:'Item Fulfillment' }
  ];
  const activity = [
    ['Order #SO-48217 pushed to RIMSS', '2 min ago'],
    ['Inventory delta · 1,284 items reconciled', '9 min ago'],
    ['Customer ACME Contracting created in RIMSS', '14 min ago']
  ];
  ...
  return (
    <BPSection id="integrations" n="11" label="Integrated" tail="HOW IT CONNECTS">
      ...
      <p style={{ marginTop:'clamp(18px, 2.2vw, 26px)', maxWidth:640, fontFamily:'var(--font-serif)', fontSize:'clamp(15px, 1.3vw, 18px)', lineHeight:1.5, color:'var(--fg-2)' }}>
        Uncap Connect runs as a Shopify-embedded app — a real-time, bidirectional RIMSS
        ...
        <div style={{ display:'flex', background:'#F1F1F1' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:14, flexWrap:'wrap' }}>
            <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
              ...
              <span style={{ fontFamily:pFont, fontSize:12.5, color:'#616161' }}>RIMSS integration</span>
            </div>
            <div style={{ display:'flex', gap:8 }}>
          ...
          <Card>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
              <div style={{ display:'flex', alignItems:'center', gap:11 }}>
                <span style={{ width:34, height:34, borderRadius:8, background:'#101A2B', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:pFont, fontSize:13, fontWeight:800, color:'#9FB4FF' }}>R</span>
                <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
                  <span style={{ fontFamily:pFont, fontSize:13.5, fontWeight:700, color:'#1A1A1A' }}>RIMSS</span>
          ...
          <Card pad={false}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.3fr) clamp(48px,6vw,72px) minmax(0,1.3fr) minmax(0,90px)', gap:'clamp(8px,1.2vw,16px)', padding:'8px 16px', background:'#FAFAFA', borderBottom:'1px solid #E3E3E3' }}>
              {['Shopify','','RIMSS','Status'].map((h,i)=>(<span key={i} style={{ fontFamily:pFont, fontSize:10.5, fontWeight:600, color:'#616161', textAlign:i===3?'right':'left' }}>{h}</span>))}
            </div>
            {maps.map((m,i)=>(
...
function BPMigration() {
  const steps = [
    ...
    { t: 'Go live', s: 'Final delta import, 301 redirect map, DNS switch with zero lost orders.' }
  ];
  const stats = ['Products', 'Collections', 'Pages', 'Blogs', 'Customers', 'Companies', 'Orders'];
  return (
    <BPSection id="migration" n="12" label="Data" paper tail="IMPORT WITHOUT LOSS" vec="bgVector2">
      <BPHeadline>
        Data import,{' '}
      ...
      <p style={{ marginTop: 18, maxWidth: 600, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)' }}>
        We import every product, customer, and order into the new build — and re-map the
          ...
          <div style={{ marginTop:'clamp(30px, 4vw, 48px)', border:'1px solid var(--line-2)', borderRadius:10, overflow:'hidden', background:'var(--uc-paper)', boxShadow:'0 24px 60px -34px rgba(10,10,10,0.4)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:cols, alignItems:'center', borderBottom:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px' }}>
                <span style={{ width:26, height:26, borderRadius:6, background:'#0B4A33', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-hero)', fontSize:12, fontWeight:800, color:'#7CE3B1' }}>M</span>
                <div style={{ display:'flex', flexDirection:'column', gap:1 }}>
                  ...
                  <span style={{ fontFamily:'var(--font-mono)', fontSize:9, color:'var(--fg-3)' }}>Source data · 12,480 records</span>
                </div>
              </div>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:10, padding:'12px 16px' }}>
                <div style={{ display:'flex', flexDirection:'column', gap:1, alignItems:'flex-end' }}>
                  ...
                  <span style={{ fontFamily:'var(--font-mono)', fontSize:9, color:'var(--fg-3)' }}>New store · imported & verified</span>
                </div>
...
function BPDelivery() {
  const milestones = [
    ...
    { wk: 'WK 09', t: 'Integrations live', s: 'RIMSS sync + B2B flows validated.' },
    { wk: 'WK 11', t: 'Data import + QA', s: 'Data in, redirects, full QA pass.' },
...
function BPRiskAssessment() {
  ...
  const risks = [
    { t: 'Data import integrity', sev: 'High',
      d: 'Products, customers, and orders don\u2019t map cleanly from the source data.',
      m: 'Dry-run into staging, reconcile counts, and spot-check edge cases before launch.' },
    { t: 'RIMSS integration complexity', sev: 'High',
      d: 'Bidirectional RIMSS sync is the hardest dependency — mismatches can block orders.',
...
function BPInvestment() {
  ...
  const erpWork = [
    'Uncap Connect deployed and configured for RIMSS',
    'Bidirectional sync: products, inventory, customers, orders',
    'Field mapping, transformation, and business rules',
    'Staging dry-runs and record-level reconciliation',
    'Go-live cutover with sync monitoring and alerts'
  ];
  const schedule = [
    { n: '01', amount: '$17,000', due: 'Due at signup' },
    { n: '02', amount: '$17,000', due: 'Due at design approval' },
    { n: '03', amount: '$17,000', due: 'Due at integration go-live' },
    { n: '04', amount: '$17,000', due: 'Due at project completion' }
  ];
  return (
    <BPSection id="investment" n="15" label="Investment" dark tail="FIXED · NO SURPRISES">
      ...
      <div style={{
        ...
        <div style={{ padding: 'clamp(28px, 3.4vw, 52px)', display: 'flex', flexDirection: 'column', gap: 'clamp(18px, 2vw, 26px)' }}>
          <div>
            ...
            <div style={{ marginTop: 'clamp(14px, 1.6vw, 20px)', fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: 'clamp(17px, 1.6vw, 24px)', lineHeight: 1.25, color: 'var(--uc-stone-300)', textWrap: 'pretty' }}>Your new storefront, built for growth.</div>
          </div>
      ...
      {/* RIMSS Integration — second box */}
      <div style={{
        position: 'relative', overflow: 'hidden', marginTop: 'clamp(16px, 1.8vw, 24px)',
        background: '#0F0F0F', color: 'var(--uc-paper)',
        border: '1px solid #1F1F1F', borderRadius: 12,
        display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)'
      }}>
        ...
        <div style={{ padding: 'clamp(28px, 3.4vw, 52px)', display: 'flex', flexDirection: 'column', gap: 'clamp(18px, 2vw, 26px)' }}>
          <div>
            <h3 style={{ margin: 0, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(34px, 4vw, 60px)', letterSpacing: '-0.045em', lineHeight: 0.9, color: 'var(--uc-paper)' }}>RIMSS <span style={{ fontFamily: 'var(--font-serif)', fontWeight: 400, fontSize: '0.62em', letterSpacing: '-0.02em' }}>Integration</span></h3>
            <div style={{ marginTop: 'clamp(14px, 1.6vw, 20px)', fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: 'clamp(16px, 1.4vw, 21px)', lineHeight: 1.25, color: 'var(--uc-stone-300)', textWrap: 'pretty' }}>Your RIMSS system and your storefront, speaking in real time.</div>
          </div>
          <div style={{ padding: 'clamp(16px, 1.8vw, 24px) clamp(18px, 2vw, 26px)', background: 'var(--uc-signal)', borderRadius: 12, boxShadow: '0 22px 60px -26px rgba(232,255,82,0.45)', alignSelf: 'stretch' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--uc-black)' }}>Fixed price</div>
            <div style={{ marginTop: 4, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(44px, 4.6vw, 68px)', letterSpacing: '-0.05em', lineHeight: 0.85, color: 'var(--uc-black)' }}>$22k</div>
          </div>
        </div>
        <div style={{ padding: 'clamp(28px, 3.4vw, 52px)', borderLeft: '1px solid #1F1F1F' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--uc-stone-500)', marginBottom: 'clamp(18px, 2vw, 26px)' }}>The work</div>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ marginTop: 6, fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.08em', color: 'var(--uc-stone-300)' }}>100 hrs</div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'parmancorp_approved_v1';
