...
function brandName() { return (window.__brand && window.__brand.name) || 'Trusty-Cook'; }
function brandHandle() { return brandName().toLowerCase().replace(/[^a-z0-9]+/g, ''); }
// Client wordmark — used only inside the storefront mockups (Commerce + Content)
function TCLogo({ h = 9 }) { return <img src="assets/trusty-cook-svg.svg" alt="Trusty-Cook" style={{ height: h, width: 'auto', display: 'block', flexShrink: 0 }}/>; }
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 016 · PREPARED AUGUST 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              { k: 'Client lead',  v: 'Brock Trusty',  s: 'Engineering & Operations' },
              { k: 'Company', v: 'Trusty-Cook Inc.', s: '10530 E. 59th Street, Indianapolis, IN 46236' },
...
function BPWhere() {
  const items = [
    { n: '01', t: 'Aging tech stack', tag: 'Platform', impact: 'High', pct: 82, d: 'A WordPress build patched together with plugins and tools never made for real commerce.' },
    { n: '02', t: 'Outdated experience', tag: 'Storefront', impact: 'High', pct: 82, d: 'Unfriendly shopping experience and no mobile optimization.' },
    { n: '03', t: 'Limited optionality', tag: 'Systems', impact: 'Critical', pct: 94, d: 'Lack of ability to integrate and plug and play with modern advanced commerce tools and solutions.' },
...
function BPScope() {
  const layers = [
    ...
    { code: '05', t: 'QuickBooks Integration', d: 'Bidirectional, real-time sync native to Shopify.' },
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>EXPEDITE SERVICE · ORDER BY 10AM EDT, SHIPS SAME DAY</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <TCLogo h={10}/>
        <div style={{ display:'flex', gap:9, marginLeft:6, flexShrink:0 }}>{['Hammers','Liners','Hush Tubes','Wear'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)', whiteSpace:'nowrap' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Dead Blow Hammers', items: ['Standard','Slimline','Ball Peen','Bossing Mallets','Sledge'] },
    { h: 'Spindle Liners', items: ['Round ID','Hex ID','Multi-piece','Quick Six Sleeves'] },
    { h: 'Hush Tubes', items: ['Screw machines','Bar feeders','Custom lengths'] },
    { h: 'Featured', items: ['Expedite Service','Custom Wear Shoes','Distributor Program'] }
  ];
  return (
    <GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['TC24 Hammer','$62.00','24 oz','#CB202A','BEST','4.9','214'],
    ['Slimline','$74.00','32 oz','#A81C24',null,'4.8','156'],
    ['Liner 65mm','$186.00','Round','#3A3A3C','NEW','4.9','88'],
    ['Bossing Mallet','$58.00','2 lb','#B4353A',null,'4.7','132'],
    ['Hush Tube','$142.00','1.25"','#5A5A5C','PRO','4.9','61'],
    ['Ball Peen','$48.00','16 oz','#8E1B21',null,'4.8','297']
  ];
  const facets = [
    { h: 'Category', items: [['Hammers', true],['Liners', true],['Hush Tubes', false],['Wear Shoes', false],['Sleeves', false]] },
    { h: 'Machine', items: [['Haas', true],['Okuma', false],['Mazak', false]] },
    { h: 'Availability', items: [['In stock', true],['Expedite', false],['Custom', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>EXPEDITE SERVICE · ORDER BY 10AM EDT, SHIPS SAME DAY</div>
      <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <TCLogo h={9}/>
        <div style={{ display:'flex', gap:8, marginLeft:4, flexShrink:0 }}>{['Hammers','Liners','Hush Tubes'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)', whiteSpace:'nowrap' }}>{x}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
          <span style={{ width:84, minWidth:0, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 8px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8, flexShrink:0 }}>⌕</span><span style={{ minWidth:0, overflow:'hidden', textOverflow:'ellipsis' }}>Search…</span></span>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / DEAD BLOW HAMMERS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Dead Blow Hammers</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>186 results</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Hammers ✕','Liners ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-brand)', fontWeight:700 }}>Clear all</span>
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'minmax(0,104px) minmax(0,1fr)', borderTop:'1px solid var(--line-1)' }}>
        {/* faceted sidebar */}
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', minWidth:0, overflow:'hidden', display:'flex', flexDirection:'column', gap:11 }}>
          {facets.map((f,fi)=>(
            <div key={fi} style={{ display:'flex', flexDirection:'column', gap:6 }}>
              ...
              {f.items.map(([l,on],i)=>(<div key={i} style={{ display:'flex', gap:6, alignItems:'center' }}><span style={{ width:9,height:9,borderRadius:2, background:on?'var(--uc-black)':'var(--uc-paper)', border:'1px solid var(--line-2)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{on && <svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="var(--uc-paper)" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg>}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, color:on?'var(--fg-1)':'var(--fg-3)', fontWeight:on?700:400, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{l}</span></div>))}
            </div>
          ))}
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$40</span><span>$260</span></div>
          </div>
        </div>
        ...
        <div style={{ padding:10, display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:8 }}>
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:3, whiteSpace:'nowrap' }}><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'var(--uc-brand)' }}>★★★★★</span><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'var(--fg-3)' }}>{p[5]} ({p[6]})</span></div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', gap:3 }}><span style={{ fontFamily:'var(--font-mono)', fontSize:9, fontWeight:700, color:'var(--fg-1)', whiteSpace:'nowrap' }}>{p[1]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{p[2]}</span></div>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#3F8B5D' }}>Distributor price ✓</span>
            </div>
          ))}
        </div>
      </div>
...
function GfxCart() {
  const lines=[['TC24 Hammer · 24 oz','24','$54.00','$1,296'],['Liner 65mm · Round','6','$172.00','$1,032'],['Hush Tube · 1.25"','4','$134.00','$536']];
  const grid=[['TC24 Hammer','$62.00','#CB202A','BEST'],['Slimline','$74.00','#A81C24',null],['Liner 65mm','$186.00','#3A3A3C','NEW'],['Bossing Mallet','$58.00','#B4353A',null],['Hush Tube','$142.00','#5A5A5C',null],['Ball Peen','$48.00','#8E1B21',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          <TCLogo h={9}/>
          <div style={{ display:'flex', gap:8, marginLeft:4, flexShrink:0 }}>{['Hammers','Liners','Hush Tubes','Wear'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)', whiteSpace:'nowrap' }}>{x}</span>))}</div>
        ...
        <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
          <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / SPINDLE LINERS</span>
            <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Spindle Liners</span>
          </div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, color:'var(--fg-3)' }}>1,000s of specs · Sort ▾</span>
        </div>
        {/* filter chips */}
        <div style={{ display:'flex', gap:6, padding:'0 14px 11px' }}>{['Round ID','Hex ID','Multi-piece','In stock'].map((f,i)=>(<span key={f} style={{ padding:'3px 9px', borderRadius:999, border:'1px solid', borderColor:i===0?'var(--uc-black)':'var(--line-1)', background:i===0?'var(--uc-black)':'transparent', color:i===0?'var(--uc-paper)':'var(--fg-2)', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700 }}>{f}</span>))}</div>
        {/* product grid */}
      ...
      <div style={{
        ...
        {/* upsells — compact single row */}
        <div style={{ padding:'8px 13px', background:'var(--uc-cream)', borderBottom:'1px solid var(--line-1)', display:'flex', flexDirection:'column', gap:5 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)' }}>PAIRS WITH</span>
          <div style={{ display:'flex', flexWrap:'wrap', gap:5, minWidth:0 }}>{[['Face Caps','$12'],['Handle Grips','$16'],['Liner Grease','$24']].map((u,i)=>(<span key={i} style={{ display:'inline-flex', alignItems:'center', gap:4, border:'1px solid var(--line-1)', borderRadius:999, padding:'2px 7px', background:'var(--uc-paper)', whiteSpace:'nowrap' }}><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:6.5, color:'var(--fg-1)' }}>{u[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--uc-black)' }}>+{u[1]}</span></span>))}</div>
        </div>
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Upgrade to TC Pro <span style={{ color:'var(--uc-signal)' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>$49/mo · free freight · expedite priority</div></div>
        ...
        <div style={{ marginTop:'auto', padding:'10px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$2,864</span></div>
          ...
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'var(--uc-black)', color:'var(--uc-paper)', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · NET-30 · $2,554</span>
        </div>
      </div>
    </div>
  );
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / HAMMERS / STANDARD</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#CB202A','#1F1F1F','#8E1B21','#5A5A5C'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #CB202A, #8E1B21 60%, #241A1B)', position:'relative', overflow:'hidden' }}>
            <span style={{ position:'absolute', top:8, left:8, padding:'2px 7px', background:'var(--tc-red)', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>BEST SELLER</span>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>TRUSTY-COOK · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>TC24<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Standard Dead Blow Hammer</span></div>
          ...
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#CB202A','#1F1F1F','#8E1B21','#5A5A5C'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Lifetime warranty','✓ Made in Indiana','✓ Ships today'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['TC24 Hammer','$62','#CB202A'],['Ball Peen 16 oz','$48','#8E1B21'],['Face Cap Set','$18','#3A3A3C']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$128</span></span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Head weight','24 oz'],['Face material','Urethane'],['Handle','Steel core'],['Made in','Indianapolis']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“No rebound, no marring. We keep one at every machine.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Ball Peen 16 oz','$48','#CB202A'],['Bossing Mallet','$58','#8E1B21'],['Sledge 8 lb','$118','#3A3A3C'],['Face Cap Set','$18','#5A5A5C']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <TCLogo h={9}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>PRECISION MACHINE CO.</span>
...
function GfxHomepage() {
  const nav = ['Shop','Hammers','Liners'];
  const mega = [
    { h: 'By Product', items: ['Dead Blow Hammers','Spindle Liners','Hush Tubes','Wear Shoes','Guide Sleeves','Custom Urethane'] },
    { h: 'By Machine', items: ['Haas','Okuma','Mazak','Doosan / LNS'] },
    { h: 'Quick Links', items: ['Expedite Service','Request a Quote','Distributor Login','Bulk Orders'] }
  ];
  const brands = ['HAAS','OKUMA','MAZAK','DOOSAN','LNS','EDGE'];
  const feat = [['TC24 Hammer','$62.00','#CB202A','BEST'],['Liner 65mm','$186.00','#3A3A3C',null],['Hush Tube','$142.00','#5A5A5C','NEW'],['Bossing Mallet','$58.00','#8E1B21','PRO']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>EXPEDITE SERVICE · ORDER BY 10AM EDT, SHIPS SAME DAY</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <TCLogo h={10}/>
        <div style={{ display:'flex', gap:9, marginLeft:6, flexShrink:0 }}>{nav.map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)', position:'relative', whiteSpace:'nowrap' }}>{x}{i===0 && <span style={{ position:'absolute', left:0, right:0, bottom:-9, height:2, background:'var(--uc-black)' }}/>}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
          <span style={{ width:86, minWidth:0, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 8px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8, flexShrink:0 }}>⌕</span><span style={{ minWidth:0, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>Search…</span></span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #16161A 0%, #3A1B1E 55%, #CB202A 145%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'#FFFFFF' }}>EXPEDITE · SHIPS SAME DAY</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Machine-shop<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>urethane, in stock.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'var(--tc-red)', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Start your quote →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--uc-signal)', whiteSpace:'nowrap' }}>FIND YOUR MACHINE</span>
        {['Brand','Model','Drawtube ID'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Find Liner →</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #8E1B21, #201416)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EST. 1984</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Pioneers of polyurethane since 1984 — still made, by hand, in Indianapolis.</span>
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          ...
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search hammers, liners…</span>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>EXPEDITE SERVICE · ORDER BY 10AM EDT, SHIPS SAME DAY</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #16161A 0%, #3A1B1E 55%, #CB202A 145%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'#FFFFFF' }}>EXPEDITE · SHIPS SAME DAY</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Machine-shop<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>urethane, in stock.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'var(--tc-red)', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Start your quote →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['HAAS','OKUMA','MAZAK','DOOSAN','LNS'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['TC24 Hammer','$62.00','#CB202A','BEST'],['Liner 65mm','$186.00','#3A3A3C',null],['Hush Tube','$142.00','#5A5A5C','NEW'],['Ball Peen','$48.00','#8E1B21','DEAL']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              <div style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:6 }}>SHOP BY PRODUCT</div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Hammers','#CB202A'],['Liners','#3A3A3C'],['Hush Tubes','#8E1B21'],['Wear Shoes','#5A5A5C']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--uc-signal)' }}/>EST. 1984 · FAMILY-OWNED</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Pioneers of<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>polyurethane since 1984.</span></span>
            </div>
            {/* overlapping image cards */}
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              <div style={{ position:'absolute', left:0, top:8, width:'58%', aspectRatio:'4/3', borderRadius:5, background:'linear-gradient(150deg, #CB202A, var(--uc-bone))', transform:'rotate(-4deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.45)' }}/>
              <div style={{ position:'absolute', right:6, top:0, width:'52%', aspectRatio:'3/4', borderRadius:5, background:'linear-gradient(150deg, #3A3A3C, var(--uc-bone))', transform:'rotate(6deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.4)' }}/>
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>INDIANAPOLIS · 1984</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“We stick with what we know best — polyurethane, perfected over four decades.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['40+','Years'],['1,000s','Liner specs'],['1','US plant']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>News & Resources</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #CB202A, #5A1114)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:'var(--tc-red)', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>SPINDLE LINERS · 7 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Spindle liners as a milling solution.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['Quick Six guide sleeves, explained','5 min','#CB202A'],['Choosing a drawtube ID','6 min','#3A3A3C'],['Dead blow vs. sledge: when to use each','8 min','#8E1B21']].map((a,i)=>(
...
function BPTechStack() {
  const rows = [
    ...
    { name: 'Webgility',                   fn: 'QuickBooks ↔ Shopify integration',    pri: 'start',  cost: '–' },
    ...
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            ...
            { tier:'02', name:'Data & Integration', note:'System of record', tools:[['Webgility','start'],['Matrixify','start']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) 120px', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontSize:13, color:'var(--uc-stone-500)' }}>Platform + apps, before QuickBooks integration</span>
          <span/>
...
function ShopAdminNav({ activeApp }) {
  ...
  return (
    <div style={{ flexShrink:0, width:'clamp(132px, 15vw, 174px)', background:'#EBEBEB', borderRight:'1px solid #DEDEDE', padding:'12px 10px', display:'flex', flexDirection:'column', gap:1 }}>
      ...
      {/* Webgility — expandable */}
      <AppParent label="Webgility" glyph="W" open={activeApp==='connect'}/>
      {activeApp==='connect' && <>
  ...
  return (
    <BPSection id="integrations" n="11" label="Integrated" tail="HOW IT CONNECTS">
      ...
      <p style={{ marginTop:'clamp(18px, 2.2vw, 26px)', maxWidth:640, fontFamily:'var(--font-serif)', fontSize:'clamp(15px, 1.3vw, 18px)', lineHeight:1.5, color:'var(--fg-2)' }}>
        Webgility connects Shopify to QuickBooks — orders, products, taxes, and customers
        mapped once at the field level, then synced automatically on every order.
      </p>

      {/* Webgility — QuickBooks mapping */}
      <div style={{ marginTop:'clamp(30px, 4vw, 52px)', border:'1px solid var(--line-2)', borderRadius:12, overflow:'hidden', boxShadow:'0 24px 60px -32px rgba(10,10,10,0.45)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:9, padding:'9px 14px', background:'var(--uc-bone)', borderBottom:'1px solid var(--line-1)' }}>
          {['#E5564B','#E5A93D','#3FB154'].map(c=>(<span key={c} style={{ width:10, height:10, borderRadius:999, background:c }}/>))}
          <span style={{ marginLeft:6, flex:1, maxWidth:320, height:19, borderRadius:999, background:'var(--uc-paper)', border:'1px solid var(--line-1)', display:'flex', alignItems:'center', padding:'0 10px', fontFamily:'var(--font-mono)', fontSize:9, color:'var(--fg-3)' }}>app.webgility.com / mapping</span>
        </div>
        <img src="assets/webgility-mapping.webp" alt="Webgility — mapping products, taxes, and customers from Shopify into QuickBooks" style={{ display:'block', width:'100%', height:'auto' }}/>
      </div>

      <div style={{ marginTop:20, fontFamily:'var(--font-mono)', fontSize:11, color:'var(--fg-3)', letterSpacing:'0.06em' }}>
        ↳ Webgility app · mapped once, applied to every order · no manual re-keying
      </div>
    </BPSection>
  );
}
          ...
          <div style={{ marginTop:'clamp(30px, 4vw, 48px)', border:'1px solid var(--line-2)', borderRadius:10, overflow:'hidden', background:'var(--uc-paper)', boxShadow:'0 24px 60px -34px rgba(10,10,10,0.4)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:cols, alignItems:'center', borderBottom:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px' }}>
                <span style={{ width:26, height:26, borderRadius:6, background:'#121118', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-hero)', fontSize:12, fontWeight:800, color:'#96588A' }}>W</span>
...
function BPRiskAssessment() {
  ...
  const risks = [
    ...
    { t: 'QuickBooks integration complexity', sev: 'Medium',
      d: 'Bidirectional QuickBooks sync is the hardest dependency — mismatches can block orders.',
      m: 'Field-level mapping validated early in Webgility, with a live sync log.' },
...
function BPInvestment() {
  const work = [
    ...
    'Shopify-side QuickBooks integration guidance',
  ...
  const schedule = [
    { n: '01', amount: '$29,000', due: 'Due at signup' },
    { n: '02', amount: '$14,500', due: 'Due at design approval' },
    { n: '03', amount: '$14,500', due: 'Due at project completion' }
  ];
  return (
    <BPSection id="investment" n="15" label="Investment" dark tail="FIXED · NO SURPRISES">
      ...
      <div style={{
        ...
        <div style={{ padding: 'clamp(28px, 3.4vw, 52px)', display: 'flex', flexDirection: 'column', gap: 'clamp(18px, 2vw, 26px)' }}>
          ...
          <div style={{ padding: 'clamp(18px, 2vw, 26px) clamp(20px, 2.2vw, 28px)', background: 'var(--uc-signal)', borderRadius: 12, boxShadow: '0 22px 60px -26px rgba(232,255,82,0.45)' }}>
            ...
            <div style={{ marginTop: 4, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(52px, 6vw, 88px)', letterSpacing: '-0.05em', lineHeight: 0.85, color: 'var(--uc-black)' }}>$58k</div>
          </div>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ marginTop: 6, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(34px, 3.6vw, 52px)', letterSpacing: '-0.045em', lineHeight: 0.9, color: 'var(--uc-paper)' }}>$12k</div>
          <div style={{ marginTop: 8, fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', color: 'var(--uc-stone-500)' }}>90 HRS</div>
        </div>
      </div>
    </BPSection>
  );
...
function BPApproveButton() {
  const APPROVED_KEY = 'trusty-cook_approved_v1';
