...
function brandName() { return (window.__brand && window.__brand.name) || 'Liberty Systems'; }
...
function BPIntro() {
  ...
  const coName = (bpCo && bpCo.name) || 'Liberty Systems';
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 027 · PREPARED SEPTEMBER 2026 · CONFIDENTIAL</span>
        </div>
...
function BPScope() {
  const layers = [
    ...
    { code: '05', t: 'System Integrations',   d: 'Bidirectional, real-time sync native to Shopify Plus.' },
...
function LibLogo({ h = 14 }) { return <img src="assets/liberty-logo.svg" alt="Liberty Systems" style={{ height:h, width:'auto', display:'block', flexShrink:0 }}/>; }
const LIB_SCOPE = { '--uc-signal':'#fa4616', '--uc-brand':'#fa4616', '--uc-signal-soft':'#FFE3D8' };
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-brand)', color:'#fff', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $999 · BUSINESS PRICING</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', background:'#0A0A0A', borderBottom:'1px solid #0A0A0A' }}>
        <span style={{ display:'flex', alignItems:'center', gap:6 }}><LibLogo h={14}/></span>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Scanners','Mobile','Printers','RFID'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'#FFFFFF':'#A3A3A3' }}>{x}</span>))}</div>
        <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:8 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'#A3A3A3' }}>Search…</span>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'#FFFFFF', background:'var(--uc-brand)', padding:'3px 8px', borderRadius:3 }}>Cart 2</span>
        </div>
      </div>
      {children}
    </div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Barcode Scanners', items: ['Zebra','Honeywell','Datalogic','ProGlove'] },
    { h: 'Mobile Computers', items: ['Zebra','Honeywell','RealWear','Unitech'] },
    { h: 'Barcode Printers', items: ['Zebra','Sato','Printronix','Epson'] },
    { h: 'Featured', items: ['Zebra ET56 Android','RFID Cycle Counting','LibertyCare'] }
  ];
  return (
    <GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Zebra DS2208','$139','Corded 2D','#3A3A3A','BULK','4.8','212'],
    ['Honeywell Xenon 1950g','$229','2D imager','#5A5A5A',null,'4.7','158'],
    ['Zebra DS8178','$389','Cordless','#2B2B2B','NEW','4.9','64'],
    ['Datalogic Gryphon','$259','Rugged','#8A8780',null,'4.7','309'],
    ['ProGlove MARK 3','$399','Wearable','#FA4616','PRO','4.9','47'],
    ['Zebra LI4278','$219','Linear','#6E6E6E',null,'4.6','521']
  ];
  const facets = [
    { h: 'Type', items: [['Handheld', true],['Cordless', true],['Wearable', false],['Presentation', false],['Fixed mount', false]] },
    { h: 'Brand', items: [['Zebra', true],['Honeywell', false],['Datalogic', false]] },
    { h: 'Availability', items: [['In stock', true],['Ships today', false],['Free trial', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'var(--uc-brand)', color:'#fff', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $999 · BUSINESS PRICING</div>
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', background:'#0A0A0A', borderBottom:'1px solid #0A0A0A' }}>
        <LibLogo h={14}/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Scanners','Mobile','Printers','RFID'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'#FFFFFF':'#A3A3A3' }}>{x}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:110, height:18, borderRadius:999, border:'1px solid #3A3A3A', background:'#1A1A1A', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'#A3A3A3', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search products…</span>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'#FFFFFF', background:'var(--uc-brand)', padding:'3px 8px', borderRadius:3 }}>Cart 0</span>
        </span>
      </div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / PRODUCTS / BARCODE SCANNERS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Barcode Scanners</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>184 results</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Handheld ✕','Zebra ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'104px 1fr', borderTop:'1px solid var(--line-1)' }}>
        ...
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:11 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$100</span><span>$2,000</span></div>
          </div>
        </div>
...
function GfxCart() {
  const lines=[['Zebra DS2208 · USB kit','20+','$124','$2,480'],['Zebra ZD421 Printer','5+','$459','$2,295'],['4×6 Thermal Labels','50+','$38','$1,900']];
  const grid=[['Zebra TC58','$1,295','#2B2B2B','NEW'],['Honeywell CT47','$1,495','#3A3A3A',null],['Zebra MC9400','$2,390','#5A5A5A','BULK'],['Datalogic Memor 11','$1,150','#8A8780',null],['RealWear Navigator','$2,495','#6E6E6E',null],['Unitech EA630','$899','#9A9A9A',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', background:'#0A0A0A', borderBottom:'1px solid #0A0A0A' }}>
          <LibLogo h={14}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Scanners','Mobile','Printers','RFID'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'#FFFFFF':'#A3A3A3' }}>{x}</span>))}</div>
          <span style={{ marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'#FFFFFF', background:'var(--uc-brand)', padding:'3px 8px', borderRadius:3 }}>Cart 3</span>
        </div>
        {/* category title + breadcrumb */}
        <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
          <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / PRODUCTS / MOBILE COMPUTERS</span>
            <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Mobile Computers</span>
          </div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, color:'var(--fg-3)' }}>48 products · Sort ▾</span>
        </div>
        {/* filter chips */}
        <div style={{ display:'flex', gap:6, padding:'0 14px 11px' }}>{['Zebra','Honeywell','Android','In stock'].map((f,i)=>(<span key={f} style={{ padding:'3px 9px', borderRadius:999, border:'1px solid', borderColor:i===0?'var(--uc-black)':'var(--line-1)', background:i===0?'var(--uc-black)':'transparent', color:i===0?'var(--uc-paper)':'var(--fg-2)', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700 }}>{f}</span>))}</div>
        {/* product grid */}
      ...
      <div style={{
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Add LibertyCare <span style={{ color:'var(--uc-signal)' }}>coverage</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>Repairs · advance replacement · support</div></div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'#FFFFFF', background:'var(--uc-signal)', padding:'4px 8px', borderRadius:3 }}>ADD</span>
        </div>
        {/* totals + checkout */}
        <div style={{ marginTop:'auto', padding:'10px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$6,675</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'#3F8B5D', marginBottom:7 }}><span>Tier savings</span><span>−$540</span></div>
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'var(--uc-black)', color:'var(--uc-paper)', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · NET-30 · $6,135</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', background:'#0A0A0A', borderBottom:'1px solid #0A0A0A' }}>
        <LibLogo h={14}/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Scanners','Mobile','Printers','RFID'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'#FFFFFF':'#A3A3A3' }}>{x}</span>))}</div>
        <span style={{ marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'#FFFFFF', background:'var(--uc-brand)', padding:'3px 8px', borderRadius:3 }}>Cart 2</span>
      </div>
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / PRODUCTS / MOBILE COMPUTERS</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#1F1F1F','#3A3A3A','#5A5A5A','#FA4616'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #FA4616, #3A3A3A 55%, #141414)', position:'relative', overflow:'hidden' }}>
            ...
            <svg viewBox="0 0 100 100" style={{ position:'absolute', bottom:'4%', right:'2%', width:'80%', opacity:0.92 }}><rect x="34" y="14" width="34" height="72" rx="6" fill="none" stroke="rgba(255,255,255,0.55)" strokeWidth="1.7"/><rect x="39" y="22" width="24" height="34" rx="2" fill="rgba(255,255,255,0.12)"/><path d="M42 64 H60 M42 70 H60 M42 76 H54" stroke="rgba(255,255,255,0.4)" strokeWidth="1.4"/></svg>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>ZEBRA · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>ZEBRA TC58<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Mobile Computer</span></div>
          ...
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#1F1F1F','#5A5A5A'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Free trial','✓ LibertyCare','✓ Ships today'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['Zebra TC58','$1,295','#1F1F1F'],['Charge Cradle','$189','#5A5A5A'],['Trigger Handle','$79','#8A8780']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$1,563</span></span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['OS','Android 13'],['Display','6" FHD'],['Scan engine','SE55 1D/2D'],['Wireless','Wi-Fi 6E · 5G']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“Rolled out to every picker in a week. Fastest scanning we’ve had.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Charge Cradle','$189','#5A5A5A'],['Trigger Handle','$79','#3A3A3A'],['Holster','$34','#8A8780'],['Ext. Battery','$95','#1F1F1F']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxPortal() {
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'var(--uc-brand)', color:'#fff', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>PRIVATE ORDERING PORTAL · NET-45 TERMS ACTIVE</div>
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', background:'#0A0A0A', borderBottom:'1px solid #0A0A0A' }}>
        <LibLogo h={14}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'#A3A3A3' }}>NORTHSTAR DISTRIBUTION</span>
          <span style={{ width:18, height:18, borderRadius:999, background:'linear-gradient(135deg,#FA4616,#8A2A0C)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800, color:'#fff' }}>N</span>
        </span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'112px 1fr', gap:1, background:'var(--line-1)' }}>
        ...
        <div style={{ background:'var(--uc-paper)', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
            ...
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'#FFFFFF', background:'var(--uc-signal)', padding:'4px 9px', borderRadius:3 }}>+ New order</span>
          </div>
          {/* stat tiles */}
          ...
          <div style={{ padding:'0 14px 12px' }}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'70px 1fr auto auto', gap:8, alignItems:'center', padding:'6px 0', borderTop:'1px solid var(--line-1)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'#FFFFFF', background:'var(--uc-signal)', padding:'2px 7px', borderRadius:2 }}>Reorder</span>
            </div>))}
          </div>
        </div>
      </div>
...
function GfxHomepage() {
  const nav = ['Products','Services','Industries','News','About'];
  const mega = [
    { h: 'Products', items: ['Barcode Scanners','Mobile Computers','Barcode Printers','Labels & Ribbons','RFID','Tablets'] },
    { h: 'Industry Solutions', items: ['Wireless Warehouse','Distribution & Logistics','Manufacturing','PLEX Barcode'] },
    { h: 'Services', items: ['Needs Analysis','LibertyCare','Free Trial','Repair Services'] }
  ];
  const brands = ['ZEBRA','HONEYWELL','DATALOGIC','PROGLOVE','SOTI','NEWCASTLE'];
  const feat = [['Zebra ET56 Android','$1,899','#2B2B2B','NEW'],['Honeywell CT47','$1,495','#3A3A3A',null],['Zebra ZT411 Printer','$1,995','#6E6E6E','DEAL'],['ProGlove MARK Display','$449','#5A5A5A','PRO']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-brand)', color:'#fff', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $999 · BUSINESS PRICING</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', background:'#0A0A0A', borderBottom:'1px solid #0A0A0A' }}>
        <LibLogo h={14}/>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{nav.map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'#FFFFFF':'#A3A3A3', position:'relative' }}>{x}{i===0 && <span style={{ position:'absolute', left:0, right:0, bottom:-9, height:2, background:'var(--uc-brand)' }}/>}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:120, height:18, borderRadius:999, border:'1px solid #3A3A3A', background:'#1A1A1A', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'#A3A3A3', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search scanners, printers…</span>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'#FFFFFF', background:'var(--uc-brand)', padding:'3px 8px', borderRadius:3 }}>Cart 6</span>
        </span>
      </div>

      {/* hero + mega menu overlay */}
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #141414 0%, #2A2A2A 55%, #FA4616 140%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>RFID CYCLE COUNTING</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Count a stock room<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>in seconds.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'var(--uc-signal)', color:'#FFFFFF', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Watch RFID demo →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--uc-signal)', whiteSpace:'nowrap' }}>DEVICE FINDER</span>
        {['Industry','Use case','Environment'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:'var(--uc-signal)', color:'#FFFFFF', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Find Devices →</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #3A3A3A, #141414)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>FAMILY-OWNED</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>25+ years of customer-focused warehouse solutions — proven, measurable.</span>
          <span style={{ alignSelf:'flex-start', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', borderBottom:'1.5px solid var(--uc-signal)', paddingBottom:1 }}>See what makes us different →</span>
        </div>
      </div>
    </div>
  );
  ...
  const groups = [
    {
      ...
      gfx: <div style={LIB_SCOPE}><GfxHomepage/></div>
    },
    {
      ...
      gfx: <div style={LIB_SCOPE}><GfxCollection/></div>
    },
    {
      ...
      gfx: <div style={LIB_SCOPE}><GfxCart/></div>
    },
    {
      ...
      gfx: <div style={LIB_SCOPE}><GfxPDP/></div>
    },
    {
      ...
      gfx: <div style={LIB_SCOPE}><GfxPortal/></div>
    }
  ];
  return (
...
function GfxPhone({ kind }) {
  // Mobile Liberty Systems storefront. kind: 'home' | 'about' | 'blog'
  return (
    <div style={{
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        {/* status bar */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px', fontFamily:'var(--font-mono)', fontSize:7.5, color:'#A3A3A3', background:'#0A0A0A', flexShrink:0 }}>
          <span>9:41</span>
          <span style={{ display:'inline-flex', gap:3 }}><span style={{ width:14, height:6, border:'1px solid #A3A3A3', borderRadius:2 }}/></span>
        </div>
        {/* optimized header: wordmark + actions, then big open search */}
        <div style={{ flexShrink:0, borderBottom:'1px solid #0A0A0A', background:'#0A0A0A' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px' }}>
            <span style={{ display:'inline-flex', alignItems:'center', gap:8 }}>
              <span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:22, height:22, border:'1px solid #3A3A3A', borderRadius:5, fontSize:9, color:'#FFFFFF' }}>☰</span>
              <LibLogo h={17}/>
            </span>
            <span style={{ display:'inline-flex', gap:6, alignItems:'center' }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, height:22, padding:'0 9px', background:'var(--uc-brand)', borderRadius:5, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'#FFFFFF' }}>⛒ Cart<span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', minWidth:11, height:11, padding:'0 2px', background:'#FFFFFF', color:'var(--uc-black)', borderRadius:999, fontSize:6, fontWeight:800 }}>6</span></span>
            </span>
          </div>
          {/* big open search */}
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid #FFFFFF', background:'#FFFFFF', borderRadius:5, padding:'0 4px 0 10px' }}>
              <span style={{ fontSize:10, color:'var(--fg-2)' }}>⌕</span>
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search scanners, printers…</span>
              <span style={{ alignSelf:'stretch', display:'flex', alignItems:'center', padding:'0 9px', margin:'3px 0', background:'var(--uc-signal)', color:'#FFFFFF', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>GO</span>
            </div>
          </div>
        </div>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-brand)', color:'#fff', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $999 · BUSINESS PRICING</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #141414 0%, #2A2A2A 55%, #FA4616 140%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>RFID CYCLE COUNTING</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Count a stock room<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>in seconds.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'var(--uc-signal)', color:'#FFFFFF', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Watch RFID demo →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['ZEBRA','HONEYWELL','DATALOGIC','SOTI'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Zebra ET56','$1,899','#2B2B2B','NEW'],['Zebra DS2208','$139','#3A3A3A','BULK'],['Zebra ZD421','$489','#6E6E6E',null],['ProGlove MARK','$399','#5A5A5A','DEAL']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Scanners','#3A3A3A'],['Mobile','#2B2B2B'],['Printers','#6E6E6E'],['RFID','#FA4616']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--uc-signal)' }}/>25+ YEARS · FAMILY-OWNED</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Customer-focused<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>warehouse solutions.</span></span>
            </div>
            {/* overlapping image cards */}
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              <div style={{ position:'absolute', left:0, top:8, width:'58%', aspectRatio:'4/3', borderRadius:5, background:'linear-gradient(150deg, #3A3A3A, var(--uc-bone))', transform:'rotate(-4deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.45)' }}/>
              <div style={{ position:'absolute', right:6, top:0, width:'52%', aspectRatio:'3/4', borderRadius:5, background:'linear-gradient(150deg, #FA4616, var(--uc-bone))', transform:'rotate(6deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.4)' }}/>
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>STILLWATER, MN</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“Today, it’s all about doing more with less. Proven solutions, measurable results.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['25+','Years'],['40+','Brands'],['24/7','Support']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>News & Insights</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #FA4616, #2B2B2B)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:'var(--uc-signal)', color:'#FFFFFF', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>RFID · 6 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Count a stock room in seconds with RFID.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['Chip shortages: is your warehouse ready?','5 min','#3A3A3A'],['5 reasons the Zebra ET401 is built for warehouse work','6 min','#6E6E6E'],['Zebra Picklist Mode: fix multi-barcode errors','4 min','#FA4616']].map((a,i)=>(
...
function BPContent() {
  const screens = [
    { kind: 'home',  label: 'Landing Pages', d: 'Short hero, brand wall, live product tabs, and a department grid — a merchandising surface that gets buyers to the full catalog fast.' },
    { kind: 'about', label: 'Brand Story', d: 'Supplier story and proof — partners and credentials that win operations teams’ trust and rank for branded search.' },
      ...
      <div style={{
        ...
        {screens.map((s, i) => (
          <div key={i} style={{ display:'flex', flexDirection:'column', gap:18 }}>
            <div style={{ ...LIB_SCOPE, maxWidth:260, margin:'0 auto', width:'100%' }}><GfxPhone kind={s.kind}/></div>
  ...
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            { tier:'04', name:'Growth & Intelligence', note:'After launch', tools:[['Shopify Flow','launch'],['JudgeMe','launch'],['Shopify Knowledge Base','launch']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) auto', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span/>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(18px,1.8vw,24px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right', whiteSpace:'nowrap' }}>~$2,455/mo</span>
        </div>
      </div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'libertysystems_approved_v1';
  ...
  const BRAND_NAME              = 'Liberty Systems';
