...
// ── Central brand. brandName() = the DOCUMENT/client brand (cover, copy).
//    The store wireframe mockups use a fixed STORE brand below. ──
function brandName() { return (window.__brand && window.__brand.name) || 'STOCKYARD'; }
const STORE_BRAND = 'Mitutoyo';
function brandHandle() { return STORE_BRAND.toLowerCase().replace(/[^a-z0-9]+/g, ''); }
function BrandMark({ fontSize = 11, color = 'var(--fg-1)', family = 'var(--font-display)', weight = 700, letterSpacing = '-0.01em', dotColor = 'var(--uc-brand)', height }) {
  ...
  if (b.storeLogoSrc) return <img src={b.storeLogoSrc} alt={STORE_BRAND} style={{ height: height || Math.round(fontSize * 1.35), width: 'auto', display: 'block', objectFit: 'contain' }}/>;
  return <span style={{ fontFamily: family, fontWeight: weight, fontSize, letterSpacing, color }}>{STORE_BRAND}<span style={{ color: dotColor }}>.</span></span>;
}

// ── Section shell ─────────────────────────────────────────────────────────
function BPSection({ id, n, label, dark, paper, children, tail, vec = 'bgVector1' }) {
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              A new storefront built for growth on Shopify — same caliber
          ...
          <div style={{
            ...
            {[
              ...
              { k: 'Client lead',  v: 'Daniel Rogers · Mitutoyo',  s: 'Daniel.Rogers@mitutoyo.com' },
              { k: 'Valid through', v: 'Jul 4, 2026',    s: '30 days' }
            ].map((c, i) => (
              <div key={i} style={{
    ...
    <BPSection id="summary" n="01" label="Executive Summary" paper tail="TL;DR">
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <p style={{ margin: 0, fontFamily: 'var(--font-serif)', fontSize: 'clamp(16px, 1.4vw, 20px)', lineHeight: 1.55, color: 'var(--fg-2)' }}>
            {brandName()} is ready to sell online. This blueprint lays out how we build a
            state-of-the-art storefront on Shopify — a unified B2C and B2B experience with
            the tools to make precision easy to find, compare, and buy.
          </p>
          <p style={{ margin: 0, fontFamily: 'var(--font-serif)', fontSize: 'clamp(16px, 1.4vw, 20px)', lineHeight: 1.55, color: 'var(--fg-2)' }}>
            The result: one best-in-class experience, built to scale online revenue —
            delivered in a fixed scope, on a fixed timeline.
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        {[
          ...
          { v: 'Agentic', l: 'AI product discoverability built in' },
          { v: 'Low Cost', l: 'Build with native tech stack' }
        ].map((s, i) => (
          <div key={i} style={{
            padding: 'clamp(24px, 3vw, 36px)',
            ...
            <div style={{ fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(26px, 2.6vw, 40px)', letterSpacing: '-0.04em', lineHeight: 0.95, color: 'var(--fg-1)', whiteSpace: 'nowrap' }}>{s.v}</div>
...
function BPWhere() {
  const items = [
    { n: '01', t: 'No storefront to transact', tag: 'Storefront', impact: 'Critical', pct: 94, d: 'No online channel to sell B2B or B2C — every order runs through people.' },
    { n: '02', t: 'No product search visibility', tag: 'Search', impact: 'High', pct: 82, d: 'Buyers can\u2019t find the right instrument fast across thousands of SKUs.' },
    { n: '03', t: 'No product AI discovery', tag: 'Discovery', impact: 'High', pct: 78, d: 'Invisible to AI answer engines and agentic shopping assistants.' },
    { n: '04', t: 'No storytelling experience', tag: 'Brand', impact: 'Medium', pct: 64, d: 'No content to explain precision, build trust, or rank for branded search.' }
  ];
  return (
    <BPSection id="where" n="02" label="Today" tail="STARTING FROM ZERO">
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', gap: 'clamp(28px, 5vw, 80px)', alignItems: 'end' }}>
        <BPHeadline size="clamp(40px, 6vw, 96px)">
          Today,{' '}
          <BPSerif>no online commerce.</BPSerif>
        </BPHeadline>
        <p style={{ margin: 0, maxWidth: 380, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)', textWrap: 'pretty' }}>
          No storefront, no search, no discovery, no story. Four gaps between Mitutoyo and modern commerce &mdash; every one built from scratch on Shopify.
        </p>
      </div>
...
function BPScope() {
  const layers = [
    ...
    { code: '04', t: 'Data Import',    d: 'Products, customers, pricing, and content via Matrixify.' },
    { code: '05', t: 'Launch & Support',  d: 'QA, a go-live runbook, and 30-day post-launch care.', kind: 'base' }
  ];
  return (
    <BPSection id="scope" n="05" label="Scope of Work" tail="THE SYSTEM">
      <div style={{ maxWidth: 720 }}>
        ...
        <p style={{
          ...
          Five workstreams, stacked into one Shopify system — from the storefront
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $250 · ISO-17025 CALIBRATED</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Calipers','Micrometers','Indicators','Gages'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Calipers', items: ['ABSOLUTE Digimatic','Dial','Vernier','Carbon fiber'] },
    { h: 'Micrometers', items: ['Outside','Inside','Depth','QuantuMike'] },
    { h: 'Indicators', items: ['Dial indicators','Test indicators','Digimatic','Stands'] },
    { h: 'Featured', items: ['Full Catalog','New · QuantuMike','Calibration'] }
  ];
  return (
    <GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['ABSOLUTE Digimatic Caliper','$172.00','500-196-30','#6E7378','TOP','4.9','312'],
    ['Digimatic Micrometer','$208.00','293-340-30','#5A5F63',null,'4.8','158'],
    ['Dial Indicator 2046S','$96.00','2046S','#7A7F83','NEW','4.9','64'],
    ['Digimatic Height Gage','$642.00','192-130','#62676B',null,'4.8','109'],
    ['Surftest SJ-210','$3,290','178-560','#4F5458','PRO','4.9','41'],
    ['Gauge Block Set 47pc','$1,180','516-936','#6E7378',null,'4.7','73']
  ];
  const facets = [
    { h: 'Category', items: [['Calipers', true],['Micrometers', true],['Indicators', false],['Height Gages', false],['Vision / CMM', false]] },
    { h: 'Series', items: [['500 · ABSOLUTE', true],['293 · Digimatic', false],['192 · Height Gage', false]] },
    { h: 'Availability', items: [['In stock', true],['Ships today', false],['ISO calibrated', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Calipers','Micrometers','Indicators','Gages'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / SMALL TOOL INSTRUMENTS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Calipers</span>
        </div>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Calipers ✕','ABSOLUTE ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
...
function GfxCart() {
  const lines=[['ABSOLUTE Caliper · 500-196-30','40+','$158','$6,320'],['Digimatic Micrometer · 293-340','25+','$192','$4,800'],['Dial Indicator · 2046S','60+','$88','$5,280']];
  const grid=[['ABSOLUTE Caliper','$172','#6E7378','TOP'],['Digimatic Micrometer','$208','#5A5F63',null],['Dial Indicator 2046S','$96','#7A7F83','NEW'],['Digimatic Height Gage','$642','#62676B',null],['Surftest SJ-210','$3,290','#4F5458',null],['Gauge Block Set','$1,180','#6E7378',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          <span style={{ width:14, height:14, borderRadius:999, background:'var(--uc-black)' }}/>
          <BrandMark fontSize={11}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Calipers','Micrometers','Indicators','Gages'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Upgrade to StorePro <span style={{ color:'var(--uc-signal)' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>$49/mo · free shipping · priority support</div></div>
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / TOOLS / POWER TOOLS</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>500 SERIES · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>ABSOLUTE Caliper<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>500-196-30 · 6" / 150mm</span></div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['ABSOLUTE Caliper','$172','#6E7378'],['Caliper Case','$38','#3d3e3e'],['SPC Cable','$52','#7A7F83']].map((p,i)=>(
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Range','0–6" / 150mm'],['Resolution','0.0005" / 0.01mm'],['Accuracy','±0.001"'],['Output','Digimatic SPC']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['SPC Data Cable','$52','#6E7378'],['Caliper Case','$38','#8A8780'],['Gauge Block Set','$1,180','#3A3632'],['Calibration','$65','#C8A14E']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <BrandMark fontSize={11}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>PRECISION QC LABS</span>
  ...
  const groups = [
    ...
    },
    {
      ...
      points: [
        ...
        { t: 'Quick Reorder', d: 'One-click reorders and saved lists for repeat buyers.' },
        { t: 'Email / Text Commerce', d: 'Buyers reply to a message and it becomes a Shopify order. No portal required.' }
      ],
      gfx: <GfxPortal/>
    }
  ];
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          ...
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search 40,000+ SKUs…</span>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $250 · ISO-17025 CALIBRATED</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #7A3B16 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>NEW · 500 SERIES</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Precision,<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>shipped to spec.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Shop calipers →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['500','293','192','2046','SJ-210'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['ABSOLUTE Caliper','$172','#6E7378','TOP'],['Digimatic Micrometer','$208','#5A5F63',null],['Dial Indicator 2046S','$96','#7A7F83','NEW'],['Surftest SJ-210','$3,290','#62676B','NEW']].map((p,i)=>(
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Calipers','#C2A878'],['Micrometers','#9A938A'],['Indicators','#C8A14E'],['Gages','#7A7F83']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--uc-signal)' }}/>EST. 1974 · FAMILY-OWNED</span>
            ...
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“Measured to the micron since 1934. Same promise: precision you can trust, shipped on time.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['5,000+','Products'],['1934','Established'],['ISO','Calibrated']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              ...
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>MICROMETERS · 7 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Reading a micrometer to 0.0001".</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['How to read a micrometer','5 min','#6E7378'],['Caliper care & calibration','6 min','#7A7F83'],['Choosing a dial indicator','8 min','#62676B']].map((a,i)=>(
...
function BPContent() {
  ...
  return (
    <BPSection id="content" n="06b" label="Content" paper tail="STORY · SEO · GEO · PAID">
      ...
      {(() => {
        const seo = [
          ['Site Crawl', 'Inventory every URL planned for the new store.'],
          ['301 Redirects', 'Map any pre-existing URLs — products, collections, pages, assets.'],
          ['Title Tags', 'Set every title tag from the catalog source data.'],
          ...
          ['Body Content', 'Author clean, complete on-page content for each template.'],
          ...
          ['Alt Text', 'Set descriptive image alt text across the catalog.'],
          ...
          ['Search Console', 'Verify domain, properties, and settings before launch.']
        ];
        return (
...
function BPTechStack() {
  const rows = [
    { name: 'Shopify Advanced',         fn: 'Commerce platform + B2B',          pri: 'start',  cost: '$399/mo' },
    ...
    { name: 'Shopify Inbox',            fn: 'Customer messaging',               pri: 'launch', cost: 'Free' },
    { name: 'Shopify Forms',            fn: 'Lead capture + accounts',          pri: 'launch', cost: 'Free' },
    { name: 'Shopify Bundles',          fn: 'Product bundles + kits',           pri: 'launch', cost: 'Free' },
    { name: 'Judge.me',                 fn: 'Product reviews',                  pri: 'launch', cost: '$15/mo' },
    { name: 'Matrixify',                fn: 'Data import / export',             pri: 'launch', cost: '$20/mo' }
  ];
  const priMeta = {
  ...
  return (
    <BPSection id="techstack" n="07" label="Architecture" dark tail="WHAT IT RUNS ON">
      <BPHeadline dark>
        A stack chosen for{' '}
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:18, flexWrap:'wrap', gap:8 }}>
          ...
          <span style={{ fontFamily:'var(--font-mono)', fontSize:10, color:'var(--uc-stone-500)', letterSpacing:'0.04em' }}>Shopify Advanced at the core · native apps included</span>
        </div>

        {(() => {
          const layers = [
            { tier:'04', name:'Growth & Engagement', note:'After launch', tools:[['Shopify Forms','launch'],['Shopify Inbox','launch']] },
            { tier:'03', name:'Discovery & Merchandising', note:'Customer-facing', tools:[['Search & Discovery','launch'],['Shopify Bundles','launch']] },
            { tier:'02', name:'Automation & Data', note:'Behind the scenes', tools:[['Shopify Flow','launch'],['Matrixify','launch']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        {rows.map((r, i) => {
          ...
          return (
            <div key={i} style={{ display:'grid', gridTemplateColumns:'minmax(0,1.1fr) minmax(0,1.6fr) minmax(0,1.1fr) minmax(0,auto)', gap:16, padding:'14px 18px', borderBottom: i < rows.length-1 ? '1px solid #1F1F1F' : 'none', alignItems:'center' }}>
        ...
        {/* Footer total */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.1fr) minmax(0,1.6fr) minmax(0,1.1fr) minmax(0,auto)', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontSize:13, color:'var(--uc-stone-500)' }}>Platform + apps — most native apps free</span>
          <span/>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(18px,1.8vw,24px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right' }}>$434/mo</span>
        </div>
      </div>
...
function BPAgentic() {
  ...
  const chat = [
    { who: 'buyer', text: 'I need a 6" caliper and a 0–1" micrometer for QC.' },
    { who: 'agent', text: 'Here are the right instruments at your trade pricing:' },
    { who: 'cards', items: [
      { t: 'ABSOLUTE Caliper · 500-196-30', price: '$172', c: '#6E7378', qty: '×1', icon: <g stroke="#fff" strokeWidth="2.4" fill="none" strokeLinecap="round"><path d="M6 18 L42 18 L42 24 L6 24 Z"/><path d="M14 24 L14 32"/><path d="M30 12 L30 18"/><path d="M30 12 L40 12"/></g> },
      { t: 'Digimatic Micrometer · 293-340', price: '$208', c: '#5A5F63', qty: '×1', icon: <g stroke="#fff" strokeWidth="2.4" fill="none" strokeLinecap="round"><path d="M8 24 q8 -10 18 0 q-8 10 -18 0 Z"/><path d="M26 24 L40 24"/><path d="M40 18 L40 30"/></g> }
    ] },
    { who: 'buyer', text: 'Add both, ship to the QC lab, NET-30.' }
  ];
  return (
...
function BPMigration() {
  const steps = [
    { t: 'Source & map', s: 'Catalogue every record from CSV exports — products, customers, content.' },
    { t: 'Transform', s: 'Clean, dedupe, and map data to the Shopify model. Nothing imported blind.' },
    { t: 'Stage & verify', s: 'Dry-run into staging. Reconcile counts, spot-check edge cases.' },
    { t: 'Go live', s: 'Final import, publish, and DNS switch — a clean, fully-loaded launch.' }
  ];
  const stats = [
    { v: '8,342', l: 'Products' },
    { v: '5,500+', l: 'SKUs' },
    { v: '342', l: 'Collections' },
    { v: '1,860', l: 'Spec assets' }
  ];
  return (
    <BPSection id="migration" n="10" label="Data" paper tail="LOAD WITHOUT LOSS" vec="bgVector2">
      <BPHeadline>
        Launch{' '}
        <BPSerif>fully loaded.</BPSerif>
      </BPHeadline>
      <p style={{ marginTop: 18, maxWidth: 600, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)' }}>
        A brand-new store, imported clean — every product, spec, and asset from CSV
        files via Matrixify, mapped to the Shopify model: categories become tags, custom
        attributes become metafields. Nothing imported blind, nothing lost.
      </p>

      {/* source-to-Shopify field crosswalk */}
      {(() => {
        const source = [['Title','text'],['Part No.','text'],['Price','money'],['Brand','text'],['Categories','list'],['spec_sheet','custom'],['cad_model','custom'],['Image set','media']];
        const target = [['Title','text','1:1'],['SKU / Variant','text','1:1'],['Price','money','1:1'],['Vendor','text','from Brand'],['Tags','list','merged'],['spec_sheet','metafield','metafield'],['cad_model','metafield','metafield'],['Media','media','mapped']];
          ...
          <div style={{ marginTop:'clamp(30px, 4vw, 48px)', border:'1px solid var(--line-2)', borderRadius:10, overflow:'hidden', background:'var(--uc-paper)', boxShadow:'0 24px 60px -34px rgba(10,10,10,0.4)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:cols, alignItems:'center', borderBottom:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px' }}>
                <span style={{ width:26, height:26, borderRadius:6, background:'#121118', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-hero)', fontSize:13, fontWeight:800, color:'#8FA0FF' }}>{'{ }'}</span>
                <div style={{ display:'flex', flexDirection:'column', gap:1 }}>
                  ...
                  <span style={{ fontFamily:'var(--font-mono)', fontSize:9, color:'var(--fg-3)' }}>CSV import · 14,200 records</span>
                </div>
              </div>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:10, padding:'12px 16px' }}>
                <div style={{ display:'flex', flexDirection:'column', gap:1, alignItems:'flex-end' }}>
                  <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:14, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Shopify Advanced</span>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:16, padding:'9px 16px', borderBottom:'1px solid var(--line-1)', flexWrap:'wrap' }}>
              {['Products','Customers','Pricing','Content'].map((t,i)=>(<span key={t} style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:11.5, letterSpacing:'-0.01em', color:i===0?'var(--fg-1)':'var(--fg-3)', borderBottom:i===0?'2px solid var(--uc-black)':'2px solid transparent', paddingBottom:3 }}>{t}</span>))}
            ...
            <div style={{ display:'flex', alignItems:'center', gap:18, padding:'11px 16px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)', flexWrap:'wrap', fontFamily:'var(--font-mono)', fontSize:10, color:'var(--fg-3)' }}>
              ...
              <span style={{ display:'inline-flex', alignItems:'center', gap:7 }}><span style={{ width:18, height:2, background:'repeating-linear-gradient(90deg, var(--uc-brand) 0 5px, transparent 5px 9px)' }}/>Transformed — tags · metafields · media</span>
...
function BPDelivery() {
  const milestones = [
    ...
    { wk: 'WK 11', t: 'Data import + QA', s: 'Catalog loaded, full QA pass.' },
...
function BPTeam() {
  const team = [
    { n: 'Denis Dyli', r: 'Principal', img: 'assets/team-denis.webp' },
  ...
  return (
    <BPSection id="team" n="14" label="Team" tail="WHO DOES THE WORK">
      <BPHeadline>
        Senior from{' '}
  ...
  const packages = [
    {
      ...
      work: [
        ...
        'Experience design on a proven framework',
        'B2C and light B2B selling from day one',
        'Front and back-end Shopify development',
        'Essential integrations: payments, shipping, core tools',
        'Data import: products, customers, pricing, content',
        'SEO and GEO, so you launch findable',
        'Go live, a clean handoff, and a 30-day warranty'
      ],
      outcome: 'A brand new store, live fast and selling both ways, with zero surprise invoices.',
    ...
    },
    {
      ...
      priceLabel: 'Fixed price', price: '$98k'
    },
    {
      n: '03', name: 'Transform', tagline: 'The whole operation, rebuilt as one revenue engine.',
      ...
      work: [
        ...
        'Custom integrated systems and automations, built for you',
        'Smart Agents and AI workflows tuned to your operation',
        'A phased plan that holds, priority support, and a path into Uncap Growth'
      ],
      outcome: 'A full operation rebuilt around how you run, crafted exclusively, built to grow.',
      priceLabel: 'Project based', price: 'Custom'
    }
  ];
  return (
    <BPSection id="investment" n="12" label="Investment" tail="FIXED · NO SURPRISES">
      <BPHeadline>
        Priced up front.{' '}
      ...
      <p style={{ marginTop: 'clamp(18px, 2.2vw, 26px)', maxWidth: 620, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)' }}>
        Three ways to start, depending on how much of the operation we&rsquo;re solving. Every one is fixed-scope, fixed-price, and yours to keep.
      </p>
      <div style={{ marginTop: 'clamp(30px, 4vw, 52px)', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, alignItems: 'stretch' }}>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ marginTop: 6, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(34px, 3.6vw, 52px)', letterSpacing: '-0.045em', lineHeight: 0.9, color: 'var(--fg-1)' }}>$9.5k</div>
        </div>
      </div>
...
function BPGrowth() {
  ...
  return (
    <BPSection id="growth" n="13" label="Growth" paper tail="AFTER LAUNCH">
      <BPHeadline>
        Launch is the start.{' '}
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 15, color: 'var(--fg-1)' }}>
          Uncap Essentials · <span style={{ fontFamily: 'var(--font-hero)', fontWeight: 800 }}>$2.5k / mo</span>
        </div>
...
function BPWhy() {
  const reviews = [
    ...
    { name: 'Google', score: '4.8' },
    { name: 'Shopify', score: '5.0' },
    { name: 'Trustpilot', score: '4.9' }
  ];
  const clients = ['VOSGES', 'KOOKS', 'AGRI DRAIN', 'PAWSTRUCK'];
  return (
    <BPSection id="why" n="15" label="Uncap" dark tail="THE CASE">
      ...
      <div style={{ marginTop: 'clamp(32px, 4vw, 48px)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '24px 44px', alignItems: 'center' }}>
          {['canon','ULEgroup','totalboat','thermosoft','signwarehouse','sanitaire','leki','e3sparkplugs','eea','blueroot','farmers','garrison','kooks','vermontwoods'].map((n) => (
            <img key={n} src={'assets/' + n + '-white.svg'} alt={n} style={{ height: 48, width: 'auto', display: 'block', opacity: 0.92 }}/>
          ))}
        </div>
      </div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'mitutoyo_approved_v3';
