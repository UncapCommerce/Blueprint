    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 009 · PREPARED JUNE 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              { k: 'Client lead',  v: 'Steven Montaño',  s: 'General Manager' },
...
function BPWhere() {
  const items = [
    ...
    { n: '03', t: 'Outdated experience', tag: 'UX', impact: 'High', pct: 76, d: 'Unfriendly shopping experience and no mobile optimization.' },
    { n: '04', t: 'Zero agentic optimisation', tag: 'AI', impact: 'Critical', pct: 90, d: 'No in-site technical components and integrations with AI discovery platforms.' }
  ];
  return (
...
function BPScope() {
  const layers = [
    ...
    { code: '05', t: 'Integrations',   d: 'System integrations native to Shopify Plus.' },
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING ON ORDERS OVER $150 · WHOLESALE PRICING</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Alternators','Starters','By Vehicle','Brands'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Alternators', items: ['12V Reman','140A+ Units','OEM Replacement','New Units'] },
    { h: 'Starters', items: ['Reman Starters','High Torque','Mini Starters','New Units'] },
    { h: 'By Vehicle', items: ['Ford','Chevy / GM','Toyota','Dodge / RAM'] },
    { h: 'Featured', items: ['Wholesale Pricing','New Arrivals','Core Returns'] }
  ];
  return (
    <GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Reman Alt 120A','$89.00','/unit','#6B7A8D','REMAN','4.8','312'],
    ['Reman Alt 140A','$109.00','/unit','#8A9BAB',null,'4.7','198'],
    ['Reman Starter 1.4kW','$74.00','/unit','#5A6470','NEW','4.9','87'],
    ['Alt 94A Budget','$64.00','/unit','#7A8590',null,'4.6','443'],
    ['HD Starter 2.2kW','$129.00','/unit','#4A5560','PRO','4.9','56'],
    ['OEM Alt 150A','$139.00','/unit','#9AA5AE',null,'4.5','221']
  ];
  const facets = [
    { h: 'Type', items: [['Alternators', true],['Starters', true],['New Units', false],['Cores', false]] },
    { h: 'Make', items: [['Ford', true],['Chevy / GM', false],['Toyota', false]] },
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Alternators','Starters','By Vehicle','Brands'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / ALTERNATORS & STARTERS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Alternators & Starters</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>847 units</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Alternators ✕','Starters ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
...
function GfxCart() {
  const lines=[['Reman Alt 120A · Ford F-150','2','$89.00','$178'],['Reman Starter 1.4kW · Chevy','1','$74.00','$74'],['OEM Alt 140A · Toyota','3','$109.00','$327']];
  const grid=[['Reman Alt 120A','$89.00','#6B7A8D','REMAN'],['Reman Alt 140A','$109.00','#8A9BAB',null],['Reman Starter','$74.00','#5A6470','NEW'],['Alt 94A Budget','$64.00','#7A8590',null],['HD Starter','$129.00','#4A5560',null],['OEM Alt 150A','$139.00','#9AA5AE',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          <span style={{ width:14, height:14, borderRadius:999, background:'var(--uc-black)' }}/>
          <BrandMark fontSize={11}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Alternators','Starters','By Vehicle','Brands'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Upgrade to FleetPro <span style={{ color:'var(--uc-signal)' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>$49/mo · free shipping · priority support</div></div>
        ...
        <div style={{ marginTop:'auto', padding:'10px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$579</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'#3F8B5D', marginBottom:7 }}><span>Tier savings</span><span>−$58</span></div>
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'var(--uc-black)', color:'var(--uc-paper)', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · $521</span>
        </div>
      </div>
    </div>
  );
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / ALTERNATORS / FORD</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          ...
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 55% 25%, #5A6880, #2A3545 60%, #141820)', position:'relative', overflow:'hidden' }}>
            ...
            <svg viewBox="0 0 100 100" style={{ position:'absolute', bottom:'8%', right:'8%', width:'65%', opacity:0.7 }}><circle cx="50" cy="50" r="36" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="2"/><circle cx="50" cy="50" r="24" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1.5"/><circle cx="50" cy="50" r="10" fill="rgba(255,255,255,0.12)" stroke="rgba(255,255,255,0.55)" strokeWidth="2"/><line x1="50" y1="14" x2="50" y2="26" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5"/><line x1="50" y1="74" x2="50" y2="86" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5"/><line x1="14" y1="50" x2="26" y2="50" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5"/><line x1="74" y1="50" x2="86" y2="50" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5"/></svg>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>TUCSON REMAN · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>REMAN 12V 150A<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Alternator — Ford F-150</span></div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ 2-yr warranty','✓ Core return','✓ Ships today'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['Voltage Reg 12V','$28','#6B7A8D'],['Alt Belt K060400','$18','#4A5560'],['Pulley Kit','$14','#8A9BAB']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$60</span></span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Voltage','12V'],['Output','150A'],['Weight','7.2 lb'],['Condition','Reman']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Voltage Reg 12V','$28','#6B7A8D'],['Alt Belt','$18','#4A5560'],['Pulley Kit','$14','#8A9BAB'],['Reman Starter','$74','#5A6470']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxHomepage() {
  ...
  const mega = [
    { h: 'By Department', items: ['Lumber','Concrete & Rebar','Electrical','Plumbing','Fasteners','Tools'] },
    { h: 'By Project', items: ['Framing','Foundations','Finishing','Site Work'] },
    { h: 'Quick Links', items: ['New Arrivals','Clearance','Pro Pricing','Bulk Orders'] }
  ];
  const brands = ['FORGE','ATLAS','TITAN','KILN','VOLT','BUILDRITE'];
  const feat = [['Reman Alt 120A','$89.00','#6B7A8D','REMAN'],['Reman Alt 140A','$109.00','#8A9BAB',null],['Reman Starter','$74.00','#5A6470','NEW'],['LED High-Bay','$89.00','#C8A14E','PRO']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING ON ORDERS OVER $150 · WHOLESALE PRICING</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:120, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search 40,000+ SKUs…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #7A3B16 135%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>PRO SEASON · ENDS FRIDAY</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Jobsite-ready<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>stock, on sale.</span></span>
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--uc-signal)', whiteSpace:'nowrap' }}>FIND YOUR FIT</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #6E5235, #2A2018)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EST. 1974</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>One yard became fourteen branches — same promise, on the truck, on time.</span>
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING ON ORDERS OVER $150 · WHOLESALE PRICING</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #7A3B16 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>PRO SEASON · ENDS FRIDAY</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Jobsite-ready<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>stock, on sale.</span></span>
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Reman Alt 120A','$89.00','#6B7A8D','REMAN'],['Reman Alt 140A','$109.00','#8A9BAB',null],['Reman Starter','$74.00','#5A6470','NEW'],['LED High-Bay','$89.00','#C8A14E','DEAL']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
...
function BPTechStack() {
  const rows = [
    ...
    { name: 'Shopify Search & Discovery',        fn: 'Native search + filtering',          pri: 'start',  cost: 'Included' },
    { name: 'Shopify Checkout Blocks',           fn: 'Customisable checkout experience',    pri: 'start',  cost: 'Included' },
    { name: 'Shopify Flow',                      fn: 'Automation + workflow engine',        pri: 'start',  cost: 'Included' },
    { name: 'Shopify Bundles',                   fn: 'Product bundling + kits',             pri: 'start',  cost: 'Included' },
    { name: 'Shopify Marketplace Connect',       fn: 'Multi-channel marketplace sync',      pri: 'launch', cost: 'Included' },
    { name: 'Shopify Knowledge Base',            fn: 'Customer self-serve support',         pri: 'launch', cost: 'Included' },
    { name: 'Uncap Quotes',                      fn: 'Custom quoting + negotiation',        pri: 'start',  cost: '$100/mo' },
    { name: 'Uncap Garage',                      fn: 'YMM fitment + vehicle lookup',        pri: 'start',  cost: '$100/mo' },
    { name: 'Matrixify',                         fn: 'Bulk data import + migration',        pri: 'launch', cost: '$20/mo' },
  ];
  const priMeta = {
  ...
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            { tier:'04', name:'Growth & Intelligence', note:'After launch', tools:[['Uncap Garage','start'],['Shopify Marketplace Connect','launch']] },
            { tier:'03', name:'Apps & Tools', note:'Extend Shopify', tools:[['Uncap Quotes','start'],['Matrixify','launch'],['Shopify Knowledge Base','launch']] },
            { tier:'02', name:'Shopify Native', note:'Built-in apps', tools:[['Shopify Search & Discovery','start'],['Shopify Checkout Blocks','start'],['Shopify Flow','start'],['Shopify Bundles','start']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        {/* Footer total */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.1fr) minmax(0,1.6fr) minmax(0,0.5fr) minmax(0,120px)', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span/>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(16px,1.6vw,22px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right', whiteSpace:'nowrap' }}>~$2,520/mo</span>
        </div>
      </div>
...
function BPMigration() {
  ...
  const stats = [
    'Products', 'Collections', 'Pages', 'Orders', 'Customers'
  ];
  return (
    <BPSection id="migration" n="11" label="Migration" paper tail="MOVE WITHOUT LOSS" vec="bgVector2">
      ...
      {/* guarantee strip */}
      <div style={{ marginTop: 'clamp(28px, 3vw, 40px)', display: 'flex', flexWrap: 'wrap', gap: 0, borderTop: '1px solid var(--line-1)', borderBottom: '1px solid var(--line-1)' }}>
        {stats.map((s, i) => (
          <div key={i} style={{ padding: '20px 24px 20px 0', paddingLeft: i > 0 ? 24 : 0, borderLeft: i > 0 ? '1px solid var(--line-1)' : 'none', position: 'relative' }}>
            <span aria-hidden="true" style={{ position: 'absolute', top: 0, left: i > 0 ? 24 : 0, width: 16, height: 2, background: 'var(--uc-signal)' }}/>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'clamp(12px, 1.2vw, 15px)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--fg-1)' }}>{s}</div>
          </div>
        ))}
      </div>
    </BPSection>
    ...
    <BPSection id="growth" n="15" label="Growth" paper tail="AFTER LAUNCH">
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-3)', letterSpacing: '0.06em' }}>
          12 MONTHS · MANAGED SERVICE RETAINER
        </div>
      </div>
    </BPSection>
  );
...
function BPApproveButton() {
  const APPROVED_KEY = 'tucsonalternator_approved_v1';
  ...
  const BRAND_NAME              = 'Tucson Alternator';
