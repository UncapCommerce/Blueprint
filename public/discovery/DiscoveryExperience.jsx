...
(function () {
  ...
  function Experience({ role, company, address, profile, palette, hasLogo, initial, token }) {
    ...
    const noteFocus = (qid) => {
      focusedQidRef.current = qid;
      setFocusedQid(qid);
      // Putting the cursor in a field is the capture trigger: if the engine
      // isn't running (mic was denied at load, or the reconnect backoff gave
      // up), focusing revives it. 'nokey' stays put — that needs the secret.
      if (isAdmin && !liveRef.current && live !== 'starting' && live !== 'nokey') {
        retryRef.current = 0;
        startAuto();
      }
    };
                        ...
                        <div key={q.id} style={{ marginTop: 16 }}>
                          ...
                          {q.type === 'text' && (
                            <input value={v || ''} onClick={(e) => e.stopPropagation()} onFocus={() => noteFocus(q.id)} onChange={(e) => setAnswer(q.id, e.target.value)} placeholder={q.placeholder || ''}
                              style={{ width: '100%', padding: '10px 12px', border: '1px solid ' + (live === 'on' && focusedQid === q.id ? '#2F7A47' : '#C9C7C0'), boxShadow: live === 'on' && focusedQid === q.id ? '0 0 0 2px rgba(47,122,71,0.35)' : 'none', borderRadius: 5, fontSize: 14, background: '#FDFCF9', outline: 'none', color: '#0A0A0A' }}/>
                          )}
                          {q.type === 'textarea' && (
                            <textarea value={v || ''} onClick={(e) => e.stopPropagation()} onFocus={() => noteFocus(q.id)} onChange={(e) => setAnswer(q.id, e.target.value)} placeholder={q.placeholder || ''} rows={3}
                              style={{ width: '100%', padding: '10px 12px', border: '1px solid ' + (live === 'on' && focusedQid === q.id ? '#2F7A47' : '#C9C7C0'), boxShadow: live === 'on' && focusedQid === q.id ? '0 0 0 2px rgba(47,122,71,0.35)' : 'none', borderRadius: 5, fontSize: 14, background: '#FDFCF9', outline: 'none', resize: 'vertical', lineHeight: 1.5, color: '#0A0A0A' }}/>
                          )}
