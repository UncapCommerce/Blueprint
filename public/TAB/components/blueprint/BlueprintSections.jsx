...
function brandName() { return (window.__brand && window.__brand.name) || 'TAB'; }
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 021 · PREPARED SEPTEMBER 2026 · CONFIDENTIAL</span>
        </div>
    ...
    <BPSection id="summary" n="01" label="Executive Summary" paper tail="TL;DR">
      ...
      <div style={{
        ...
        {[
          ...
          { v: 'Unified', l: 'B2C, B2B, POS on Shopify integrated with Infor SyteLine' },
...
function BPApproach() {
  const steps = [
    ...
    { n: 'Phase 2', t: 'Build', d: 'Storefront, B2B, Infor SyteLine sync. Fixed scope, senior team, no surprises.', wk: 'Weeks 20+' },
    { n: 'Phase 3', t: 'Grow', d: 'Migrate, go live, then optimize conversion, AOV, and automation.', wk: 'Months 7–12' }
  ];
  return (
...
function BPScope() {
  const layers = [
    ...
    { code: '05', t: 'ERP Integration',   d: 'Bidirectional, real-time Infor SyteLine sync native to Shopify.' },
...
function BPGantt() {
  const groups = [
    { name: 'Foundation', range: 'WK 01–07', tasks: [
      ...
      { n: '03', t: 'Experience Design',  tag: 'UX/UI Design · Web · Mobile',   s: 3, e: 6,  range: 'W3–6' },
      { n: '04', t: 'Approval',                   tag: 'Tech Stack Review & Design Sign-off', s: 6, e: 7,  range: 'W6–7' }
    ] },
    { name: 'Production', range: 'WK 07–17', tasks: [
      { n: '05', t: 'Development',                tag: 'Build',    s: 7,  e: 15, range: 'W7–15' },
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $99 · MADE IN THE USA</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <img src="assets/tab-logo.svg" alt="TAB" style={{ height:15, width:'auto', display:'block' }}/>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['File Folders','Labels','Pockets','Indexes'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'File Folders', items: ['Manila','Color / Stripe','Classification','Pressboard'] },
    { h: 'Labels', items: ['Alphabetic','Numeric','Year / Month','Solid Color'] },
    { h: 'Pockets', items: ['Expansion','Manila','X-Ray Jackets','Art Portfolios'] },
    { h: 'Featured', items: ['2026 Catalog','TABQUIK Software','Custom Solutions'] }
  ];
  return (
    <GfxChrome>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:1, background:'var(--line-1)' }}>
        {cols.map((c,ci)=>(
          <div key={ci} style={{ background:'var(--uc-paper)', padding:'12px 13px 16px', display:'flex', flexDirection:'column', gap:7 }}>
            ...
            {c.items.map((it,ri)=>(<span key={ri} style={{ fontFamily:'var(--font-display)', fontWeight:ci===3&&ri===0?700:500, fontSize:10.5, letterSpacing:'-0.01em', color: ci===0&&ri===0?'var(--uc-black)':'var(--fg-2)', display:'flex', alignItems:'center', gap:5 }}>{ci===0&&ri===0 && <span style={{ width:14, height:3, background:'#00A1E1' }}/>}{it}</span>))}
          </div>
        ))}
      </div>
    </GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Alpha Labels A–Z','$42.10','/set','#F2A33C','BULK','4.8','212'],
    ['Numeric Labels 0–9','$36.40','/roll','#5FB4D9',null,'4.6','158'],
    ['Year Labels 2026','$18.90','/roll','#7BB661','NEW','4.9','64'],
    ['Solid Color Labels','$24.00','500 ct','#7C6FB0',null,'4.7','309'],
    ['Label Protectors','$27.00','500/set','#E9B44C','PRO','4.9','47'],
    ['TABSTIK Name Labels','$16.20','/pack','#E8C87E',null,'4.5','521']
  ];
  const facets = [
    { h: 'Category', items: [['Folders', true],['Labels', true],['Pockets', false],['Indexes', false],['Accessories', false]] },
    { h: 'Brand', items: [['TAB', true],['AMES', false],['BIBBERO', false]] },
    { h: 'Availability', items: [['In stock', true],['Ships today', false],['Custom order', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <img src="assets/tab-logo.svg" alt="TAB" style={{ height:13, width:'auto', display:'block' }}/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['File Folders','Labels','Pockets','Indexes'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / COLOR-CODED LABELS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Color-Coded Labels</span>
        </div>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Labels ✕','Alphabetic ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'#DC001B', fontWeight:700 }}>Clear all</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'104px 1fr', borderTop:'1px solid var(--line-1)' }}>
        ...
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:11 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$3</span><span>$90</span></div>
          </div>
        </div>
          ...
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              <div style={{ aspectRatio:'1/1', background:`linear-gradient(140deg, ${p[3]}, var(--uc-bone))`, borderRadius:2, position:'relative' }}>
                {p[4] && <span style={{ position:'absolute', top:4, left:4, padding:'1px 5px', background:p[4]==='NEW'?'#DC001B':(p[4]==='PRO'?'var(--uc-black)':'#00A1E1'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800 }}>{p[4]}</span>}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:3 }}><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'#DC001B' }}>★★★★★</span><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'var(--fg-3)' }}>{p[5]} ({p[6]})</span></div>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#00A1E1' }}>Volume price ✓</span>
            </div>
          ))}
        </div>
      </div>
...
function GfxCart() {
  const lines=[['End Tab Folders · Letter','200+','$0.68','$136'],['Alpha Label Set A–Z','10+','$38.90','$389'],['Expansion Pockets · Legal','50+','$2.10','$105']];
  const grid=[['End Tab Folders','$0.84','#E8C87E','BULK'],['Alpha Labels A–Z','$42.10','#F2A33C',null],['Year Labels 2026','$18.90','#7BB661','NEW'],['Manila Pockets','$38.60','#7C6FB0',null],['Label Protectors','$27.00','#E9B44C',null],['Out Guides','$31.20','#5FB4D9',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          <img src="assets/tab-logo.svg" alt="TAB" style={{ height:13, width:'auto', display:'block' }}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['File Folders','Labels','Pockets','Indexes'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{
        ...
        <div style={{ padding:'9px 13px', borderBottom:'1px solid var(--line-1)', background:'var(--uc-cream)' }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-1)', marginBottom:5 }}>🚚 You&rsquo;ve unlocked <span style={{ color:'#00A1E1', fontWeight:700 }}>FREE shipping</span></div>
          <div style={{ height:4, background:'var(--line-1)', borderRadius:999, overflow:'hidden' }}><span style={{ display:'block', width:'100%', height:'100%', background:'#00A1E1' }}/></div>
        </div>
        {/* lines */}
        {lines.map((l,i)=>(
          <div key={i} style={{ padding:'8px 13px', borderBottom:'1px solid var(--line-1)', display:'grid', gridTemplateColumns:'26px 1fr auto', gap:9, alignItems:'center' }}>
            ...
            <div><div style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:10, color:'var(--fg-1)' }}>{l[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>qty {l[1]} · <span style={{ color:'#DC001B', fontWeight:700 }}>{l[2]}</span></div></div>
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          <span style={{ width:18, height:18, borderRadius:999, border:'2px solid #00A1E1', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}><span style={{ width:7, height:7, borderRadius:999, background:'#00A1E1' }}/></span>
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>First order with TAB <span style={{ color:'#00A1E1' }}>−20%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>Join the email list · applied at checkout</div></div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'#fff', background:'#DC001B', padding:'4px 8px', borderRadius:3 }}>ADD</span>
        </div>
        {/* totals + checkout */}
        <div style={{ marginTop:'auto', padding:'10px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$630</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'#00A1E1', marginBottom:7 }}><span>Tier savings</span><span>−$78</span></div>
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'#DC001B', color:'#fff', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · NET-30 · $552</span>
        </div>
      </div>
    </div>
  );
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / FILE FOLDERS / CLASSIFICATION</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#00A1E1','#002B49','#7C6FB0','#E9B44C'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #00A1E1, #002B49 60%, #001C30)', position:'relative', overflow:'hidden' }}>
            <span style={{ position:'absolute', top:8, left:8, padding:'2px 7px', background:'#00A1E1', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>BEST SELLER</span>
            <svg viewBox="0 0 100 100" style={{ position:'absolute', bottom:'4%', right:'2%', width:'80%', opacity:0.92 }}><path d="M18 30 L42 30 L48 38 L84 38 L84 74 L18 74 Z" fill="none" stroke="rgba(255,255,255,0.55)" strokeWidth="1.7"/><path d="M18 50 L84 50" stroke="rgba(255,255,255,0.4)" strokeWidth="1.4"/><rect x="60" y="30" width="16" height="8" rx="1" fill="rgba(255,255,255,0.18)"/></svg>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#00A1E1' }}>TAB · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>Classification<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Folders · End Tab</span></div>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'#DC001B' }}>★★★★★</span><span style={{ color:'var(--fg-3)' }}>4.9 · 128 reviews</span></div>
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#00A1E1','#1F1F1F','#E9B44C','#7C6FB0'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6, marginTop:2 }}>
            <div style={{ display:'flex', gap:6 }}>
              ...
              <span style={{ flex:1, padding:'8px', textAlign:'center', background:'#DC001B', color:'#fff', borderRadius:3, fontFamily:'var(--font-display)', fontSize:9.5, fontWeight:800 }}>Add to cart</span>
            </div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Made in USA','✓ Free returns','✓ Ships today'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['Classification Folder','$3.90','#00A1E1'],['Alpha Label Set','$42.10','#F2A33C'],['Out Guides','$31.20','#7BB661']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$77</span></span>
              <span style={{ flex:1, padding:'6px', textAlign:'center', background:'#DC001B', color:'#fff', borderRadius:3, fontFamily:'var(--font-display)', fontSize:8.5, fontWeight:700 }}>Add all 3</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Stock','25 pt pressboard'],['Fasteners','2″ embedded'],['Dividers','2 per folder'],['Box qty','10 / box']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
        <div style={{ background:'var(--uc-cream)', padding:'11px 14px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['Color-coded end tabs cut misfiles and retrieval time.','Volume pricing and NET terms unlock at 5+ boxes.','Made in America at the Mayville, WI facility.'].map((t,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'12px 1fr', gap:7, alignItems:'start' }}><span style={{ width:10, height:10, borderRadius:999, background:'#00A1E1', marginTop:1, display:'flex', alignItems:'center', justifyContent:'center' }}><svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="#fff" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/></svg></span><span style={{ fontFamily:'var(--font-serif)', fontSize:9, lineHeight:1.35, color:'var(--fg-2)' }}>{t}</span></div>))}</div>
        </div>
      </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'#DC001B' }}>★★★★★</div>
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“Our records room runs on these. We reorder for every clinic we manage.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Alpha Labels A–Z','$42.10','#F2A33C'],['Out Guides','$31.20','#7BB661'],['Label Protectors','$27.00','#7C6FB0'],['Unit File Boxes','$54.00','#E9B44C']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>

      {/* compare products */}
      <div style={{ padding:'11px 14px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
        <div style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:8 }}>COMPARE</div>
        {(() => {
          const cols = [['Classification · End Tab','$3.90','#00A1E1',true],['Classification · Top Tab','$3.70','#F2A33C',false],['Expansion · End Tab','$4.60','#7BB661',false]];
          const feats = [['Stock','25 pt','25 pt','25 pt'],['Dividers','2','2','—'],['Fasteners','2″','2″','2″'],['Expansion','—','—','1½″'],['Color-code ready','✓','✓','✓']];
          return (
            <div style={{ border:'1px solid var(--line-1)', borderRadius:3, overflow:'hidden' }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', background:'var(--uc-bone)' }}>
                <div style={{ padding:6 }}/>
                {cols.map((c,i)=>(
                  <div key={i} style={{ padding:6, borderLeft:'1px solid var(--line-1)', display:'flex', flexDirection:'column', gap:4, background:c[3]?'var(--uc-paper)':'transparent' }}>
                    <div style={{ aspectRatio:'1/0.6', borderRadius:2, background:`linear-gradient(140deg, ${c[2]}, var(--uc-bone))` }}/>
                    <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:6.5, letterSpacing:'-0.01em', color:'var(--fg-1)', lineHeight:1.2 }}>{c[0]}{c[3] && <span style={{ marginLeft:3, padding:'1px 4px', background:'#00A1E1', color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5, fontWeight:800 }}>THIS</span>}</span>
                    <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)' }}>{c[1]}</span>
                  </div>
                ))}
              </div>
              {feats.map((r,ri)=>(
                <div key={ri} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', borderTop:'1px solid var(--line-1)' }}>
                  <span style={{ padding:'4px 6px', fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{r[0]}</span>
                  {[1,2,3].map(ci=>(<span key={ci} style={{ padding:'4px 6px', borderLeft:'1px solid var(--line-1)', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-1)', textAlign:'center', background:ci===1?'var(--uc-cream)':'transparent' }}>{r[ci]}</span>))}
                </div>
              ))}
            </div>
          );
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <img src="assets/tab-logo.svg" alt="TAB" style={{ height:13, width:'auto', display:'block' }}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>LAKESIDE HEALTH RECORDS</span>
          <span style={{ width:18, height:18, borderRadius:999, background:'linear-gradient(135deg,#00A1E1,#00699C)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800, color:'#fff' }}>L</span>
        </span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'112px 1fr', gap:1, background:'var(--line-1)' }}>
        ...
        <div style={{ background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:3 }}>
          {nav.map((n,i)=>(<div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'5px 7px', borderRadius:3, background:n[2]?'var(--uc-black)':'transparent' }}><span style={{ fontSize:9, color:n[2]?'#00A1E1':'var(--fg-3)' }}>{n[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:n[2]?'var(--uc-paper)':'var(--fg-2)' }}>{n[1]}</span></div>))}
        ...
        <div style={{ background:'var(--uc-paper)', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
            ...
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'#fff', background:'#DC001B', padding:'4px 9px', borderRadius:3 }}>+ New order</span>
          </div>
          {/* stat tiles */}
          ...
          <div style={{ padding:'0 14px 12px' }}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'70px 1fr auto auto', gap:8, alignItems:'center', padding:'6px 0', borderTop:'1px solid var(--line-1)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'#fff', background:'#DC001B', padding:'2px 7px', borderRadius:2 }}>Reorder</span>
            </div>))}
          </div>
        </div>
      </div>
...
function GfxHomepage() {
  const nav = ['File Folders','Labels','Pockets','Indexes','Accessories'];
  const mega = [
    { h: 'File Folders', items: ['Manila','Color / Stripe','Classification','Pressboard'] },
    { h: 'Labels', items: ['Alphabetic','Numeric','Year / Month','Solid Color'] },
    { h: 'Pockets', items: ['Expansion','Manila','X-Ray Jackets','Art Portfolios'] }
  ];
  const brands = ['TAB','AMES','BIBBERO','INHEALTH','GBS','JETER'];
  const feat = [['Alpha Label Sheets','$42.10','#F2A33C','BULK'],['Manila Pockets · Legal','$38.60','#E8C87E',null],['Year Labels 2026','$18.90','#7BB661','NEW'],['ICD-10 Code Book','$89.00','#5FB4D9','PRO']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $99 · MADE IN THE USA</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:78, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 8px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #001623 0%, #003A5C 55%, #002B49 135%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'#00A1E1' }}>NEW · 2026 PRODUCT CATALOG</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Store it. Find it.<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>Use it.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'#DC001B', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Shop file folders →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'#00A1E1', whiteSpace:'nowrap' }}>MATCH YOUR SYSTEM</span>
        {['System','Size','Tab position'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:'#DC001B', color:'#fff', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Find supplies →</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #00699C, #00263F)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EST. 1950</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Made in Mayville, Wisconsin — 250,000 specialty folders a day, trusted by 90% of the Fortune 500.</span>
          <span style={{ alignSelf:'flex-start', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', borderBottom:'1.5px solid #00A1E1', paddingBottom:1 }}>Read our story →</span>
        </div>
      </div>
    </div>
  );
  ...
  const groups = [
    {
      ...
      points: [
        ...
        { t: 'Product Finding', d: 'System / Size / Tab-position matching, quizzes, and creative ways to find products.' }
      ],
      gfx: <GfxHomepage/>
    },
    {
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px' }}>
            <span style={{ display:'inline-flex', alignItems:'center', gap:8 }}>
              ...
              <img src="assets/tab-logo.svg" alt="TAB" style={{ height:16, width:'auto', display:'block' }}/>
            </span>
            <span style={{ display:'inline-flex', gap:6, alignItems:'center' }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, height:22, padding:'0 9px', background:'var(--uc-black)', borderRadius:5, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)' }}>⛒ Cart<span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', minWidth:11, height:11, padding:'0 2px', background:'#DC001B', color:'#fff', borderRadius:999, fontSize:6, fontWeight:800 }}>6</span></span>
            </span>
          </div>
          {/* big open search */}
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search the catalog…</span>
              <span style={{ alignSelf:'stretch', display:'flex', alignItems:'center', padding:'0 9px', margin:'3px 0', background:'#DC001B', color:'#fff', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>GO</span>
            </div>
          </div>
        </div>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $99 · MADE IN THE USA</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #001623 0%, #003A5C 55%, #002B49 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'#00A1E1' }}>NEW · 2026 PRODUCT CATALOG</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Store it. Find it.<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>Use it.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'#DC001B', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Shop file folders →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['TAB','AMES','BIBBERO','GBS','JETER'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Alpha Label Sheets','$42.10','#F2A33C','BULK'],['Manila Pockets','$38.60','#E8C87E',null],['Year Labels 2026','$18.90','#7BB661','NEW'],['Label Protectors','$27.00','#E9B44C','DEAL']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
                  <div style={{ aspectRatio:'1/0.86', borderRadius:5, background:`linear-gradient(150deg, ${p[2]}, var(--uc-bone))`, position:'relative' }}>{p[3] && <span style={{ position:'absolute', top:5, left:5, padding:'1px 6px', background:p[3]==='DEAL'?'#DC001B':(p[3]==='NEW'?'var(--uc-black)':'#00A1E1'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5, fontWeight:800 }}>{p[3]}</span>}<span style={{ position:'absolute', bottom:5, right:5, width:16, height:16, borderRadius:999, background:'var(--uc-paper)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'var(--fg-1)' }}>＋</span></div>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Folders','#E8C87E'],['Labels','#F2A33C'],['Pockets','#7BB661'],['Indexes','#5FB4D9']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'#00A1E1' }}/>EST. 1950 · MADE IN WISCONSIN</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Records made easy<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>for 75 years.</span></span>
            </div>
            {/* overlapping image cards */}
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              <div style={{ position:'absolute', left:0, top:8, width:'58%', aspectRatio:'4/3', borderRadius:5, background:'linear-gradient(150deg, #F2A33C, var(--uc-bone))', transform:'rotate(-4deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.45)' }}/>
              <div style={{ position:'absolute', right:6, top:0, width:'52%', aspectRatio:'3/4', borderRadius:5, background:'linear-gradient(150deg, #002B49, var(--uc-bone))', transform:'rotate(6deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.4)' }}/>
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>MAYVILLE, WI · HQ</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“We make the records and information essential to your business easy to store, find, and use.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['250k','Folders/day'],['90%','Fortune 500'],['10M+','Labels printed']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #F2A33C, #00699C)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:'#DC001B', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>FILING · 7 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Color-coding 101: fewer misfiles.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['End tab vs. top tab folders','5 min','#E8C87E'],['Choosing label protectors','4 min','#7C6FB0'],['Folder stock weights, explained','6 min','#E9B44C']].map((a,i)=>(
...
function BPContent() {
  const screens = [
    ...
    { kind: 'about', label: 'Brand Story', d: 'Brand story and proof — American manufacturing heritage and credentials that win buyer trust and rank for branded search.' },
...
function BPTechStack() {
  const rows = [
    ...
    { name: 'Shopify Subscriptions',      fn: 'Subscriptions & recurring orders',    pri: 'future', cost: 'Free' },
    ...
    { name: 'Uncap Connect',               fn: 'Infor SyteLine ↔ Shopify integration',           pri: 'start',  cost: '$390/mo' },
    { name: 'Klaviyo Email',               fn: 'Email marketing & automation',        pri: 'launch', cost: '–' },
    { name: 'Klaviyo Reviews',             fn: 'Reviews & social proof',              pri: 'launch', cost: '–' },
  ...
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            { tier:'04', name:'Growth & Intelligence', note:'After launch', tools:[['Shopify Flow','launch'],['Shopify Subscriptions','future'],['Shopify Knowledge Base','launch'],['Klaviyo Email','launch'],['Klaviyo Reviews','launch']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) 120px', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontSize:13, color:'var(--uc-stone-500)' }}>Platform + apps, before Infor SyteLine integration</span>
          <span/>
...
function BPIntegrations() {
  ...
  const activity = [
    ['Order #SO-48217 pushed to Infor SyteLine', '2 min ago'],
    ['Inventory delta · 1,284 items reconciled', '9 min ago'],
    ['Customer ACME Contracting created in SyteLine', '14 min ago']
  ];
  ...
  return (
    <BPSection id="integrations" n="11" label="Integrated" tail="HOW IT CONNECTS">
      ...
      <p style={{ marginTop:'clamp(18px, 2.2vw, 26px)', maxWidth:640, fontFamily:'var(--font-serif)', fontSize:'clamp(15px, 1.3vw, 18px)', lineHeight:1.5, color:'var(--fg-2)' }}>
        Uncap Connect runs as a Shopify-embedded app — a real-time, bidirectional Infor SyteLine
        ...
        <div style={{ display:'flex', background:'#F1F1F1' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:14, flexWrap:'wrap' }}>
            <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
              ...
              <span style={{ fontFamily:pFont, fontSize:12.5, color:'#616161' }}>Infor SyteLine ERP integration</span>
            </div>
            <div style={{ display:'flex', gap:8 }}>
          ...
          <Card>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
              <div style={{ display:'flex', alignItems:'center', gap:11 }}>
                <span style={{ width:34, height:34, borderRadius:8, background:'#101A2B', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:pFont, fontSize:13, fontWeight:800, color:'#9FB4FF' }}>SL</span>
                <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
                  <span style={{ fontFamily:pFont, fontSize:13.5, fontWeight:700, color:'#1A1A1A' }}>Infor SyteLine</span>
          ...
          <Card pad={false}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.3fr) clamp(48px,6vw,72px) minmax(0,1.3fr) minmax(0,90px)', gap:'clamp(8px,1.2vw,16px)', padding:'8px 16px', background:'#FAFAFA', borderBottom:'1px solid #E3E3E3' }}>
              {['Shopify','','Infor SyteLine','Status'].map((h,i)=>(<span key={i} style={{ fontFamily:pFont, fontSize:10.5, fontWeight:600, color:'#616161', textAlign:i===3?'right':'left' }}>{h}</span>))}
            </div>
            {maps.map((m,i)=>(
          ...
          <div style={{ marginTop:'clamp(30px, 4vw, 48px)', border:'1px solid var(--line-2)', borderRadius:10, overflow:'hidden', background:'var(--uc-paper)', boxShadow:'0 24px 60px -34px rgba(10,10,10,0.4)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:cols, alignItems:'center', borderBottom:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px' }}>
                <span style={{ width:26, height:26, borderRadius:6, background:'#121118', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-hero)', fontSize:12, fontWeight:800, color:'#8FA0FF' }}>W</span>
...
function BPDelivery() {
  const milestones = [
    ...
    { wk: 'WK 12', t: 'Integrations live', s: 'SyteLine sync + B2B flows validated.' },
    { wk: 'WK 15', t: 'Migration + QA', s: 'Data moved, redirects, full QA pass.' },
...
function BPInvestment() {
  const work = [
    ...
    'Shopify-side Infor SyteLine integration guidance',
  ...
  const schedule = [
    { n: '01', amount: '$60,000', due: 'Due at signup' },
    { n: '02', amount: '$30,000', due: 'Due at design approval' },
    { n: '03', amount: '$30,000', due: 'Due at project completion' }
  ];
  return (
...
function BPApproveButton() {
  const APPROVED_KEY = 'tab_approved_v1';
