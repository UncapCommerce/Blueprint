    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 010 · PREPARED JUNE 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              A plan to unify {brandName()}&rsquo;s commerce operation on Shopify, enable native B2B and upgrade to a modern user experience for growth.
            </div>
          </div>
    ...
    <BPSection id="summary" n="01" label="Executive Summary" paper tail="TL;DR">
      ...
      <div style={{
        ...
        {[
          ...
          { v: 'Unified', l: 'B2C, B2B and back-office on Shopify' },
          { v: 'OS 2.0', l: 'Upgraded Shopify Theme and Experience' },
          { v: 'Speed', l: 'Sidekick based self management' }
        ].map((s, i) => (
          <div key={i} style={{
            padding: 'clamp(24px, 3vw, 36px)',
...
function BPWhere() {
  const items = [
    { n: '01', t: 'Outdated storefront', tag: 'Storefront', impact: 'High', pct: 82, d: 'Experience, design, and performance have not kept pace — conversion and customer expectations are both suffering.' },
    { n: '02', t: 'Old theme', tag: 'Theme', impact: 'High', pct: 78, d: 'Legacy theme version is limiting to build on, difficult to extend, and costly to support.' },
    { n: '03', t: 'No B2B enablement', tag: 'B2B', impact: 'Critical', pct: 94, d: 'Native Shopify B2B is not enabled or utilized — wholesale is running outside the platform.' },
    { n: '04', t: 'Limited ERP integration', tag: 'ERP', impact: 'High', pct: 76, d: 'ERP integration has not been upgraded to support B2B primitives — orders, pricing, and inventory are not in sync.' }
  ];
  return (
...
function BPObjectives() {
  const items = [
    { n: '01', t: 'Seamless upgrade', d: 'Upgrade to a modern Shopify experience without affecting day-to-day operations.', metric: 'Zero',   unit: 'disruption',       tag: 'Upgrade' },
    ...
    { n: '03', t: 'Growth-ready foundation', d: 'Conversion, AOV, retention — engineered in, not bolted on.',                  metric: 'Higher', unit: 'AOV + LTV',        tag: 'Growth' },
...
function BPScope() {
  const layers = [
    ...
    { code: '03', t: 'B2B Enablement',    d: 'Catalogs, pricing tiers, NET terms, and account portals.' },
    { code: '04', t: 'ERP Integration Assistance',   d: 'Guidance and support to upgrade ERP integration for B2B and full sync.' },
...
function BPGantt() {
  const groups = [
    ...
    { name: 'Production', range: 'WK 04–10', tasks: [
      ...
      { n: '07', t: 'QA & Testing',               tag: 'Verify',   s: 10, e: 11, range: 'W10–11' }
    ] },
    { name: 'Launch', range: 'WK 11–12', tasks: [
      { n: '08', t: 'Client UAT & Launch',        tag: 'Go-live',  s: 11, e: 12, range: 'W11–12' }
    ] }
  ];
  return (
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE US SHIPPING OVER $100 · AUTHENTIC WESTERN WEAR SINCE 1878</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['New','Men','Women','Kids','Cowboygirl'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: "Men's", items: ['Cattleman Classics','Short Sleeve Shirts','Flannels & Plaids','Retros','Outerwear'] },
    { h: "Women's", items: ['Cowboygirl',"Shirts & Tops",'Outerwear',"All Women's"] },
    { h: 'Kids', items: ["Boy's Shirts","Girl's Shirts",'Family Matching','All Kids'] },
    { h: 'Featured', items: ['New Arrivals','Ely Cattleman PRO','Clearance','Since 1878'] }
  ];
  return (
    <GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Cattleman Classic Plaid','$44.99','L/S','#4A6B8A','BEST','4.9','312'],
    ['Long Sleeve Snap Shirt','$38.99','L/S','#7A2D2D',null,'4.7','186'],
    ['Textured Western Shirt','$42.99','L/S','#8B6B4A','NEW','4.8','64'],
    ['Flannel Snap Shirt','$36.99','L/S','#4A5C4A',null,'4.6','209'],
    ['Retro Western Shirt','$39.99','L/S','#C2A878','SALE','4.8','97'],
    ['Outerwear Vest','$69.99','L/S','#3A3632',null,'4.7','52']
  ];
  const facets = [
    { h: 'Category', items: [['Classics', true],['Retros', true],['Flannels', false],['Outerwear', false],['PRO', false]] },
    { h: 'Size', items: [['S', false],['M', true],['L', true],['XL', false],['2XL', false]] },
    { h: 'Availability', items: [['In stock', true],['New Arrivals', false],['Sale', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['New','Men','Women','Kids','Cowboygirl'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / MEN / WESTERN SHIRTS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Men's Western Shirts</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>148 styles</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Classics ✕','Retros ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
        ...
        <div style={{ padding:10, display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:8 }}>
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#3F8B5D' }}>Free US shipping ✓</span>
            </div>
          ))}
        </div>
      </div>
...
function GfxCart() {
  const lines=[['Cattleman Classic Plaid · L','1','$44.99','$44.99'],['Long Sleeve Snap Shirt · XL','2','$38.99','$77.98'],['Flannel Western Shirt · M','1','$36.99','$36.99']];
  const grid=[['Cattleman Classic Plaid','$44.99','#4A6B8A','BEST'],['Long Sleeve Snap Shirt','$38.99','#7A2D2D',null],['Textured Western Shirt','$42.99','#8B6B4A','NEW'],['Flannel Snap Shirt','$36.99','#4A5C4A',null],['Retro Western Shirt','$39.99','#C2A878','SALE'],['Outerwear Vest','$69.99','#3A3632',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
          <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / MEN / CATTLEMAN CLASSICS</span>
            <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Cattleman Classics</span>
          </div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, color:'var(--fg-3)' }}>64 styles · Sort ▾</span>
        </div>
        {/* filter chips */}
        <div style={{ display:'flex', gap:6, padding:'0 14px 11px' }}>{['Classics','Plaid','In stock','Sale'].map((f,i)=>(<span key={f} style={{ padding:'3px 9px', borderRadius:999, border:'1px solid', borderColor:i===0?'var(--uc-black)':'var(--line-1)', background:i===0?'var(--uc-black)':'transparent', color:i===0?'var(--uc-paper)':'var(--fg-2)', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700 }}>{f}</span>))}</div>
        {/* product grid */}
      ...
      <div style={{
        ...
        <div style={{ padding:'9px 13px', borderBottom:'1px solid var(--line-1)', background:'var(--uc-cream)' }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-1)', marginBottom:5 }}>🚚 You&rsquo;ve unlocked <span style={{ color:'#3F8B5D', fontWeight:700 }}>FREE US shipping</span></div>
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Earn Cattleman Rewards <span style={{ color:'var(--uc-signal)' }}>+160 pts</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>Free account · redeem on next order</div></div>
        ...
        <div style={{ marginTop:'auto', padding:'10px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal (3 items)</span><span>$159.96</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'#3F8B5D', marginBottom:7 }}><span>Free US shipping</span><span>—</span></div>
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'var(--uc-black)', color:'var(--uc-paper)', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · $159.96</span>
        </div>
      </div>
    </div>
  );
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / MEN / CATTLEMAN CLASSICS</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#4A6B8A','#7A2D2D','#C2A878','#4A5C4A'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #4A6B8A, #2A3F5A 60%, #1A2940)', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>ELY CATTLEMAN · IN STOCK · FREE SHIPPING</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>Cattleman Classic<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Pearl Snap Shirt</span></div>
          ...
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#4A6B8A','#7A2D2D','#C2A878','#4A5C4A'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6, marginTop:2 }}>
            ...
            <span style={{ padding:'7px', textAlign:'center', border:'1px solid var(--uc-black)', borderRadius:3, fontFamily:'var(--font-display)', fontSize:9, fontWeight:700, color:'var(--fg-1)' }}>Add to Quote</span>
          </div>
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Free US shipping','✓ Free returns','✓ Since 1878'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['Cattleman Classic','$44.99','#4A6B8A'],['Western Belt','$28.99','#8B6B4A'],['Felt Western Hat','$54.99','#2A2018']].map((p,i)=>(
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Fit','Regular Western'],['Closure','Pearl snaps'],['Material','100% cotton'],['Care','Machine wash cold']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        <div style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:8 }}>COMPLETE THE LOOK</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Retro Western Shirt','$39.99','#C2A878'],['Flannel Snap Shirt','$36.99','#4A5C4A'],['Outerwear Vest','$69.99','#3A3632'],['Cattleman Tee','$24.99','#8B6B4A']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxHomepage() {
  const nav = ['New','Men','Women','Kids','Cowboygirl','Since 1878'];
  const mega = [
    { h: "Men's Shirts", items: ['Cattleman Classics','Short Sleeve Shirts','Flannels & Plaids','Retros','Outerwear','Ely PRO'] },
    { h: "Women's", items: ['Cowboygirl',"Women's Shirts","Women's Outerwear","All Women's"] },
    { h: 'More', items: ['Kids','Big & Tall','Clearance','Our Brands'] }
  ];
  const brands = ['ELY','CLASSICS','RETROS','PRO','FLANNEL','CBG'];
  const feat = [['Cattleman Classic Plaid','$44.99','#4A6B8A','BEST'],['First Light Ride Shirt','$42.99','#C2A878','NEW'],['Flannel Snap Shirt','$36.99','#4A5C4A',null],['Outerwear Vest','$69.99','#3A3632','PRO']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE US SHIPPING OVER $100 · AUTHENTIC WESTERN WEAR SINCE 1878</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:120, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search shirts & styles…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #7A3B16 135%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>AMERICAN SPIRIT · NEW SEASON</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Ride the trail<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>in western style.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Shop Men’s →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--uc-signal)', whiteSpace:'nowrap' }}>BUILD YOUR LOOK</span>
        {['Category','Style','Size'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
      ...
      <div style={{ padding:'12px 14px 6px', display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
        <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:14, letterSpacing:'-0.03em', color:'var(--fg-1)' }}>New &amp; Featured</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #6E5235, #2A2018)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EST. 1878</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Authentic western wear, crafted for the modern trailblazer since 1878.</span>
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          ...
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search shirts & styles…</span>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>FREE US SHIPPING OVER $100 · AUTHENTIC WESTERN WEAR SINCE 1878</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #7A3B16 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>AMERICAN SPIRIT · NEW SEASON</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Ride the trail<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>in western style.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Shop Men’s →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['ELY','CLASSICS','RETROS','PRO','CBG'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Cattleman Classic Plaid','$44.99','#4A6B8A','BEST'],['First Light Ride Shirt','$42.99','#C2A878',null],['Flannel Snap Shirt','$36.99','#4A5C4A','NEW'],['Outerwear Vest','$69.99','#3A3632','SALE']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Men','#4A6B8A'],['Women','#7A4A5A'],['Kids','#6B8A4A'],['Cowboygirl','#8B6B4A']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--uc-signal)' }}/>EST. 1878 · FAMILY-OWNED</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Authentic western<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>wear since 1878.</span></span>
            </div>
            ...
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              ...
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>RANCH, USA · 1878</span>
            </div>
            {/* manifesto */}
            ...
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['1878','Est.'],['Western','Wear'],['Family','Owned']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>Style Guides</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              ...
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>WESTERN STYLE · 5 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>How to style your Cattleman shirt.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['Caring for pearl snap shirts','4 min','#4A6B8A'],['Western shirt fit guide','6 min','#7A2D2D'],['How to layer western wear','5 min','#C2A878']].map((a,i)=>(
...
function BPTechStack() {
  const rows = [
    { name: 'Shopify',                      fn: 'Commerce platform + B2B',          pri: 'start',  cost: 'Included' },
    { name: 'Shopify Search & Discovery',     fn: 'Native search + merchandising',    pri: 'start',  cost: 'Free' },
    { name: 'Shopify Checkout Blocks',        fn: 'Checkout customization',           pri: 'start',  cost: 'Free' },
    { name: 'Shopify Flow',                   fn: 'Automation & workflows',           pri: 'start',  cost: 'Free' },
    { name: 'Uncap Quotes',                   fn: 'B2B quoting & approvals',          pri: 'start',  cost: 'TBD' },
    { name: 'Netsuite Connector',             fn: 'ERP ↔ Shopify sync',              pri: 'start',  cost: 'TBD' },
    { name: 'Klaviyo Reviews / Stamped',      fn: 'Product reviews',                  pri: 'launch', cost: 'TBD' },
    { name: 'Klaviyo Email',                  fn: 'Email marketing & lifecycle',      pri: 'launch', cost: 'TBD' },
    { name: 'Redo / Returnly',                fn: 'Order tracking & returns',         pri: 'launch', cost: 'TBD' },
    { name: 'TaxCloud / Avalara',             fn: 'Tax compliance',                   pri: 'launch', cost: 'TBD' },
    { name: 'Rewind',                         fn: 'Store backup & recovery',          pri: 'launch', cost: 'TBD' },
    { name: 'Rakuten',                        fn: 'Affiliate marketing',              pri: 'future', cost: 'TBD' }
  ];
  const priMeta = {
  ...
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            { tier:'04', name:'Growth & Engagement', note:'After launch', tools:[['Klaviyo Email','launch'],['Uncap Quotes','start'],['Rakuten','future']] },
            { tier:'03', name:'Experience', note:'Customer-facing', tools:[['Search & Discovery','start'],['Checkout Blocks','start'],['Klaviyo Reviews / Stamped','launch'],['Redo / Returnly','launch']] },
            { tier:'02', name:'Data & Integration', note:'System of record', tools:[['Netsuite Connector','start'],['Flow','start'],['TaxCloud / Avalara','launch'],['Rewind','launch']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        {/* Footer total */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.1fr) minmax(0,1.6fr) minmax(0,1.1fr)', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
...
function BPIntegrations() {
  ...
  const activity = [
    ...
    ['B2B Company Ely Cattleman Wholesale synced to NetSuite', '14 min ago']
  ];
  ...
  return (
    <BPSection id="integrations" n="11" label="Integrated" tail="HOW IT CONNECTS">
      ...
      <p style={{ marginTop:'clamp(18px, 2.2vw, 26px)', maxWidth:640, fontFamily:'var(--font-serif)', fontSize:'clamp(15px, 1.3vw, 18px)', lineHeight:1.5, color:'var(--fg-2)' }}>
        The <strong>NetSuite Connector App</strong> runs as a Shopify-embedded app — a real-time, bidirectional
        integration with field-level mapping and a live sync log, managed right inside admin. Orders, inventory,
        fulfillments, customers, and B2B company data flow automatically between Shopify and NetSuite.
      </p>

      {/* NetSuite Connector App screenshot */}
      <div style={{ marginTop:'clamp(32px, 4vw, 52px)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
          <span style={{ width:28, height:28, borderRadius:6, background:'#101A2B', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:12, fontWeight:800, color:'#9FB4FF', flexShrink:0 }}>NS</span>
          <div>
            <div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:'clamp(16px,1.5vw,20px)', letterSpacing:'-0.015em', color:'var(--fg-1)' }}>NetSuite Connector App</div>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:11, color:'var(--fg-3)', marginTop:2 }}>Oracle NetSuite ↔ Shopify · all sync flows active</div>
          </div>
        </div>
        <div style={{ border:'1px solid var(--line-2)', borderRadius:10, overflow:'hidden', boxShadow:'0 24px 60px -32px rgba(10,10,10,0.35)' }}>
          <img src="uploads/CI2H9pP7yIwDEAE=.webp" alt="NetSuite Connector App — sync flows dashboard" style={{ width:'100%', display:'block' }}/>
        </div>
        <div style={{ marginTop:14, fontFamily:'var(--font-mono)', fontSize:11, color:'var(--fg-3)', letterSpacing:'0.06em' }}>
...
function BPDelivery() {
  const milestones = [
    ...
    { wk: 'WK 10', t: 'QA & Testing', s: 'Full QA pass, UAT, and sign-off.' },
...
function BPRiskAssessment() {
  ...
  const risks = [
    { t: 'ERP integration complexity', sev: 'High',
      d: 'Netsuite Connector may not be sufficient or support all feeds that may be requested.',
      m: 'Field-level mapping validated early with a live sync log; scope feeds before build starts.' },
...
function BPInvestment() {
  ...
  const schedule = [
    { n: '01', amount: '$19,250', due: 'Due at signup' },
    { n: '02', amount: '$9,625', due: 'Due at design approval' },
    { n: '03', amount: '$9,625', due: 'Due at project completion' }
  ];
  return (
      ...
      <div style={{
        ...
        <div style={{ padding: 'clamp(28px, 3.4vw, 52px)', display: 'flex', flexDirection: 'column', gap: 'clamp(18px, 2vw, 26px)' }}>
          ...
          <p style={{ margin: 0, maxWidth: 460, fontFamily: 'var(--font-serif)', fontSize: 'clamp(14.5px, 1.2vw, 17px)', lineHeight: 1.55, color: 'var(--uc-stone-300)', textWrap: 'pretty' }}>
            A modern unified B2C/B2B storefront on Shopify with a tech stack and customer
          ...
          <div style={{ padding: 'clamp(18px, 2vw, 26px) clamp(20px, 2.2vw, 28px)', background: 'var(--uc-signal)', borderRadius: 12, boxShadow: '0 22px 60px -26px rgba(232,255,82,0.45)' }}>
            ...
            <div style={{ marginTop: 4, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(52px, 6vw, 88px)', letterSpacing: '-0.05em', lineHeight: 0.85, color: 'var(--uc-black)' }}>$38.5k</div>
          </div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'elycattleman_approved_v1';
  ...
  const BRAND_NAME              = 'Ely Cattleman';
...
function BPNext() {
  const steps = [
    ...
    { n: '02', t: 'Onboard within 1 week', d: 'Discovery session + access to systems.' },
