...
const QP = '#1844a2';
function brandName() { return (window.__brand && window.__brand.name) || 'QP Feed'; }
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 017 · PREPARED AUGUST 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              A plan to build {brandName()}&rsquo;s commerce operations on Shopify, consolidate
          ...
          <div style={{
            ...
            {[
              ...
              { k: 'Client lead',  v: 'Jacob Stukerjurgen',  s: 'Vice President' },
              { k: 'Company', v: 'Quality Plus Feed', s: '2033 Locust St, St. Paul, IA 52657' },
...
function BPWhere() {
  const items = [
    { n: '01', t: 'No storefront', tag: 'Storefront', impact: 'Critical', pct: 96, d: 'No branded online presence where buyers can browse the catalog and self-serve.' },
    { n: '02', t: 'No shopping cart', tag: 'Commerce', impact: 'Critical', pct: 94, d: 'No way to add, price, and check out an order online — every sale goes through a rep.' },
    { n: '03', t: 'No discoverability', tag: 'Search', impact: 'High', pct: 86, d: 'No search, filtering, or navigation to help buyers find the right product.' },
    { n: '04', t: 'No quoting functionality', tag: 'Trade', impact: 'High', pct: 82, d: 'Quotes handled by email and spreadsheet, with no tracking, approvals, or conversion.' }
  ];
  return (
...
function BPObjectives() {
  const items = [
    { n: '01', t: 'Seamless launch', d: 'Launch on Shopify with everything that works today — built for growth.', metric: 'Zero',   unit: 'disruption',       tag: 'Launch' },
...
function BPApproach() {
  const steps = [
    { n: 'Phase 1', t: 'Blueprint', d: 'Architecture, data model, integration map. A plan that holds up under scrutiny.', wk: '1–2 weeks' },
    { n: 'Phase 2', t: 'Build', d: 'Storefront, B2B, ERP sync. Fixed scope, senior team, no surprises.', wk: 'Weeks 16+' },
    { n: 'Phase 3', t: 'Grow', d: 'Go live, then optimize conversion, AOV, and automation.', wk: 'Months 6–12' }
  ];
  return (
...
function BPScope() {
  const layers = [
    ...
    { code: '05', t: 'ERP Integration',   d: 'Data synchronisation between Shopify and ERP.' },
    { code: '06', t: 'Data Import',       d: 'Products, collections, content, customers, and companies.' },
...
function BPGantt() {
  const groups = [
    ...
    { name: 'Production', range: 'WK 06–14', tasks: [
      ...
      { n: '06', t: 'Data Import',                tag: 'Import',   s: 9,  e: 12, range: 'W9–12' },
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE DELIVERY ON BULK FEED ORDERS · GROWER PRICING</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Cattle','Swine','Poultry','Show Feeds'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Cattle', items: ['Beef & stocker','Dairy','Mineral & tubs','Creep feed'] },
    { h: 'Swine', items: ['Starter','Grower','Sow & gestation','Show swine'] },
    { h: 'Poultry', items: ['Chick starter','Layer','Broiler','Scratch grains'] },
    { h: 'Featured', items: ['Non-GMO program','Show Feeds','Grower account'] }
  ];
  return (
    <GfxChrome>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:1, background:'var(--line-1)' }}>
        {cols.map((c,ci)=>(
          <div key={ci} style={{ background:'var(--uc-paper)', padding:'12px 13px 16px', display:'flex', flexDirection:'column', gap:7 }}>
            ...
            {c.items.map((it,ri)=>(<span key={ri} style={{ fontFamily:'var(--font-display)', fontWeight:ci===3&&ri===0?700:500, fontSize:10.5, letterSpacing:'-0.01em', color: ci===0&&ri===0?'var(--uc-black)':'var(--fg-2)', display:'flex', alignItems:'center', gap:5 }}>{ci===0&&ri===0 && <span style={{ width:14, height:3, background:'#1844a2' }}/>}{it}</span>))}
          </div>
        ))}
      </div>
    </GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['14% Cattle Grower','$14.80','/50 lb','#9C8F7A','BULK','4.8','212'],
    ['Beef Mineral Tub','$132.00','200 lb','#C2A878',null,'4.6','158'],
    ['Creep Feed Pellet','$16.40','50 lb','#D8D2C4','NEW','4.9','64'],
    ['Dairy 18% Textured','$18.20','50 lb','#8A8780',null,'4.7','309'],
    ['Non-GMO Stocker','$21.60','50 lb','#C8A14E','NON-GMO','4.9','47'],
    ['Whole Shell Corn','$11.90','50 lb','#9A938A',null,'4.5','521']
  ];
  const facets = [
    { h: 'Species', items: [['Cattle', true],['Swine', false],['Poultry', false],['Goats', false],['Sheep', false]] },
    { h: 'Brand', items: [['Quality Plus', true],['Crystalyx', false],['Show-Rite', false]] },
    { h: 'Form', items: [['Pellet', true],['Textured', false],['Meal', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Cattle','Swine','Poultry','Show Feeds'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / CATTLE FEED</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Cattle Feed</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>486 results</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Cattle ✕','Pellet ✕','Quality Plus ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'#1844a2', fontWeight:700 }}>Clear all</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'104px 1fr', borderTop:'1px solid var(--line-1)' }}>
        ...
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:11 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$8</span><span>$180</span></div>
          </div>
        </div>
          ...
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              <div style={{ aspectRatio:'1/1', background:`linear-gradient(140deg, ${p[3]}, var(--uc-bone))`, borderRadius:2, position:'relative' }}>
                {p[4] && <span style={{ position:'absolute', top:4, left:4, padding:'1px 5px', background:p[4]==='NEW'?'#1844a2':(p[4]==='PRO'?'var(--uc-black)':'#3F8B5D'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800 }}>{p[4]}</span>}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:3 }}><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'#1844a2' }}>★★★★★</span><span style={{ fontFamily:'var(--font-mono)', fontSize:6, color:'var(--fg-3)' }}>{p[5]} ({p[6]})</span></div>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#3F8B5D' }}>Grower price ✓</span>
            </div>
          ))}
        </div>
      </div>
...
function GfxCart() {
  const lines=[['14% Cattle Grower · 50 lb','40+','$13.60','$544'],['Beef Mineral Tub · 200 lb','12+','$124.00','$1,488'],['Whole Shell Corn · 50 lb','80+','$10.60','$848']];
  const grid=[['14% Cattle Grower','$14.80','#9C8F7A','BULK'],['Beef Mineral Tub','$132.00','#C2A878',null],['Creep Feed Pellet','$16.40','#D8D2C4','NEW'],['Dairy 18% Textured','$18.20','#8A8780',null],['Non-GMO Stocker','$21.60','#C8A14E',null],['Whole Shell Corn','$11.90','#9A938A',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          <span style={{ width:14, height:14, borderRadius:999, background:'var(--uc-black)' }}/>
          <BrandMark fontSize={11}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Cattle','Swine','Poultry','Show Feeds'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{
        ...
        <div style={{ padding:'6px 13px', borderBottom:'1px solid var(--line-1)', background:'var(--uc-cream)' }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-1)', marginBottom:4 }}>🚚 You&rsquo;ve unlocked <span style={{ color:'#3F8B5D', fontWeight:700 }}>FREE farm delivery</span></div>
        ...
        {/* lines */}
        {lines.map((l,i)=>(
          <div key={i} style={{ padding:'6px 13px', borderBottom:'1px solid var(--line-1)', display:'grid', gridTemplateColumns:'22px 1fr auto', gap:9, alignItems:'center' }}>
            <div style={{ width:22, height:22, borderRadius:3, background:'linear-gradient(135deg, var(--uc-stone-200), var(--uc-bone))' }}/>
            <div><div style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:10, color:'var(--fg-1)' }}>{l[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>qty {l[1]} · <span style={{ color:'#1844a2', fontWeight:700 }}>{l[2]}</span></div></div>
        ...
        <div style={{ padding:'7px 13px', background:'var(--uc-cream)', borderBottom:'1px solid var(--line-1)' }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:5 }}>PAIRS WELL WITH</div>
          <div style={{ display:'flex', gap:5 }}>{[['Mineral Block','$24'],['Feed Scoop','$12'],['Bag Ties','$8']].map((u,i)=>(<span key={i} style={{ flex:1, border:'1px solid var(--line-1)', borderRadius:3, padding:'4px 6px', background:'var(--uc-paper)', display:'flex', justifyContent:'space-between', alignItems:'center', gap:4, minWidth:0 }}><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7, color:'var(--fg-1)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{u[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--uc-black)', whiteSpace:'nowrap' }}>+{u[1]}</span></span>))}</div>
        </div>
        {/* StorePro subscription upgrade */}
        <div style={{ padding:'7px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          <span style={{ width:16, height:16, borderRadius:999, border:'2px solid #1844a2', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}><span style={{ width:7, height:7, borderRadius:999, background:'#1844a2' }}/></span>
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Join Grower Program <span style={{ color:'#1844a2' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>Standing order · free delivery · ration support</div></div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'#fff', background:QP, padding:'4px 8px', borderRadius:3 }}>ADD</span>
        </div>
        {/* totals + checkout */}
        <div style={{ marginTop:'auto', padding:'8px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$2,880</span></div>
          ...
          <span style={{ display:'block', textAlign:'center', padding:'7px', background:'var(--uc-black)', color:'var(--uc-paper)', borderRadius:3, fontSize:9.5, fontWeight:700 }}>Checkout · NET-30 · $2,570</span>
        </div>
      </div>
    </div>
  );
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / GOATS / CONCENTRATES</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#1844a2','#3A3632','#8A8780','#C8A14E'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #1844a2, #12336F 60%, #2A2622)', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>QUALITY PLUS · IN STOCK · MILLED WEEKLY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>32% GOAT<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Concentrate</span></div>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'#1844a2' }}>★★★★★</span><span style={{ color:'var(--fg-3)' }}>4.9 · 128 reviews</span></div>
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#1844a2','#1F1F1F','#C8A14E','#8A8780'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Safe Feed / Safe Food','✓ Non-GMO certified','✓ Ractopamine free'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['32% Goat Concentrate','$21.40','#1844a2'],['20% Goat Blender','$18.60','#1F1F1F'],['Goat Mineral','$26.00','#8A8780']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Program <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$66</span></span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        ...
        <div style={{ background:'var(--uc-cream)', padding:'11px 14px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['Balanced, highly digestible ration — built for herd health.','Grower pricing and NET terms unlock by the ton.','Milled locally in St. Paul, Iowa — fresh every week.'].map((t,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'12px 1fr', gap:7, alignItems:'start' }}><span style={{ width:10, height:10, borderRadius:999, background:'#1844a2', marginTop:1, display:'flex', alignItems:'center', justifyContent:'center' }}><svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="#fff" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/></svg></span><span style={{ fontFamily:'var(--font-serif)', fontSize:9, lineHeight:1.35, color:'var(--fg-2)' }}>{t}</span></div>))}</div>
        </div>
      </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'#1844a2' }}>★★★★★</div>
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“Kids gained fast and stayed healthy. We feed nothing else now.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        <div style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:8 }}>COMPLETE THE PROGRAM</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['20% Goat Blender','$18.60','#1844a2'],['Goat Mineral','$26.00','#8A8780'],['Gestation Feed','$19.80','#3A3632'],['Show Goat 16%','$23.40','#C8A14E']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxPortal() {
  const rows=[['#SO-9421','8 tons','Approved'],['#SO-9420','2 tons','Milling'],['#SO-9419','24 bags','Pending'],['#SO-9418','6 tons','Shipped'],['#SO-9417','12 bags','Delivered'],['#SO-9416','4 tons','Delivered']];
  const nav=[['◧','Dashboard',true],['▤','Orders',false],['❏','Quotes',false],['♡','Standing orders',false],['⚙','Team',false]];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>GROWER ACCOUNT PORTAL · NET-45 TERMS ACTIVE</div>
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <span style={{ width:14, height:14, borderRadius:999, background:'var(--uc-black)' }}/>
        <BrandMark fontSize={11}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>STONE RIDGE CATTLE CO.</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'112px 1fr', gap:1, background:'var(--line-1)' }}>
        ...
        <div style={{ background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:3 }}>
          {nav.map((n,i)=>(<div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'5px 7px', borderRadius:3, background:n[2]?'var(--uc-black)':'transparent' }}><span style={{ fontSize:9, color:n[2]?'#1844a2':'var(--fg-3)' }}>{n[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:n[2]?'var(--uc-paper)':'var(--fg-2)' }}>{n[1]}</span></div>))}
          <div style={{ marginTop:'auto', padding:'8px 7px 0' }}><div style={{ fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)', lineHeight:1.5 }}>CREDIT LINE<br/><span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:13, letterSpacing:'-0.02em', color:'var(--fg-1)' }}>$64k</span> open</div></div>
        </div>
        ...
        <div style={{ background:'var(--uc-paper)', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ padding:'0 14px 12px' }}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'70px 1fr auto auto', gap:8, alignItems:'center', padding:'6px 0', borderTop:'1px solid var(--line-1)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'#fff', background:QP, padding:'2px 7px', borderRadius:2 }}>Reorder</span>
            </div>))}
          </div>
        </div>
      </div>
...
function GfxHomepage() {
  const nav = ['Shop','Species','Brands','Find Feed','Grower'];
  const mega = [
    { h: 'By Species', items: ['Cattle','Swine','Poultry','Goats','Sheep','Horse','Deer','Pet'] },
    { h: 'By Program', items: ['Show Feeds','Non-GMO','Mineral & Tubs','Custom Rations'] },
    { h: 'Quick Links', items: ['Grower Account','Standing Orders','Bulk & Ton Pricing','Cash Bids'] }
  ];
  const brands = ['BEASLEY MIX','CRYSTALYX','SULLIVAN','ANILOGICS','SUNGLO','SHOW-RITE'];
  const feat = [['14% Cattle Grower','$14.80','#9C8F7A','BULK'],['Beef Mineral Tub','$132.00','#C2A878',null],['Layer Pellet 16%','$13.40','#D8D2C4','NEW'],['Show Cattle 16%','$22.80','#C8A14E','SHOW']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE DELIVERY ON BULK FEED ORDERS · GROWER PRICING</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:120, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search feeds &amp; rations…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #12336F 135%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'#1844a2' }}>FALL FEEDING PROGRAM · NOW BOOKING</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Partner in nutrition,<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>not just a supplier.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:QP, color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Shop feeds →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'#1844a2', whiteSpace:'nowrap' }}>FIND YOUR FEED</span>
        {['Species','Life stage','Form'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:QP, color:'#fff', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Find Feed →</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #6E5235, #2A2018)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EST. 1970s</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Two Iowa feed mills became one — same promise, milled right, delivered on time.</span>
          <span style={{ alignSelf:'flex-start', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', borderBottom:'1.5px solid #1844a2', paddingBottom:1 }}>Read our story →</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px' }}>
            ...
            <span style={{ display:'inline-flex', gap:6, alignItems:'center' }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, height:22, padding:'0 9px', background:'var(--uc-black)', borderRadius:5, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)' }}>⛒ Cart<span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', minWidth:11, height:11, padding:'0 2px', background:QP, color:'#fff', borderRadius:999, fontSize:6, fontWeight:800 }}>6</span></span>
            </span>
          </div>
          {/* big open search */}
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search feeds &amp; rations…</span>
              <span style={{ alignSelf:'stretch', display:'flex', alignItems:'center', padding:'0 9px', margin:'3px 0', background:QP, color:'#fff', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>GO</span>
            </div>
          </div>
        </div>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>FREE DELIVERY ON BULK FEED ORDERS · GROWER PRICING</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #12336F 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'#1844a2' }}>FALL FEEDING PROGRAM · NOW BOOKING</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Partner in<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>nutrition.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:QP, color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Shop feeds →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['BEASLEY','CRYSTALYX','SULLIVAN','SUNGLO','SHOW-RITE'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            <div style={{ display:'flex', gap:13, padding:'10px 12px 8px' }}>{['Best Sellers','New','Non-GMO'].map((t,i)=>(<span key={t} style={{ paddingBottom:4, fontFamily:'var(--font-display)', fontWeight:700, fontSize:9, color:i===0?'var(--fg-1)':'var(--fg-3)', borderBottom:i===0?'2px solid var(--uc-black)':'2px solid transparent' }}>{t}</span>))}</div>
            {/* product grid */}
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['14% Cattle Grower','$14.80','#9C8F7A','BULK'],['Layer Pellet 16%','$13.40','#C2A878',null],['Creep Feed','$16.40','#D8D2C4','NEW'],['Show Cattle 16%','$22.80','#C8A14E','SHOW']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
                  <div style={{ aspectRatio:'1/0.86', borderRadius:5, background:`linear-gradient(150deg, ${p[2]}, var(--uc-bone))`, position:'relative' }}>{p[3] && <span style={{ position:'absolute', top:5, left:5, padding:'1px 6px', background:p[3]==='DEAL'?'#1844a2':(p[3]==='NEW'?'var(--uc-black)':'#3F8B5D'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5, fontWeight:800 }}>{p[3]}</span>}<span style={{ position:'absolute', bottom:5, right:5, width:16, height:16, borderRadius:999, background:'var(--uc-paper)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'var(--fg-1)' }}>＋</span></div>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              <div style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:6 }}>SHOP BY SPECIES</div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Cattle','#C2A878'],['Swine','#9A938A'],['Poultry','#C8A14E'],['Goats','#1844a2']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'#1844a2' }}/>ST. PAUL, IOWA · FAMILY-OWNED</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Feeding Iowa<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>herds since the 70s.</span></span>
            </div>
            ...
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              ...
              <div style={{ position:'absolute', right:6, top:0, width:'52%', aspectRatio:'3/4', borderRadius:5, background:'linear-gradient(150deg, #12336F, var(--uc-bone))', transform:'rotate(6deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.4)' }}/>
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>THE MILL · ST. PAUL, IA</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“Big enough to have great capabilities, yet small enough to do the job right.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['2004','Merged'],['Non-GMO','Certified'],['50yr','Milling']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>Feeding Guides</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #C2A878, #6E5235)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:QP, color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>CATTLE · 7 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Ration basics for fall stockers.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['Feed budgeting &amp; breakevens','5 min','#9C8F7A'],['Choosing a show feed','6 min','#8A8780'],['Mineral programs by season','8 min','#C8A14E']].map((a,i)=>(
...
function BPContent() {
  ...
  return (
    <BPSection id="content" n="07" label="Content" paper tail="STORY · SEO · GEO · PAID">
      ...
      {(() => {
        const seo = [
          ['URL Architecture', 'Define a clean, crawlable URL structure from day one.'],
          ['Indexation Plan', 'Control what gets indexed — products, collections, blogs, pages.'],
          ['Title Tags', 'Templated, keyword-led titles across every page type.'],
          ['Meta Descriptions', 'Written to earn the click, not just fill the tag.'],
          ['Heading Structure', 'Keep correct H1–H6 hierarchy through the rebuild.'],
          ['Body Content', 'Substantive on-page copy on every commercial template.'],
          ...
          ['Alt Text', 'Descriptive alt text on every product and content image.'],
...
function BPTechStack() {
  const rows = [
    ...
    { name: 'Matrixify',                   fn: 'Bulk data import / export',           pri: 'start',  cost: '$50/mo' }
  ];
  const priMeta = {
...
function BPIntegrations() {
  ...
  const activity = [
    ['Order #SO-48217 pushed to Ever.Ag', '2 min ago'],
  ...
  return (
    <BPSection id="integrations" n="11" label="Integrated" tail="HOW IT CONNECTS">
        ...
        <div style={{ display:'flex', background:'#F1F1F1' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:14, flexWrap:'wrap' }}>
            <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
              ...
              <span style={{ fontFamily:pFont, fontSize:12.5, color:'#616161' }}>Ever.Ag ERP integration</span>
            </div>
            <div style={{ display:'flex', gap:8 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
              <div style={{ display:'flex', alignItems:'center', gap:11 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
                  <span style={{ fontFamily:pFont, fontSize:13.5, fontWeight:700, color:'#1A1A1A' }}>Ever.Ag ERP</span>
          ...
          <Card pad={false}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.3fr) clamp(48px,6vw,72px) minmax(0,1.3fr) minmax(0,90px)', gap:'clamp(8px,1.2vw,16px)', padding:'8px 16px', background:'#FAFAFA', borderBottom:'1px solid #E3E3E3' }}>
              {['Shopify','','Ever.Ag','Status'].map((h,i)=>(<span key={i} style={{ fontFamily:pFont, fontSize:10.5, fontWeight:600, color:'#616161', textAlign:i===3?'right':'left' }}>{h}</span>))}
            </div>
            {maps.map((m,i)=>(
      ...
      <div style={{ marginTop: 'clamp(24px, 3vw, 36px)', paddingTop: 'clamp(20px, 2.6vw, 30px)', borderTop: '1px solid var(--line-1)', display: 'flex', flexWrap: 'wrap', gap: 10 }}>
        {['Inventory', 'Orders', 'Fulfilment', 'Customers'].map((s) => (
          <span key={s} style={{ display: 'inline-flex', alignItems: 'center', gap: 9, padding: '10px 16px', border: '1px solid var(--line-2)', borderRadius: 999, fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 'clamp(14px, 1.3vw, 17px)', letterSpacing: '-0.01em', color: 'var(--fg-1)' }}>
...
function BPData() {
  const steps = [
    { t: 'Audit & map', s: 'Catalogue every record — products, collections, content, accounts.' },
    { t: 'Transform', s: 'Clean, dedupe, and shape data to the Shopify model. Nothing loaded blind.' },
    { t: 'Stage & verify', s: 'Dry-run import into staging. Reconcile counts, spot-check edge cases.' },
    { t: 'Load', s: 'Final import, scheduled feed sync, and a verified catalog at launch.' }
  ];
  const stats = ['Products', 'Collections', 'Pages', 'Blogs', 'Customers', 'Companies', 'Orders'];
  return (
    <BPSection id="data" n="12" label="Data" paper tail="CLEAN IN, CLEAN OUT" vec="bgVector2">
      <BPHeadline>
        Data in,{' '}
        <BPSerif>structured and verified.</BPSerif>
      </BPHeadline>
      <p style={{ marginTop: 18, maxWidth: 600, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)' }}>
        Every record is imported through Matrixify against a mapped schema — spec sheets
        become metafields, source categories become collections and tags, attributes land
        where merchandising can use them. Nothing loaded blind, nothing lost.
      </p>

      {/* import field crosswalk */}
      {(() => {
        const source = [['Title','text'],['SKU','text'],['Price','money'],['Brand','text'],['Categories','list'],['spec_sheet','file'],['install_guide','file'],['Attributes','list']];
        const target = [['Title','text','1:1'],['SKU / Variant','text','1:1'],['Price','money','1:1'],['Vendor','text','from Brand'],['Collections','list','mapped'],['spec_sheet','metafield','metafield'],['install_guide','metafield','metafield'],['Tags / Facets','list','mapped']];
          ...
          <div style={{ marginTop:'clamp(30px, 4vw, 48px)', border:'1px solid var(--line-2)', borderRadius:10, overflow:'hidden', background:'var(--uc-paper)', boxShadow:'0 24px 60px -34px rgba(10,10,10,0.4)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:cols, alignItems:'center', borderBottom:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px' }}>
                <span style={{ width:26, height:26, borderRadius:6, background:'var(--uc-black)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:13, fontWeight:800, color:'var(--uc-signal)' }}>▦</span>
                <div style={{ display:'flex', flexDirection:'column', gap:1 }}>
                  <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:14, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Source data</span>
                  <span style={{ fontFamily:'var(--font-mono)', fontSize:9, color:'var(--fg-3)' }}>ERP exports · sheets · supplier feeds</span>
                </div>
              </div>
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)', whiteSpace:'nowrap' }}>MATRIXIFY</span>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:10, padding:'12px 16px' }}>
                <div style={{ display:'flex', flexDirection:'column', gap:1, alignItems:'flex-end' }}>
                  <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:14, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Shopify</span>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:16, padding:'9px 16px', borderBottom:'1px solid var(--line-1)', flexWrap:'wrap' }}>
              {['Products','Collections','Customers','Companies','Content'].map((t,i)=>(<span key={t} style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:11.5, letterSpacing:'-0.01em', color:i===0?'var(--fg-1)':'var(--fg-3)', borderBottom:i===0?'2px solid var(--uc-black)':'2px solid transparent', paddingBottom:3 }}>{t}</span>))}
            ...
            <div style={{ display:'flex', alignItems:'center', gap:18, padding:'11px 16px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)', flexWrap:'wrap', fontFamily:'var(--font-mono)', fontSize:10, color:'var(--fg-3)' }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:7 }}><span style={{ width:18, height:2, background:'var(--uc-signal)' }}/>1:1 import</span>
              <span style={{ display:'inline-flex', alignItems:'center', gap:7 }}><span style={{ width:18, height:2, background:'repeating-linear-gradient(90deg, var(--uc-brand) 0 5px, transparent 5px 9px)' }}/>Transformed — collections · tags · metafields</span>
              <span style={{ marginLeft:'auto' }}>Specs, attributes &amp; custom fields preserved</span>
            </div>
          </div>
        );
      })()}
...
function BPDelivery() {
  const milestones = [
    ...
    { wk: 'WK 11', t: 'Data import + QA', s: 'Catalog and accounts loaded, full QA pass.' },
...
function BPRiskAssessment() {
  ...
  const risks = [
    { t: 'Data import integrity', sev: 'High',
      d: 'Source catalog and account data don\u2019t map cleanly into the Shopify model.',
      m: 'Dry-run into staging, reconcile counts, and spot-check edge cases before launch.' },
  ...
  return (
    <BPSection id="risks" n="14" label="Risks" tail="EYES OPEN">
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', gap: 'clamp(28px, 5vw, 80px)', alignItems: 'end' }}>
        ...
        <p style={{ margin: 0, maxWidth: 380, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)', textWrap: 'pretty' }}>
          Every build carries risk. Here are the ones we watch — each paired with how we manage it.
        </p>
      </div>
...
function BPInvestment() {
  const work = [
    ...
    'Data import: products, collections, customers, content',
  ...
  const erpWork = [
    'Discovery and data mapping across Ever.Ag and Shopify',
    'Field-level schema definition, owned and versioned',
    'Product and pricing sync, including grower and tier pricing',
    'Inventory sync with configurable thresholds',
    'Order push from Shopify into the ERP',
    'Fulfilment and tracking sync back to Shopify',
    'Customer and company account sync, both directions',
    'Error handling, retries, and a live sync log in admin',
    'Staging dry-runs and reconciliation before go-live',
  ...
  return (
    <BPSection id="investment" n="15" label="Investment" dark tail="FIXED · NO SURPRISES">
      <BPPackagePanel
        title="Shopify" serif="Build"
        tagline="Your store and your integrated systems, working as one."
        blurb={<>A modern unified B2C/B2B storefront on Shopify with a tech stack and customer experience — everything {brandName()} needs to launch and scale.</>}
        price="$46k"
        walkaway="Your successful commerce implementation on Shopify, built for growth."
        work={work}
      />

      <div style={{ marginTop: 'clamp(20px, 2.4vw, 32px)' }}>
        <BPPackagePanel
          title="ERP" serif="Integration"
          tagline="Shopify and Ever.Ag, in sync without re-keying."
          blurb={<>A real-time, bidirectional integration between {brandName()}&rsquo;s ERP and Shopify — built and monitored through Uncap Connect.</>}
          price="$22k"
          walkaway="One source of truth across inventory, orders, fulfilment, and customers."
          work={erpWork}
        />
      </div>
    ...
    <BPSection id="growth" n="16" label="Growth" paper tail="AFTER LAUNCH">
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 15, color: 'var(--fg-1)' }}>
          Uncap Growth — Core Package · <span style={{ fontFamily: 'var(--font-hero)', fontWeight: 800 }}>$2.5k / mo</span>
        </div>
  ...
  return (
    <BPSection id="proof" n="19" label="Proof" paper tail="ON THE RECORD">
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', gap: 'clamp(28px, 5vw, 80px)', alignItems: 'end' }}>
        ...
        <p style={{ margin: 0, maxWidth: 380, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)', textWrap: 'pretty' }}>
          Operators who ran the same play — built, integrated, and grew on Shopify with Uncap.
        </p>
      </div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'qpfeed_approved_v1';
