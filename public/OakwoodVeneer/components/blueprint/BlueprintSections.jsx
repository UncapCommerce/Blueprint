...
function brandName() { return (window.__brand && window.__brand.name) || 'Oakwood Veneer'; }
function brandHandle() { return brandName().toLowerCase().replace(/[^a-z0-9]+/g, ''); }
function BrandMark({ fontSize = 11, color = 'var(--fg-1)', family = 'var(--font-display)', weight = 700, letterSpacing = '-0.01em', dotColor = 'var(--uc-brand)', height, logo }) {
  const b = window.__brand || {};
  if (logo) return <img src={logo} alt={brandName()} style={{ height: height || Math.round(fontSize * 1.5), width: 'auto', display: 'block', objectFit: 'contain' }}/>;
...
function BPIntro() {
  ...
  const coName = (bpCo && bpCo.name) || 'Oakwood Veneer';
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 026 · PREPARED SEPTEMBER 2026 · CONFIDENTIAL</span>
        </div>
    ...
    <BPSection id="summary" n="01" label="Executive Summary" paper tail="TL;DR">
      ...
      <div style={{
        ...
        {[
          ...
          { v: 'Unified', l: 'Storefront & B2B on Shopify integrated with ERP' },
...
function BPWhere() {
  const items = [
    { n: '01', t: 'Cumbersome storefront', tag: 'Storefront', impact: 'High', pct: 82, d: 'Custom-built, patched for years, costing more every renewal cycle.' },
...
// Client palette for storefront mockups (Oakwood Veneer)
const OV_THEME = { '--uc-black':'#2A6666', '--uc-signal':'#A3D9D6', '--uc-brand':'#2A6666' };
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>SAME-DAY SHIPPING ON IN-STOCK VENEER · MADE IN THE USA</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <span style={{ display:'flex', alignItems:'center', gap:6 }}><BrandMark logo="assets/oakwood-veneer-logo.svg" fontSize={12} letterSpacing="-0.015em"/></span>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Sheets','Panels','Burls','Edging'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Veneer Sheets', items: ['Walnut','White Oak','Cherry','Maple'] },
    { h: 'Veneer Panels', items: ['Top Sellers','All Species','Custom Panels','Paintable'] },
    { h: 'Edgebanding', items: ['Species List','Veneer Tape','Walnut','Oak'] },
    { h: 'Featured', items: ['Premium Burls','Sample Book','Loyalty Rewards'] }
  ];
  return (
    <GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Walnut Flat Cut','$92.48','from','#6B4A2F','BEST','4.9','212'],
    ['Walnut Quartered','$127.68','from','#5E412B',null,'4.8','158'],
    ['Walnut Rift','$249.28','from','#7A5638','NEW','4.9','64'],
    ['Italian Quartered','$162.88','from','#6E4E34',null,'4.8','93'],
    ['Walnut Burl · Med','$895.68','4×8','#4E3220','BURL','5.0','47'],
    ['Two Tone w/Sap','$242.88','from','#9C7A55',null,'4.7','121']
  ];
  const facets = [
    { h: 'Cut', items: [['Flat cut', true],['Quartered', true],['Rift', false],['Planked', false],['Burl', false]] },
    { h: 'Tone', items: [['Dark', true],['Medium', false],['Two-tone', false]] },
    { h: 'Backing', items: [['10 mil', true],['WOW', false],['PSA', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
                <BrandMark logo="assets/oakwood-veneer-logo.svg" fontSize={11}/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Sheets','Panels','Burls','Edging'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / VENEER SHEETS / TOP SELLERS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Walnut</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>36 results</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Flat cut ✕','Dark ✕','10 mil ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'104px 1fr', borderTop:'1px solid var(--line-1)' }}>
        ...
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:11 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$90</span><span>$960</span></div>
          </div>
        </div>
          ...
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              <div style={{ aspectRatio:'1/1', background:`linear-gradient(140deg, ${p[3]}, var(--uc-bone))`, borderRadius:2, position:'relative' }}>
                {p[4] && <span style={{ position:'absolute', top:4, left:4, padding:'1px 5px', background:p[4]==='NEW'?'var(--uc-brand)':(p[4]==='BURL'?'var(--uc-black)':'#2A6666'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800 }}>{p[4]}</span>}
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#2A6666' }}>Trade price ✓</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:'9px 14px', borderTop:'1px solid var(--line-1)', display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
        {['‹','1','2','›'].map((n,i)=>(<span key={i} style={{ minWidth:15, textAlign:'center', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:n==='1'?'var(--uc-paper)':'var(--fg-2)', background:n==='1'?'var(--uc-black)':'transparent', borderRadius:2, padding:'2px 4px' }}>{n}</span>))}
      </div>
    </div>
  );
}
function GfxCart() {
  const lines=[['Walnut Flat Cut','5','$87.86','$439'],['Walnut Quartered','3','$121.30','$364'],['Walnut Burl','1','$639.68','$640']];
  const grid=[['Walnut Flat Cut','$92.48','#6B4A2F','BEST'],['Walnut Quartered','$127.68','#5E412B',null],['Walnut Rift','$249.28','#7A5638','NEW'],['Italian Quartered','$162.88','#6E4E34',null],['Walnut Burl','$639.68','#4E3220',null],['Two Tone w/Sap','$242.88','#9C7A55',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
                    <BrandMark logo="assets/oakwood-veneer-logo.svg" fontSize={11}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Sheets','Panels','Burls','Edging'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{
        ...
        <div style={{ padding:'9px 13px', borderBottom:'1px solid var(--line-1)', background:'var(--uc-cream)' }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-1)', marginBottom:5 }}>🚚 You&rsquo;ve unlocked <span style={{ color:'#2A6666', fontWeight:700 }}>FREE shipping</span></div>
          <div style={{ height:4, background:'var(--line-1)', borderRadius:999, overflow:'hidden' }}><span style={{ display:'block', width:'100%', height:'100%', background:'#2A6666' }}/></div>
        </div>
        {/* lines */}
        {lines.map((l,i)=>(
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Join Loyalty Rewards <span style={{ color:'var(--uc-signal)' }}>+pts</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>Earn on every order · redeem on veneer</div></div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'var(--uc-black)', background:'var(--uc-signal)', padding:'4px 8px', borderRadius:3 }}>JOIN</span>
        </div>
        {/* totals + checkout */}
        <div style={{ marginTop:'auto', padding:'10px 13px', borderTop:'1px solid var(--line-1)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$1,485</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'#2A6666', marginBottom:7 }}><span>Tier savings</span><span>−$42</span></div>
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'var(--uc-black)', color:'var(--uc-paper)', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · NET-30 · $1,443</span>
        </div>
      </div>
    </div>
  );
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / VENEER SHEETS / WALNUT</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#6B4A2F','#4E3220','#8A6A4A','#B08A62'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #9C7250, #5E412B 60%, #2E1F14)', position:'relative', overflow:'hidden' }}>
            <span style={{ position:'absolute', top:8, left:8, padding:'2px 7px', background:'#2A6666', color:'#fff', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>BEST SELLER</span>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#2A6666' }}>IN STOCK · SHIPS TODAY · MADE IN USA</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>WALNUT<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Veneer · Flat Cut</span></div>
          ...
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#6B4A2F','#3A2A1E','#B08A62','#8A6A4A'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          {/* configuration — configurable product */}
          <div style={{ marginTop:1 }}>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:4 }}>BACKING</div>
            <div style={{ display:'flex', gap:4 }}>{[['10 mil',true],['WOW',false],['PSA',false]].map(([l,on],i)=>(<span key={i} style={{ flex:1, padding:'5px 2px', textAlign:'center', border:'1px solid', borderColor:on?'var(--uc-black)':'var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:on?'var(--uc-paper)':'var(--fg-2)', background:on?'var(--uc-black)':'transparent', whiteSpace:'nowrap' }}>{l}</span>))}</div>
          </div>
          {/* tier pricing — minimalist table */}
          <div style={{ marginTop:2, border:'1px solid var(--line-1)', borderRadius:3, overflow:'hidden' }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)' }}>{[['5+','$87.86'],['20+','$83.23'],['50+','$78.61'],['100+','$73.98']].map((t,i)=>(
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Made in USA','✓ $2 samples','✓ Ships today'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['Walnut Flat Cut','$92','#6B4A2F'],['Walnut Edgebanding','$24','#4E3220'],['Veneer Tape','$12','#C9B08A']].map((p,i)=>(
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Species','Black Walnut'],['Cut','Flat cut'],['Size','4′ × 8′'],['Backing','10 mil paper']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“Gorgeous cathedrals, no cracks. Refaced the whole kitchen with it.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Walnut Quartered','$127','#5E412B'],['Walnut Rift','$249','#7A5638'],['Walnut Burl','$639','#4E3220'],['Samples','$2','#C9B08A']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxPortal() {
  const rows=[['#PO-9421','24 sheets','Approved'],['#PO-9420','8 sheets','Picking'],['#PO-9419','42 sheets','Pending'],['#PO-9418','36 sheets','Shipped'],['#PO-9417','12 sheets','Delivered'],['#PO-9416','64 sheets','Delivered']];
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
                <BrandMark logo="assets/oakwood-veneer-logo.svg" fontSize={11}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>MERIDIAN MILLWORK CO.</span>
...
function GfxHomepage() {
  const nav = ['Sheets','Panels','Burls','Edging'];
  const mega = [
    { h: 'Top Sellers', items: ['Walnut','White Oak','Cherry','Maple','Teak','Mahogany'] },
    { h: 'By Product', items: ['Veneer Sheets','Veneer Panels','Premium Burls','Edgebanding'] },
    { h: 'Quick Links', items: ['Order Samples','Sample Book','Loyalty Rewards','Request a Quote'] }
  ];
  const brands = ['LIGHT','MEDIUM','DARK','RED','GREY','TWO-TONE','RUSTIC'];
  const feat = [['Walnut Flat Cut','$92.48','#6B4A2F','BEST'],['Walnut Rift','$249.28','#7A5638','NEW'],['Walnut Burl','$639.68','#4E3220','BURL'],['Two Tone w/Sap','$242.88','#9C7A55',null]];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>SAME-DAY SHIPPING ON IN-STOCK VENEER · MADE IN THE USA</div>
      {/* header */}
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <BrandMark logo="assets/oakwood-veneer-logo.svg" fontSize={12}/>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:120, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search 100+ species…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #143333 0%, #2A6666 62%, #56B9B9 140%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>CUSTOM PANELS · MADE TO ORDER</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Premium veneer,<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>made in the USA.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Shop veneer →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--uc-signal)', whiteSpace:'nowrap' }}>FIND YOUR VENEER</span>
        {['Species','Tone','Cut'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Search →</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #6E5235, #2A2018)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>35+ YEARS</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>Trusted by woodworkers & designers for over 35 years.</span>
  ...
  const groups = [
    {
      ...
      points: [
        ...
        { t: 'Information Architecture', d: 'Mega navigation with multi-level menus built around how woodworkers and designers actually browse.' },
        { t: 'Product Finding', d: 'Species, tone, and cut finders, quizzes, and creative ways to find the right veneer.' }
      ],
      gfx: <GfxHomepage/>
    },
    {
      ...
      <div style={{ marginTop:'clamp(36px, 4.5vw, 64px)', display:'flex', flexDirection:'column', gap:0 }}>
        {groups.map((g, i) => (
          <div key={i} style={{
            ...
            <div style={{ order: i % 2 === 0 ? 2 : 1, ...OV_THEME }}>{g.gfx}</div>
          </div>
        ))}
      </div>
    </BPSection>
...
function GfxPhone({ kind }) {
  // Mobile veneer storefront. kind: 'home' | 'about' | 'blog'
  return (
    <div style={{
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px' }}>
            <span style={{ display:'inline-flex', alignItems:'center', gap:8 }}>
              ...
              <BrandMark logo="assets/oakwood-veneer-logo.svg" fontSize={14} family="var(--font-hero)" weight={800} letterSpacing="-0.045em"/>
            </span>
          ...
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search 100+ species…</span>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>SAME-DAY SHIPPING ON IN-STOCK VENEER · MADE IN THE USA</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #143333 0%, #2A6666 62%, #56B9B9 140%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>CUSTOM PANELS · MADE TO ORDER</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Premium veneer,<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>made in the USA.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'var(--uc-signal)', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Shop veneer →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['LIGHT','MEDIUM','DARK','RED','RUSTIC'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Walnut Flat Cut','$92.48','#6B4A2F','BEST'],['Walnut Quartered','$127.68','#5E412B',null],['Walnut Rift','$249.28','#7A5638','NEW'],['Walnut Burl','$639.68','#4E3220','DEAL']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
                  <div style={{ aspectRatio:'1/0.86', borderRadius:5, background:`linear-gradient(150deg, ${p[2]}, var(--uc-bone))`, position:'relative' }}>{p[3] && <span style={{ position:'absolute', top:5, left:5, padding:'1px 6px', background:p[3]==='DEAL'?'var(--uc-brand)':(p[3]==='NEW'?'var(--uc-black)':'#2A6666'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5, fontWeight:800 }}>{p[3]}</span>}<span style={{ position:'absolute', bottom:5, right:5, width:16, height:16, borderRadius:999, background:'var(--uc-paper)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'var(--fg-1)' }}>＋</span></div>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Sheets','#B08A62'],['Panels','#8A6A4A'],['Burls','#4E3220'],['Edging','#C9B08A']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--uc-signal)' }}/>TROY, MI · 35+ YEARS</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Trusted by makers<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>for 35+ years.</span></span>
            </div>
            ...
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              ...
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>TROY, MICHIGAN</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“The largest in-stock veneer inventory in North America — hand-selected and shipped same day.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['35+','Years'],['100+','Species'],['USA','Made']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>Tips & Info</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              ...
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>CABINETRY · 7 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Wood veneer for cabinet refacing.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['Veneer application hints','5 min','#B08A62'],['Wood veneer acclimation','4 min','#8A6A4A'],['Veneer matching techniques','6 min','#6B4A2F']].map((a,i)=>(
...
function BPContent() {
  const screens = [
    { kind: 'home',  label: 'Landing Pages', d: 'Short hero, shop-by-tone strip, live product tabs, and a product grid — a merchandising surface that gets makers to the full catalog fast.' },
    { kind: 'about', label: 'Brand Story', d: 'Maker story and proof — 35+ years, Made in USA, and the credentials that win designer trust and rank for branded search.' },
  ...
  return (
    <BPSection id="content" n="07" label="Content" paper tail="STORY · SEO · GEO · PAID">
      ...
      <p style={{
        ...
        We build the full mobile content surface for a large-catalog veneer manufacturer — homepage,
        brand story, and tips & guides. Each screen is structured for SEO and generative
      ...
      <div style={{
        ...
        {screens.map((s, i) => (
          <div key={i} style={{ display:'flex', flexDirection:'column', gap:18 }}>
            <div style={{ maxWidth:260, margin:'0 auto', width:'100%', ...OV_THEME }}><GfxPhone kind={s.kind}/></div>
...
function BPTechStack() {
  const rows = [
    ...
    { name: 'ShipperHQ',                   fn: 'Shipping rates & delivery rules',     pri: 'launch', cost: '$350/mo' },
    { name: 'JudgeMe',                     fn: 'Product reviews & social proof',      pri: 'launch', cost: '$15/mo' },
    { name: 'Love Loyalty',                fn: 'Loyalty programs & rewards',          pri: 'launch', cost: '$79/mo' },
  ...
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          const layers = [
            { tier:'04', name:'Growth & Intelligence', note:'After launch', tools:[['Shopify Flow','launch'],['Shopify Forms','launch'],['JudgeMe','launch'],['Love Loyalty','launch'],['Shopify Knowledge Base','launch']] },
            { tier:'03', name:'Experience & Engagement', note:'Customer-facing', tools:[['Shopify Search & Discovery','launch'],['Shopify Checkout Blocks','launch'],['Shopify Bundles','launch'],['Shopify Inbox','launch'],['ShipperHQ','launch'],['Uncap Quotes','start']] },
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) auto', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span/>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(18px,1.8vw,24px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right', whiteSpace:'nowrap' }}>~$3,274/mo</span>
        </div>
      </div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'oakwoodveneer_approved_v1';
  ...
  const BRAND_NAME              = 'Oakwood Veneer';
