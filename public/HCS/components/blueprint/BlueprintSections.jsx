...
function brandName() { return (window.__brand && window.__brand.name) || 'Hydro-Chem Systems'; }
function brandHandle() { return brandName().toLowerCase().replace(/[^a-z0-9]+/g, ''); }
// Logo used ONLY inside storefront wireframes (client's HCS mark, no dot)
function WireLogo({ height = 15 }) { return <img src="assets/hcs-logo.svg" alt={brandName()} style={{ height, width: 'auto', display: 'block', flexShrink: 0 }}/>; }
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 022 · PREPARED SEPTEMBER 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              A plan to build {brandName()}&rsquo;s new commerce operation on Shopify, consolidate
              the tech stack, and create an exceptional experience for growth.
            </div>
          </div>
...
function BPWhere() {
  const items = [
    { n: '01', t: 'No ecommerce experience', tag: 'Commerce', impact: 'High', pct: 82, d: 'No way to sell products online — no self-service, no buy online, no checkout.' },
    { n: '02', t: 'Outdated website', tag: 'Storefront', impact: 'High', pct: 82, d: 'Unfriendly website with no clear information architecture, navigation, and no mobile optimization.' },
    { n: '03', t: 'Limiting growth opportunities', tag: 'Systems', impact: 'Critical', pct: 94, d: 'Lack of ability to integrate and plug and play with modern advanced commerce tools and solutions.' },
...
function BPObjectives() {
  const items = [
    { n: '01', t: 'Seamless implementation', d: 'Build on Shopify without losing what works today — while optimizing for growth.', metric: 'Zero',   unit: 'disruption',       tag: 'New build' },
    ...
    { n: '03', t: 'Low cost of ownership', d: 'Fewer tools, one partner. Retire the current stack and its renewals.',          metric: 'Lower',  unit: 'total cost',       tag: 'Finance' },
...
function BPApproach() {
  const steps = [
    { n: 'Phase 1', t: 'Blueprint', d: 'Architecture, data model, integration map. A plan that holds up under scrutiny.', wk: 'Wk 01–04' },
    { n: 'Phase 2', t: 'Build', d: 'Storefront, B2B, ERP sync. Fixed scope, senior team, no surprises.', wk: 'Wk 05–20' },
    { n: 'Phase 3', t: 'Grow', d: 'Go live, then optimize conversion, AOV, and automation.', wk: 'Wk 20+' }
  ];
  return (
...
function BPGantt() {
  const groups = [
    { name: 'Foundation', range: 'WK 01–08', tasks: [
      { n: '01', t: 'Onboarding & Kickoff',       tag: 'Setup',    s: 1, e: 2,  range: 'W1–2' },
      { n: '02', t: 'Deep Dive Workshops',        tag: 'Strategy · Solutions · Design',   s: 1, e: 4,  range: 'W1–4' },
      { n: '03', t: 'Experience Design',  tag: 'UX/UI Design · Web · Mobile',   s: 3, e: 7,  range: 'W3–7' },
      { n: '04', t: 'Approval',                   tag: 'Tech Stack Review & Design Sign-off', s: 7, e: 8,  range: 'W7–8' }
    ] },
    { name: 'Production', range: 'WK 08–19', tasks: [
      { n: '05', t: 'Development',                tag: 'Build',    s: 8,  e: 16, range: 'W8–16' },
      { n: '06', t: 'Data Import',                tag: 'Import',   s: 12, e: 16, range: 'W12–16' },
      { n: '07', t: 'Integration',                tag: 'Connect',  s: 12, e: 18, range: 'W12–18' },
      { n: '08', t: 'QA & Testing',               tag: 'Verify',   s: 16, e: 19, range: 'W16–19' }
    ] },
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--wf-main)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>100% EMPLOYEE-OWNED · CALEDONIA, MI · FLEET ACCOUNT PRICING</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <WireLogo height={16}/>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Wash Systems','Chemicals','Mobile Fleet','Parts'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
        <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:8 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search…</span>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'var(--uc-paper)', background:'var(--wf-main)', padding:'3px 8px', borderRadius:3 }}>Cart 2</span>
        </div>
      </div>
      {children}
    </div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Step 1 Pre-soaks', items: ['Counterpunch','Uppercut','Citric Pre-soak','Blue Fusion'] },
    { h: 'Step 2 Soaps', items: ['Panda','Panda LF','Apex','Grizzly'] },
    { h: 'More Chemistry', items: ['Friction detergents','Degreasers & floor','Specialty fleet','Drums & totes'] },
    { h: 'Featured', items: ['Find your chemistry','New · Uppercut & Apex','Fleet account pricing'] }
  ];
  return (
    <GfxChrome>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:1, background:'var(--line-1)' }}>
        {cols.map((c,ci)=>(
          <div key={ci} style={{ background:'var(--uc-paper)', padding:'12px 13px 16px', display:'flex', flexDirection:'column', gap:7 }}>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color: ci===3?'var(--wf-main)':'var(--fg-3)' }}>{c.h}</div>
            {c.items.map((it,ri)=>(<span key={ri} style={{ fontFamily:'var(--font-display)', fontWeight:ci===3&&ri===0?700:500, fontSize:10.5, letterSpacing:'-0.01em', color: ci===0&&ri===0?'var(--wf-main)':'var(--fg-2)', display:'flex', alignItems:'center', gap:5 }}>{ci===0&&ri===0 && <span style={{ width:14, height:3, background:'var(--wf-cta)' }}/>}{it}</span>))}
          </div>
        ))}
      </div>
    </GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['#1 Counterpunch','$138','5 gal','#3E70A0','BEST','4.9','212'],
    ['#1 Uppercut','$112','5 gal','#1E4F80','NEW','4.8','36'],
    ['#1 Blue Fusion','$146','5 gal','#0A3560',null,'4.7','88'],
    ['#2 Panda','$164','5 gal','#6F97BF',null,'4.9','309'],
    ['#2 Apex','$98','5 gal','#A9C3DC','NEW','4.8','41'],
    ['#2 Grizzly','$152','5 gal','#002040',null,'4.7','127']
  ];
  const facets = [
    { h: 'Process', items: [['Step 1 pre-soak', true],['Step 2 soap', true],['1-step soap', false],['Friction soap', false],['Degreaser', false]] },
    { h: 'Surface', items: [['Polish-safe', true],['Painted', false],['Non-polished', false]] },
    { h: 'Availability', items: [['In stock', true],['Ships today', false],['Local pickup', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <WireLogo height={15}/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Wash Systems','Chemicals','Mobile Fleet','Parts'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          ...
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)', background:'var(--wf-main)', padding:'3px 8px', borderRadius:3 }}>Cart 0</span>
        </span>
      </div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / CHEMICALS / TRUCK WASH SOAPS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Truck Wash Soaps</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>48 results</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['2-step ✕','Polish-safe ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
        ...
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:11 }}>
          {facets.map((f,fi)=>(
            <div key={fi} style={{ display:'flex', flexDirection:'column', gap:6 }}>
              ...
              {f.items.map(([l,on],i)=>(<div key={i} style={{ display:'flex', gap:6, alignItems:'center' }}><span style={{ width:9,height:9,borderRadius:2, background:on?'var(--wf-main)':'var(--uc-paper)', border:'1px solid var(--line-2)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{on && <svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="var(--uc-paper)" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg>}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, color:on?'var(--fg-1)':'var(--fg-3)', fontWeight:on?700:400 }}>{l}</span></div>))}
            </div>
          ))}
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)' }}>PRICE</div>
            <div style={{ height:3, background:'var(--line-1)', borderRadius:2, position:'relative' }}><span style={{ position:'absolute', left:'15%', right:'35%', top:0, bottom:0, background:'var(--wf-main)', borderRadius:2 }}/><span style={{ position:'absolute', left:'15%', top:'50%', transform:'translate(-50%,-50%)', width:7, height:7, borderRadius:999, background:'var(--uc-paper)', border:'1.5px solid var(--wf-main)' }}/><span style={{ position:'absolute', left:'65%', top:'50%', transform:'translate(-50%,-50%)', width:7, height:7, borderRadius:999, background:'var(--uc-paper)', border:'1.5px solid var(--wf-main)' }}/></div>
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$40</span><span>$1,400</span></div>
          </div>
        </div>
          ...
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              <div style={{ aspectRatio:'1/1', background:`linear-gradient(140deg, ${p[3]}, var(--uc-bone))`, borderRadius:2, position:'relative' }}>
                {p[4] && <span style={{ position:'absolute', top:4, left:4, padding:'1px 5px', background:p[4]==='NEW'?'var(--uc-brand)':(p[4]==='PRO'?'var(--wf-main)':'#3F8B5D'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800 }}>{p[4]}</span>}
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#3F8B5D' }}>Fleet price ✓</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:'9px 14px', borderTop:'1px solid var(--line-1)', display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
        {['‹','1','2','3','…','8','›'].map((n,i)=>(<span key={i} style={{ minWidth:15, textAlign:'center', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:n==='1'?'var(--uc-paper)':'var(--fg-2)', background:n==='1'?'var(--wf-main)':'transparent', borderRadius:2, padding:'2px 4px' }}>{n}</span>))}
      </div>
    </div>
  );
}
function GfxCart() {
  const lines=[['#1 Counterpunch · 55 gal','2','$1,180','$2,360'],['#2 Panda · 55 gal','2','$1,320','$2,640'],['2-Step Gun System','1','$1,450','$1,450']];
  const grid=[['#2 Panda','$164','#0A3560','BEST'],['#2 Panda LF','$168','#6F97BF',null],['#2 Apex','$98','#1E4F80','NEW'],['#2 Grizzly','$152','#002040',null],['#2 Buzzsaw','$104','#A9C3DC','NEW'],['#2 Surefire','$158','#3E70A0',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          <WireLogo height={15}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Wash Systems','Chemicals','Mobile Fleet','Parts'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
          <span style={{ marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)', background:'var(--wf-main)', padding:'3px 8px', borderRadius:3 }}>Cart 3</span>
        </div>
        {/* category title + breadcrumb */}
        <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
          <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / CHEMICALS / STEP 2 SOAPS</span>
            <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Step 2 Soaps</span>
          </div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, color:'var(--fg-3)' }}>9 products · Sort ▾</span>
        </div>
        {/* filter chips */}
        <div style={{ display:'flex', gap:6, padding:'0 14px 11px' }}>{['Polish-safe','Low foam','Aggressive','In stock'].map((f,i)=>(<span key={f} style={{ padding:'3px 9px', borderRadius:999, border:'1px solid', borderColor:i===0?'var(--wf-main)':'var(--line-1)', background:i===0?'var(--wf-main)':'transparent', color:i===0?'var(--uc-paper)':'var(--fg-2)', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700 }}>{f}</span>))}</div>
        {/* product grid */}
      ...
      <div style={{
        ...
        {lines.map((l,i)=>(
          <div key={i} style={{ padding:'8px 13px', borderBottom:'1px solid var(--line-1)', display:'grid', gridTemplateColumns:'26px 1fr auto', gap:9, alignItems:'center' }}>
            ...
            <div><div style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:10, color:'var(--fg-1)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{l[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>qty {l[1]} · <span style={{ color:'var(--uc-brand)', fontWeight:700 }}>{l[2]}</span></div></div>
        ...
        <div style={{ padding:'7px 13px', background:'var(--uc-cream)', borderBottom:'1px solid var(--line-1)' }}>
          ...
          <div style={{ display:'flex', gap:6 }}>{[['Drum Pump','$86'],['Spray Tips','$24'],['HCS-400 Degreaser','$118']].map((u,i)=>(<div key={i} style={{ flex:1, border:'1px solid var(--line-1)', borderRadius:3, padding:5, display:'flex', flexDirection:'column', gap:3, background:'var(--uc-paper)' }}><div style={{ aspectRatio:'2/1', background:'var(--uc-stone-200)', borderRadius:2 }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7, color:'var(--fg-1)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{u[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--wf-main)' }}>+ {u[1]}</span></div>))}</div>
        </div>
        {/* Fleet account upgrade */}
        <div style={{ padding:'10px 13px', background:'var(--wf-main)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          <span style={{ width:18, height:18, borderRadius:999, border:'2px solid var(--wf-cta)', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}><span style={{ width:7, height:7, borderRadius:999, background:'var(--wf-cta)' }}/></span>
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Open a Fleet Account <span style={{ color:'var(--wf-cta)' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>NET-30 terms · fleet pricing · priority service</div></div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'#FFFFFF', background:'var(--wf-cta)', padding:'4px 8px', borderRadius:3 }}>APPLY</span>
        </div>
        </div>
        {/* totals + checkout */}
        <div style={{ flexShrink:0, padding:'10px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'var(--fg-3)', marginBottom:3 }}><span>Subtotal</span><span>$6,450</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:8.5, color:'#3F8B5D', marginBottom:7 }}><span>Tier savings</span><span>−$640</span></div>
          <span style={{ display:'block', textAlign:'center', padding:'9px', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:3, fontSize:10, fontWeight:700 }}>Checkout · NET-30 · $5,810</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <WireLogo height={15}/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Wash Systems','Chemicals','Mobile Fleet','Parts'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
        <span style={{ marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)', background:'var(--wf-main)', padding:'3px 8px', borderRadius:3 }}>Cart 2</span>
      </div>
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / CHEMICALS / STEP 2 SOAPS</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#0A3560','#1E4F80','#3E70A0','#001428'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--wf-main)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #1E4F80, #0A3560 60%, #001428)', position:'relative', overflow:'hidden' }}>
            ...
            <svg viewBox="0 0 100 100" style={{ position:'absolute', bottom:'6%', left:'50%', transform:'translateX(-50%)', width:'62%', opacity:0.92 }}><path d="M24 34 L76 34 L72 88 L28 88 Z" fill="rgba(255,255,255,0.08)" stroke="rgba(255,255,255,0.6)" strokeWidth="1.7"/><path d="M30 34 Q50 12 70 34" fill="none" stroke="rgba(255,255,255,0.45)" strokeWidth="1.6"/><rect x="34" y="50" width="32" height="18" rx="1.5" fill="rgba(255,255,255,0.22)"/></svg>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>STEP 2 SOAP · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>#2 PANDA<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Polish-Safe Step 2 Soap</span></div>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--uc-brand)' }}>★★★★★</span><span style={{ color:'var(--fg-3)' }}>4.9 · 212 reviews</span></div>
          {/* swatches */}
          <div style={{ display:'flex', gap:4, marginTop:1, alignItems:'center', flexWrap:'wrap' }}><span style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginRight:2 }}>PAIRS WITH</span>{['#1 Counterpunch','#1 Uppercut'].map((c,i)=>(<span key={i} style={{ padding:'2px 7px', border:'1px solid var(--line-2)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-1)' }}>{c}</span>))}</div>
          {/* configuration — configurable product */}
          <div style={{ marginTop:1 }}>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:4 }}>SIZE</div>
            <div style={{ display:'flex', gap:4 }}>{[['5 gal pail',true],['55 gal drum',false],['275 gal tote',false]].map(([l,on],i)=>(<span key={i} style={{ flex:1, padding:'5px 2px', textAlign:'center', border:'1px solid', borderColor:on?'var(--wf-main)':'var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:on?'var(--uc-paper)':'var(--fg-2)', background:on?'var(--wf-main)':'transparent', whiteSpace:'nowrap' }}>{l}</span>))}</div>
          </div>
          {/* tier pricing — minimalist table */}
          <div style={{ marginTop:2, border:'1px solid var(--line-1)', borderRadius:3, overflow:'hidden' }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)' }}>{[['5+','$156'],['20+','$148'],['50+','$139'],['100+','$128']].map((t,i)=>(
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6, marginTop:2 }}>
            <div style={{ display:'flex', gap:6 }}>
              <div style={{ display:'flex', alignItems:'center', border:'1px solid var(--wf-main)', borderRadius:3, overflow:'hidden' }}>
              ...
              <span style={{ flex:1, padding:'8px', textAlign:'center', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:3, fontFamily:'var(--font-display)', fontSize:9.5, fontWeight:800 }}>Add to cart</span>
            </div>
            <span style={{ padding:'7px', textAlign:'center', border:'1px solid var(--wf-main)', borderRadius:3, fontFamily:'var(--font-display)', fontSize:9, fontWeight:700, color:'var(--fg-1)' }}>Request a Quote</span>
          </div>
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ Polish-safe','✓ Biodegradable','✓ Ships today'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['#2 Panda · 5 gal','$164','#0A3560'],['#1 Counterpunch','$138','#3E70A0'],['Pail Pump','$32','#001428']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$334</span></span>
              <span style={{ flex:1, padding:'6px', textAlign:'center', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:3, fontFamily:'var(--font-display)', fontSize:8.5, fontWeight:700 }}>Add all 3</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Process','Step 2 · touchless'],['Chemistry','High pH · alkaline'],['Foam profile','High'],['Polish-safe','Yes']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
        <div style={{ background:'var(--uc-cream)', padding:'11px 14px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['Strong foam lifts grease, diesel film, and road grime.','Fleet pricing and NET terms unlock at 5+ pails.','Formulated for the systems that apply it — one source.'].map((t,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'12px 1fr', gap:7, alignItems:'start' }}><span style={{ width:10, height:10, borderRadius:999, background:'var(--wf-cta)', marginTop:1, display:'flex', alignItems:'center', justifyContent:'center' }}><svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="var(--wf-main)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/></svg></span><span style={{ fontFamily:'var(--font-serif)', fontSize:9, lineHeight:1.35, color:'var(--fg-2)' }}>{t}</span></div>))}</div>
        </div>
      </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', display:'flex', gap:10, alignItems:'center', background:'var(--uc-paper)' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:9, lineHeight:1.35, color:'var(--fg-2)', marginTop:2 }}>“Cut our wash time in half — and the shine keeps clients coming back.”</div>
        </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        <div style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:8 }}>COMPLETE THE 2-STEP</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['#1 Counterpunch','$138','#3E70A0'],['HCS-400 Degreaser','$118','#001428'],['Foam Nozzle','$48','#1E4F80'],['Pail Pump','$32','#6F97BF']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxPortal() {
  const rows=[['#PO-9421','4 drums','Approved'],['#PO-9420','12 pails','Picking'],['#PO-9419','2 totes','Pending'],['#PO-9418','8 drums','Shipped'],['#PO-9417','24 pails','Delivered'],['#PO-9416','1 tote','Delivered']];
  const nav=[['◧','Dashboard',true],['▤','Orders',false],['❏','Quotes',false],['♡','Saved lists',false],['⚙','Team',false]];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'var(--wf-main)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FLEET ACCOUNT PORTAL · NET-30 TERMS ACTIVE</div>
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <WireLogo height={15}/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)' }}>LAKESHORE LOGISTICS</span>
          <span style={{ width:18, height:18, borderRadius:999, background:'linear-gradient(135deg,#1E4F80,#0A3560)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800, color:'#fff' }}>L</span>
        </span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'112px 1fr', gap:1, background:'var(--line-1)' }}>
        ...
        <div style={{ background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:3 }}>
          {nav.map((n,i)=>(<div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'5px 7px', borderRadius:3, background:n[2]?'var(--wf-main)':'transparent' }}><span style={{ fontSize:9, color:n[2]?'var(--wf-cta)':'var(--fg-3)' }}>{n[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:n[2]?'var(--uc-paper)':'var(--fg-2)' }}>{n[1]}</span></div>))}
          <div style={{ marginTop:'auto', padding:'8px 7px 0' }}><div style={{ fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)', lineHeight:1.5 }}>CREDIT LINE<br/><span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:13, letterSpacing:'-0.02em', color:'var(--fg-1)' }}>$48k</span> open</div></div>
        </div>
        ...
        <div style={{ background:'var(--uc-paper)', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ padding:'0 14px 12px' }}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'70px 1fr auto auto', gap:8, alignItems:'center', padding:'6px 0', borderTop:'1px solid var(--line-1)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color: r[2]==='Approved'?'var(--uc-paper)':'var(--fg-3)', background: r[2]==='Approved'?'var(--wf-main)':'transparent', border: r[2]==='Approved'?'none':'1px solid var(--line-1)', padding:'2px 6px', borderRadius:2 }}>{r[2]}</span>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'#FFFFFF', background:'var(--wf-cta)', padding:'2px 7px', borderRadius:2 }}>Reorder</span>
            </div>))}
          </div>
        </div>
      </div>
...
function GfxHomepage() {
  const nav = ['Chemicals','Systems','Mobile','Parts','Service'];
  const mega = [
    { h: 'By Product', items: ['Step 1 Pre-soaks','Step 2 Soaps','Friction Detergents','Degreasers & Floor','Specialty Fleet','Drums & Totes'] },
    { h: 'By Process', items: ['2-Step Touchless','1-Step Touchless','Friction (Brush)','Undercarriage'] },
    { h: 'By Fleet', items: ['Private Fleets','Bus & Transit','Waste & Municipal','Heavy Duty'] }
  ];
  const brands = ['HYDRO-TEK','LANDA','CAT PUMPS','GENERAL PUMP','HOTSY'];
  const feat = [['#2 Panda · 5 gal','$164','#0A3560','BEST'],['#1 Counterpunch · 5 gal','$138','#3E70A0',null],['#1 Uppercut · 5 gal','$112','#1E4F80','NEW'],['2-Step Gun System','$1,450','#001428','KIT']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--wf-main)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>100% EMPLOYEE-OWNED · CALEDONIA, MI · FLEET ACCOUNT PRICING</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        <WireLogo height={16}/>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{nav.map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)', position:'relative' }}>{x}{i===0 && <span style={{ position:'absolute', left:0, right:0, bottom:-9, height:2, background:'var(--wf-main)' }}/>}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ flex:'0 1 120px', minWidth:0, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search soaps & parts…</span>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)', background:'var(--wf-main)', padding:'3px 8px', borderRadius:3 }}>Cart 6</span>
        </span>
      </div>

      {/* hero + mega menu overlay */}
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #001428 0%, #0A3560 55%, #1E4F80 135%)', overflow:'hidden' }}>
        <svg viewBox="0 0 200 90" preserveAspectRatio="none" style={{ position:'absolute', right:-10, bottom:-10, width:220, opacity:0.18 }}><path d="M0 62 C30 48 60 76 100 62 S170 48 200 62" fill="none" stroke="#fff" strokeWidth="1.6"/><path d="M0 74 C30 60 60 88 100 74 S170 60 200 74" fill="none" stroke="#fff" strokeWidth="1.2"/><path d="M0 50 C30 36 60 64 100 50 S170 36 200 50" fill="none" stroke="#fff" strokeWidth="1"/></svg>
        {/* hero copy (right 1/3) */}
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'var(--wf-cta)' }}>TOUCHLESS 2-STEP CHEMISTRY</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Wash faster.<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>3 minutes per truck.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Shop soaps →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--wf-main)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--wf-cta)', whiteSpace:'nowrap' }}>FIND YOUR CHEMISTRY</span>
        {['Fleet type','Surface','Process'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Find Soaps →</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #0A3560, #001428)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>50+ YEARS</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>We build the systems and formulate the soaps — one source, 100% employee-owned.</span>
          <span style={{ alignSelf:'flex-start', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', borderBottom:'1.5px solid var(--wf-cta)', paddingBottom:1 }}>Read our story →</span>
        </div>
      </div>
    </div>
  );
...
function GfxPhone({ kind }) {
  // Mobile fleet-wash storefront. kind: 'home' | 'about' | 'blog'
  return (
    <div style={{
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px' }}>
            ...
            <span style={{ display:'inline-flex', gap:6, alignItems:'center' }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, height:22, padding:'0 9px', background:'var(--wf-main)', borderRadius:5, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-paper)' }}>⛒ Cart<span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', minWidth:11, height:11, padding:'0 2px', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:999, fontSize:6, fontWeight:800 }}>6</span></span>
            </span>
          </div>
          {/* big open search */}
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--wf-main)', borderRadius:5, padding:'0 4px 0 10px' }}>
              <span style={{ fontSize:10, color:'var(--fg-2)' }}>⌕</span>
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search soaps & parts…</span>
              <span style={{ alignSelf:'stretch', display:'flex', alignItems:'center', padding:'0 9px', margin:'3px 0', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>GO</span>
            </div>
          </div>
        </div>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--wf-main)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>100% EMPLOYEE-OWNED · FLEET PRICING</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #001428 0%, #0A3560 55%, #1E4F80 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              <svg viewBox="0 0 120 60" style={{ position:'absolute', right:-6, bottom:-6, width:120, opacity:0.2 }}><path d="M0 38 C18 28 36 48 60 38 S102 28 120 38" fill="none" stroke="#fff" strokeWidth="1.6"/><path d="M0 48 C18 38 36 58 60 48 S102 38 120 48" fill="none" stroke="#fff" strokeWidth="1.2"/></svg>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--wf-cta)' }}>TOUCHLESS 2-STEP CHEMISTRY</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Wash faster.<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>3 minutes per truck.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Shop soaps →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['HYDRO-TEK','LANDA','CAT PUMPS','HOTSY'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            <div style={{ display:'flex', gap:13, padding:'10px 12px 8px' }}>{['Best Sellers','New','Kits'].map((t,i)=>(<span key={t} style={{ paddingBottom:4, fontFamily:'var(--font-display)', fontWeight:700, fontSize:9, color:i===0?'var(--fg-1)':'var(--fg-3)', borderBottom:i===0?'2px solid var(--wf-main)':'2px solid transparent' }}>{t}</span>))}</div>
            {/* product grid */}
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['#2 Panda','$164','#0A3560','BEST'],['#1 Counterpunch','$138','#3E70A0',null],['#1 Uppercut','$112','#1E4F80','NEW'],['2-Step Gun Kit','$1,450','#001428','KIT']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
                  <div style={{ aspectRatio:'1/0.86', borderRadius:5, background:`linear-gradient(150deg, ${p[2]}, var(--uc-bone))`, position:'relative' }}>{p[3] && <span style={{ position:'absolute', top:5, left:5, padding:'1px 6px', background:p[3]==='DEAL'?'var(--uc-brand)':(p[3]==='NEW'?'var(--wf-main)':'#3F8B5D'), color:'#fff', borderRadius:2, fontFamily:'var(--font-mono)', fontSize:5, fontWeight:800 }}>{p[3]}</span>}<span style={{ position:'absolute', bottom:5, right:5, width:16, height:16, borderRadius:999, background:'var(--uc-paper)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'var(--fg-1)' }}>＋</span></div>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Systems','#0A3560'],['Soaps','#1E4F80'],['Degreasers','#3E70A0'],['Parts','#001428']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--wf-cta)' }}/>50+ YEARS · 100% EMPLOYEE-OWNED</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Cleaner fleets<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>for over 50 years.</span></span>
            </div>
            {/* overlapping image cards */}
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              <div style={{ position:'absolute', left:0, top:8, width:'58%', aspectRatio:'4/3', borderRadius:5, background:'linear-gradient(150deg, #1E4F80, var(--uc-bone))', transform:'rotate(-4deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.45)' }}/>
              <div style={{ position:'absolute', right:6, top:0, width:'52%', aspectRatio:'3/4', borderRadius:5, background:'linear-gradient(150deg, #0A3560, var(--uc-bone))', transform:'rotate(6deg)', boxShadow:'0 8px 16px -10px rgba(10,10,10,0.4)' }}/>
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>CALEDONIA, MI · HQ</span>
            </div>
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“We engineer the wash systems and formulate the soaps — together. One source, no blame games.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['50+','Years'],['100%','Employee-owned'],['3 min','Per truck']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>Fleet Wash Guides</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #1E4F80, #0A3560)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:'var(--wf-cta)', color:'#FFFFFF', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:700, letterSpacing:'0.12em', color:'var(--fg-3)' }}>CHEMISTRY · 6 MIN</span>
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>Choosing the right Step 1 pre-soak.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['Road salt & winter washing','5 min','#6F97BF'],['Dilution ratios, explained','4 min','#A9C3DC'],['Touchless vs. brush washing','7 min','#3E70A0']].map((a,i)=>(
...
function BPContent() {
  const screens = [
    { kind: 'home',  label: 'Landing Pages', d: 'Short hero, brand wall, live product tabs, and a category grid — a merchandising surface that gets fleet buyers to the full catalog fast.' },
    { kind: 'about', label: 'Brand Story', d: 'Manufacturer story and proof — employee ownership and 50 years of chemistry that win fleet trust and rank for branded search.' },
    ...
    <BPSection id="content" n="07" label="Content" paper tail="STORY · SEO · GEO · PAID">
      ...
      {(() => {
        const seo = [
          ...
          ['Title Tags', 'Carry every title tag as-is unless there&rsquo;s a proven issue.'],
          ['Meta Descriptions', 'Preserve existing descriptions to avoid CTR drops.'],
          ['Heading Structure', 'Keep correct H1–H6 hierarchy through the new build.'],
          ...
          ['Alt Text', 'Carry all image alt text over without loss.'],
          ...
          ['Search Console', 'Verify domain, properties, and settings before go-live.']
        ];
        return (
          <div style={{ marginTop: 'clamp(28px, 3.5vw, 44px)' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-3)', letterSpacing: '0.04em' }}>{seo.length}-point launch checklist</span>            </div>
...
function BPTechStack() {
  const rows = [
    ...
    { name: 'ShipperHQ',                   fn: 'Shipping rates & rules',              pri: 'launch', cost: '$350/mo' },
    { name: 'Matrixify',                   fn: 'Bulk import / export',                pri: 'start',  cost: '$50/mo' }
  ];
  const priMeta = {
  ...
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        {/* Header */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) minmax(96px,auto)', gap:16, padding:'12px 18px', background:'#0F0F0F', borderBottom:'1px solid #1F1F1F', fontFamily:'var(--font-mono)', fontSize:10, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--uc-stone-500)' }}>
        ...
        {rows.map((r, i) => {
          ...
          return (
            <div key={i} style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) minmax(96px,auto)', gap:16, padding:'14px 18px', borderBottom: i < rows.length-1 ? '1px solid #1F1F1F' : 'none', alignItems:'center' }}>
        ...
        {/* Footer total */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.2fr) minmax(0,1.5fr) minmax(0,1fr) minmax(96px,auto)', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span/>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(18px,1.8vw,24px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right', whiteSpace:'nowrap' }}>~$3,180/mo</span>
        </div>
      </div>
...
function BPMigration() {
  const steps = [
    ...
    { t: 'Go-live', s: 'Final delta sync, 301 redirect map, DNS switch with zero lost orders.' }
  ];
  const stats = ['Products', 'Collections', 'Pages', 'Blogs', 'Customers', 'Companies', 'Orders'];
  return (
    <BPSection id="migration" n="12" label="Data" paper tail="BUILD WITHOUT LOSS" vec="bgVector2">
      <BPHeadline>
        Build new{' '}
      ...
      <p style={{ marginTop: 18, maxWidth: 600, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)' }}>
        Your data comes with you — every product, customer, and order — re-mapped to the
        new build: current categories become tags, custom attributes become metafields,
        old URLs become 301s. Nothing copied blind, nothing lost.
      </p>

      {/* cart-to-cart field crosswalk */}
      {(() => {
        const source = [['Title','text'],['SKU','text'],['Price','money'],['Brand','text'],['Categories','list'],['spec_sheet','custom'],['install_guide','custom'],['Current URL','url']];
          ...
          <div style={{ marginTop:'clamp(30px, 4vw, 48px)', border:'1px solid var(--line-2)', borderRadius:10, overflow:'hidden', background:'var(--uc-paper)', boxShadow:'0 24px 60px -34px rgba(10,10,10,0.4)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:cols, alignItems:'center', borderBottom:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px' }}>
                <span style={{ width:26, height:26, borderRadius:6, background:'#1F6F3F', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-hero)', fontSize:12, fontWeight:800, color:'#FFFFFF' }}>M</span>
                <div style={{ display:'flex', flexDirection:'column', gap:1 }}>
                  ...
                  <span style={{ fontFamily:'var(--font-mono)', fontSize:9, color:'var(--fg-3)' }}>Import sheets · 12,480 records</span>
                </div>
              </div>
...
function BPDelivery() {
  const milestones = [
    ...
    { wk: 'WK 04', t: 'Blueprint sign-off', s: 'Plan, prototype, and budget locked.' },
    { wk: 'WK 10', t: 'Storefront alpha', s: 'Core theme + catalog in staging.' },
    { wk: 'WK 14', t: 'Integrations live', s: 'ERP sync + B2B flows validated.' },
    { wk: 'WK 18', t: 'Data + QA', s: 'Data imported, redirects, full QA pass.' },
    { wk: 'WK 20', t: 'Launch', s: 'Go live + 30-day support begins.' }
  ];
  return (
...
function BPRiskAssessment() {
  ...
  const risks = [
    { t: 'Data integrity', sev: 'High',
      d: 'Products, customers, and orders don\u2019t map cleanly into the new build.',
      m: 'Dry-run into staging, reconcile counts, and spot-check edge cases before go-live.' },
...
function BPInvestment() {
  ...
  const schedule = [
    { n: '01', amount: '$38,000', due: 'Due at signup' },
    { n: '02', amount: '$19,000', due: 'Due at design approval' },
    { n: '03', amount: '$19,000', due: 'Due at project completion' }
  ];
  return (
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <span aria-hidden="true" style={{ width: 16, height: 2, background: 'var(--uc-signal)' }}/>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--uc-stone-500)' }}>Integration</span>
          </div>
          <div style={{ fontFamily: 'var(--font-hero)', fontWeight: 700, fontSize: 'clamp(20px, 2vw, 28px)', letterSpacing: '-0.03em', lineHeight: 1.05, color: 'var(--uc-paper)' }}>NetSuite integration build <span style={{ fontFamily: 'var(--font-serif)', fontWeight: 400 }}>via Uncap Connect.</span></div>
          <p style={{ margin: '10px 0 0', maxWidth: 560, fontFamily: 'var(--font-serif)', fontSize: 'clamp(14px, 1.2vw, 16px)', lineHeight: 1.5, color: 'var(--uc-stone-300)', textWrap: 'pretty' }}>
            Bidirectional NetSuite ↔ Shopify sync — products, inventory, pricing, customers, and orders — built, mapped, and tested on Uncap Connect.
          </p>
        </div>
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ marginTop: 6, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(34px, 3.6vw, 52px)', letterSpacing: '-0.045em', lineHeight: 0.9, color: 'var(--uc-paper)' }}>$22k</div>
        </div>
      </div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'hydrochemsystems_approved_v1';
  ...
  const BRAND_NAME              = 'Hydro-Chem Systems';
