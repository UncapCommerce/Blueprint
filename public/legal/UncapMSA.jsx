...
(function () {
  ...
  // Default Section 11 (Attorney's Fees) text — used when no override is passed.
  const DEFAULT_ATTORNEY_FEES =
    `Service Provider shall be entitled to recover from Customer all costs and expenses, including without limitation, reasonable attorneys' fees and paralegals' fees, incurred by Service Provider in connection with Service Provider's enforcing any of its rights and remedies under this Agreement and any Statement of Work.`;
  // Default Section 23 (Choice of Forum) body — Cook County, Illinois.
  const DEFAULT_FORUM = `in any forum other than in courts having situs within Cook County, Illinois, and any local, state, or federal court located within Cook County, Illinois. Each Party irrevocably and unconditionally submits to the exclusive jurisdiction of such courts and agrees to bring any such action, litigation, or proceeding only in courts having situs in Cook County, Illinois.`;
  // Default Section 14 (Non Solicitation) body — one-directional (Customer
  // restricted). A blueprint can pass terms.nonSolicit to mutualize it.
  const DEFAULT_NONSOLICIT = `The Customer understands and acknowledges that the Service Provider has expended and continues to expend significant time and expense in recruiting and training its employees and that the loss of employees would cause significant and irreparable harm to the Service Provider. The Customer agrees and covenants not to directly or indirectly solicit, hire, or recruit for their own benefit or the benefit of any other person, or so attempt to solicit, hire, or recruit, any employee of the Service Provider ("Covered Employee"), or induce any Covered Employee to terminate their employment for 24 months immediately following the termination of this Agreement, regardless of the reason for the termination, whether voluntary or involuntary ("Restricted Period").`;
  // Default Section 18 (Assignment) inner HTML — title + body. A blueprint
  // can pass terms.assignmentBody (e.g. "Intentionally Omitted.") to replace it.
  const DEFAULT_ASSIGNMENT = `<span class="msa-ctitle u">Assignment.</span> Customer shall not assign, transfer, delegate, or subcontract any of its rights or delegate any of its obligations under this Agreement without the prior written consent of Service Provider. Any purported assignment or delegation in violation of this Section 18 shall be null and void. No assignment or delegation shall relieve the Customer of any of its obligations under this Agreement. Service Provider may assign any of its rights or delegate any of its obligations to any affiliate or to any person acquiring all or substantially all of Service Provider's assets without Customer's consent.`;

  // MSA body builder. `terms` lets a single blueprint override specific
  // clauses without forking the whole document — every field defaults to
  // the standard text, so blueprints that pass nothing are unchanged.
  //   terms.includeExhibitA  (bool, default true)  — Section 1 SOW exhibit line
  //   terms.cureDays         (string, default 'ten (10)') — Section 8.2(a)
  //   terms.ipCancellation   (bool, default false) — extra sentence on Section 6
  //   terms.attorneyFees     (string) — full replacement for Section 11
  //   terms.governingState   (string, default 'Illinois') — Section 22
  //   terms.forum            (string) — full replacement for the Section 23 venue body
  const MSA_BODY_HTML = (effective, terms) => {
    terms = terms || {};
    const exhibitA       = terms.includeExhibitA === false ? '' : ' The initial accepted Statement of Work is attached hereto as Exhibit A.';
    const cureDays       = terms.cureDays || 'ten (10)';
    const ipCancellation = terms.ipCancellation ? ' Customer retains the right to cancel this grant by notifying Service Provider by email.' : '';
    const attorneyFees   = terms.attorneyFees || DEFAULT_ATTORNEY_FEES;
    const governingState = terms.governingState || 'Illinois';
    const forum          = terms.forum || DEFAULT_FORUM;
    // Additional per-blueprint overrides. Each defaults to the standard text
    // so any blueprint that doesn't pass them renders exactly as before.
    const paymentDays    = terms.paymentDays || 'ten (10)';                                  // §4.1
    const disputedQual   = terms.disputedQual != null ? terms.disputedQual : 'successfully '; // §4.4
    const interestRate   = terms.interestRate || '1.5% per month';                            // §4.4
    const collectFees    = terms.collectFees || `attorneys' fees and costs`;                  // §4.4
    const deliverableTail = terms.deliverableTail || 'only after Customer has paid off all payments, fees, and expenses under this Agreement.'; // §4.5
    const cureCrossRef   = terms.cureCrossRef || '';                                          // §5.2(a)
    const customerMaterials = terms.customerMaterials || 'customer materials';                // §6
    const ipGrantDuration = terms.ipGrantDuration || '';                                      // §6
    const confidLabeling = terms.confidLabeling != null ? terms.confidLabeling : 'that, if disclosed in writing or other tangible form is clearly labeled as "confidential," or if disclosed orally, is identified as confidential when disclosed and within ten (10) days thereafter, is summarized in writing and confirmed as confidential '; // §7
    const advisorsList   = terms.advisorsList || 'attorneys, accountants, and financial advisors'; // §7
    const terminationRefs = terms.terminationRefs || 'Section 8.2 or Section 8.3';            // §8.1
    const cureAction     = terms.cureAction || '';                                            // §8.2(a)
    const nonpaymentDays = terms.nonpaymentDays || 'three (3)';                               // §8.3
    const liabParty1     = terms.liabParty1 || 'SERVICE PROVIDER';                            // §9.1
    const liabParty2     = terms.liabParty2 || 'CUSTOMER';                                    // §9.1
    const indemnFeesReasonable = terms.indemnFeesReasonable || '';                            // §10
    const indemnReciprocal = terms.indemnReciprocal || '';                                    // §10 extra paragraph
    const noticeMailConj = terms.noticeMailConj || 'and';                                     // §13
    const noticeCustomer = terms.noticeCustomer || 'At the address set forth for Customer in the accompanying Statement of Work.'; // notice grid, customer cell
    const copyLeftHtml   = terms.copyLeftHtml || `${COUNSEL.name}<br>E: ${COUNSEL.email}<br>P: ${COUNSEL.phone}`; // "with a copy to" left cell
    const copyRightHtml  = terms.copyRightHtml || '';                                         // "with a copy to" right cell
    const nonSolicit     = terms.nonSolicit || DEFAULT_NONSOLICIT;                            // §14
    const assignmentBody = terms.assignmentBody || DEFAULT_ASSIGNMENT;                        // §18
    return `
    <div class="msa-intro">
    ...
    <div class="msa-sub"><span class="msa-num">4.4</span><p class="msa-body">Except for invoiced payments that the Customer has ${disputedQual}disputed, all late payments shall bear interest at the lesser of (a) the rate of ${interestRate} and (b) the highest rate permissible under applicable law, calculated daily and compounded monthly. Customer shall also reimburse Service Provider for all costs incurred in collecting any late payments, including, without limitation, ${collectFees}. In addition to all other remedies available under this Agreement or at law (which Service Provider does not waive by the exercise of any rights hereunder), Service Provider shall be entitled to suspend the provision of any Services if the Customer fails to pay any invoices when due hereunder and such failure continues for ten (10) days following written notice thereof.</p></div>
    <div class="msa-sub"><span class="msa-num">4.5</span><p class="msa-body">All rights and ownership to all documents, work product, and other materials that are delivered to Customer under this Agreement or prepared by or on behalf of the Service Provider in the course of performing the Services (collectively, the "Deliverables") shall be transferred to Customer ${deliverableTail}</p></div>
    ...
    <div class="msa-sub"><span class="msa-num">8.1</span><p class="msa-body">This Agreement shall commence as of the Effective Date and shall continue thereafter until the completion of the Services under all Statements of Work, unless sooner terminated pursuant to ${terminationRefs}.</p></div>
    ...
    <div class="msa-sub"><span class="msa-alpha">(a)</span><p class="msa-body">Materially breaches this Agreement, and such breach is incapable of cure, or with respect to a breach capable of cure, the Defaulting Party ${cureAction}does not cure such breach within ${cureDays} days after receipt of written notice of such breach.</p></div>
    ...
    <div class="msa-notice-grid" style="margin-top:16px;">
      <div class="msa-nb">
        ...
        <div class="msa-ln">${copyLeftHtml}</div>
      </div>${copyRightHtml ? `
      <div class="msa-nb">
        <div class="msa-lbl">With a copy to</div>
        <div class="msa-ln">${copyRightHtml}</div>
      </div>` : ''}
    </div>

    <div class="msa-clause h"><span class="msa-num">14.</span><p class="msa-body"><span class="msa-ctitle u">Non Solicitation of Employees.</span> ${nonSolicit}</p></div>
    ...
    <div class="msa-clause h"><span class="msa-num">18.</span><p class="msa-body">${assignmentBody}</p></div>
