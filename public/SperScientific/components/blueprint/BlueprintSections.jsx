...
function brandName() { return (window.__brand && window.__brand.name) || 'Sper Scientific'; }
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 007 · PREPARED JUNE 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              A plan to upgrade {brandName()}&rsquo;s online store on Shopify — optimized experience, streamlined apps, built to grow.
            </div>
          </div>
          ...
          <div style={{
            ...
            {[
              ...
              { k: 'Client lead',  v: 'Henry Perlmutter',  s: 'CEO' },
    ...
    <BPSection id="summary" n="01" label="Executive Summary" paper tail="TL;DR">
      ...
      <div style={{
        ...
        {[
          ...
          { v: 'OS 2.0', l: 'Upgraded theme framework' },
          { v: 'Clean', l: 'Less apps and more flexibility' }
        ].map((s, i) => (
          <div key={i} style={{
            padding: 'clamp(24px, 3vw, 36px)',
...
function BPWhere() {
  const items = [
    ...
    { n: '02', t: 'Too many apps', tag: 'Systems', impact: 'High', pct: 74, d: 'A growing stack of third-party apps adding cost, complexity, and fragility with every addition.' },
  ];
  return (
...
function BPObjectives() {
  const items = [
    { n: '01', t: 'Upgrade Shopify', d: 'Migrate to the latest Shopify OS 2.0 theme framework without losing what works — while optimizing for growth.', metric: 'Zero',   unit: 'disruption',       tag: 'Migration' },
    ...
    { n: '03', t: 'Cut costs', d: 'Fewer tools, one partner. Retire the legacy stack and its renewals.',          metric: 'Lower',  unit: 'total cost',       tag: 'Finance' },
...
function BPApproach() {
  const steps = [
    ...
    { n: 'Phase 2', t: 'Build', d: 'Storefront, theme upgrade, app clean-up. Fixed scope, senior team, no surprises.', wk: '12 weeks' },
...
function BPScope() {
  const layers = [
    ...
    { code: '04', t: 'Launch & Hypercare Support',  d: 'QA, a go-live runbook, and 30-day post-launch care.', kind: 'base' }
  ];
  return (
    <BPSection id="scope" n="05" label="Scope of Work" tail="THE SYSTEM">
      ...
      <div style={{ marginTop: 'clamp(30px, 4vw, 52px)', display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 10 }}>
          {layers.slice(0, 3).map(L => <BPBuildLayer key={L.code} L={L}/>)}
        </div>
        <div>
          {layers.slice(3).map(L => <BPBuildLayer key={L.code} L={L}/>)}
        </div>
      </div>
    </BPSection>
  );
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $299 · PRECISION INSTRUMENTS</div>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{['Products','Industries','Calibration','Support'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:8.5, fontWeight:700, letterSpacing:'0.04em', color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
...
function GfxMegaNav() {
  const cols = [
    { h: 'Thermometers', items: ['Infrared','Thermocouple','RTD / PT100','Glass'] },
    { h: 'Water Quality', items: ['pH / ORP','Conductivity','Turbidity','Dissolved O₂'] },
    { h: 'Light Meters', items: ['Visible','UV','Laser / LED','Multi-purpose'] },
    { h: 'Quick Links', items: ['New Arrivals','On Sale Now','Calibration','Bulk Orders'] }
  ];
  return (
    <GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Sound Meter','$149','Type II','#2C4A7C','NEW','4.9','86'],
    ['Infrared Therm.','$79','12:1 D:S','#3A6B4E',null,'4.8','241'],
    ['pH / ORP Meter','$119','±0.01 pH','#6B4E3A',null,'4.7','178'],
    ['Light Meter','$89','Lux / fc','#4A5E2A','PRO','4.9','52'],
    ['Anemometer','$129','0–45 m/s','#2A4A6B',null,'4.6','115'],
    ['Data Logger','$229','6-ch','#3D3D3D',null,'4.8','93']
  ];
  const facets = [
    { h: 'Category', items: [['Thermometers', true],['Light Meters', true],['Water Quality', false],['Sound', false],['Weather', false]] },
    { h: 'Brand', items: [['Sper Scientific', true],['Fluke', false],['Extech', false]] },
    { h: 'Availability', items: [['In stock', true],['Ships today', false],['Calibrated', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Products','Industries','Calibration','Support'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / LIGHT METERS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Light Meters</span>
        </div>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Thermometers ✕','Light Meters ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'104px 1fr', borderTop:'1px solid var(--line-1)' }}>
        ...
        <div style={{ borderRight:'1px solid var(--line-1)', background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:11 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)' }}><span>$49</span><span>$599</span></div>
          </div>
        </div>
        ...
        <div style={{ padding:10, display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:8 }}>
          {prods.map((p,i)=>(
            <div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:3, padding:6, display:'flex', flexDirection:'column', gap:4, background:'var(--uc-paper)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:'#3F8B5D' }}>Calibrated ✓</span>
            </div>
          ))}
        </div>
      </div>
...
function GfxCart() {
  const lines=[['Infrared Therm. · 12:1','10+','$71.10','$711'],['Sound Level Meter','5+','$141.55','$708'],['pH Meter 850081','15+','$107.10','$1,607']];
  const grid=[['Infrared Therm.','$79.00','#2C4A7C','NEW'],['Sound Meter','$149.00','#3A6B4E',null],['pH Meter','$119.00','#4A5E2A','PRO'],['Light Meter','$89.00','#2A4A6B',null],['Data Logger','$229.00','#3D3D3D',null],['Anemometer','$129.00','#2E5A4A',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
          <span style={{ width:14, height:14, borderRadius:999, background:'var(--uc-black)' }}/>
          <BrandMark fontSize={11}/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Products','Industries','Calibration','Support'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'var(--fg-1)':'var(--fg-3)' }}>{x}</span>))}</div>
        ...
        <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
          <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / WATER QUALITY</span>
            <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Water Quality</span>
          </div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, color:'var(--fg-3)' }}>312 products · Sort ▾</span>
        </div>
        {/* filter chips */}
        <div style={{ display:'flex', gap:6, padding:'0 14px 11px' }}>{['Rebar','Mix','Block','In stock'].map((f,i)=>(<span key={f} style={{ padding:'3px 9px', borderRadius:999, border:'1px solid', borderColor:i===0?'var(--uc-black)':'var(--line-1)', background:i===0?'var(--uc-black)':'transparent', color:i===0?'var(--uc-paper)':'var(--fg-2)', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700 }}>{f}</span>))}</div>
        {/* product grid */}
      ...
      <div style={{
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Upgrade to StorePro <span style={{ color:'var(--uc-signal)' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>$49/mo · free freight · priority support</div></div>
...
function GfxPDP() {
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / SOUND / SOUND LEVEL METERS</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'26px 1fr', gap:7 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['#2C4A7C','#1A1A1A','#3A6B4E','#8A8A8A'].map((c,i)=>(<div key={i} style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${c}, var(--uc-bone))`, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)' }}/>))}</div>
          <div style={{ aspectRatio:'4/4.6', borderRadius:4, background:'radial-gradient(120% 90% at 65% 20%, #2C4A7C, #1A3A7A 60%, #0C1E40)', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>SPER SCIENTIFIC · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>850045<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>Sound Level Meter</span></div>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--uc-brand)' }}>★★★★★</span><span style={{ color:'var(--fg-3)' }}>4.8 · 94 reviews</span></div>
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#2C4A7C','#1A1A1A','#3A6B4E','#8A8A8A'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ NIST-traceable cal.','✓ 1-yr warranty','✓ Ships today'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['850045 Drill','$199','#C2410C'],['20V Battery 2pk','$129','#1F1F1F'],['Bit Set 40pc','$38','#8A8780']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$436</span></span>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['Sound Calibrator','$149','#2C4A7C'],['Data Logger 6ch','$229','#3D3D3D'],['Carry Case','$49','#3A6B4E'],['Tripod Mount','$35','#4A5E2A']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxPortal() {
  ...
  const nav=[['◧','Dashboard',true],['▤','Orders',false],['♡','Saved lists',false],['⚙','Team',false]];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
...
function GfxHomepage() {
  const nav = ['Shop','Brands','Deals','Fitment','Trade'];
  const mega = [
    { h: 'By Category', items: ['Thermometers','Light Meters','Water Quality','Sound Meters','Weather Stations','Lab Equipment'] },
    { h: 'By Industry', items: ['Environmental','Industrial Hygiene','Food & Beverage','Laboratory'] },
    { h: 'Quick Links', items: ['New Arrivals','On Sale Now','Calibration','Trade Pricing'] }
  ];
  const brands = ['SPER','FLUKE','EXTECH','BRK','3NH','SENTRY'];
  const feat = [['Infrared Therm.','$79.00','#2C4A7C','NEW'],['Sound Meter','$149.00','#3A6B4E',null],['pH Meter','$119.00','#4A5E2A','PRO'],['PID Controller','$122.00','#2A4A6B','SALE']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', textAlign:'center', padding:'4px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $299 · PRECISION INSTRUMENTS</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid var(--line-1)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:120, height:18, borderRadius:999, border:'1px solid var(--line-2)', display:'flex', alignItems:'center', gap:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)', whiteSpace:'nowrap', overflow:'hidden' }}><span style={{ fontSize:8 }}>⌕</span>Search 5,000+ instruments…</span>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #0A1D40 0%, #0C2A5E 55%, #1A3A7A 135%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>NEW ARRIVALS · Q2 2026</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Precision tools<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>calibrated and ready.</span></span>
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--uc-signal)', whiteSpace:'nowrap' }}>INSTRUMENT FINDER</span>
        {['Industry','Parameter','Application'].map((f,i)=>(<span key={f} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', height:22, background:'var(--uc-paper)', borderRadius:4, padding:'0 9px', fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-2)' }}>{f}<span style={{ color:'var(--fg-3)' }}>▾</span></span>))}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #6E5235, #2A2018)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EST. 1988</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>35 years of trusted instruments — calibrated, supported, and built for every field.</span>
  ...
  const groups = [
    {
      ...
      points: [
        ...
        { t: 'Product Finding', d: 'Find the right instrument fast — guided by industry, parameter, and application. Backed by smart search and a catalog structure built for how buyers actually think.' }
      ],
      gfx: <GfxHomepage/>
    },
    {
    ...
    },
    {
      ...
      points: [
        ...
        { t: 'Product Options & Linkings', d: 'Curated product linking and configured products bundled and checked out together as one seamless order.' },
    ...
    },
    {
      ...
      points: [
        { t: 'Self-Serve Ordering Portal', d: 'A logged-in account experience for reorders, saved lists, and team management.' },
...
function GfxPhone({ kind }) {
  // Mobile scientific instruments storefront. kind: 'home' | 'about' | 'blog'
  return (
    <div style={{
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>
          ...
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid var(--uc-black)', borderRadius:5, padding:'0 4px 0 10px' }}>
              ...
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--fg-3)' }}>Search 5,000+ instruments…</span>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $299 · PRECISION INSTRUMENTS</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #0A1D40 0%, #0C2A5E 55%, #1A3A7A 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--uc-signal)' }}>NEW ARRIVALS · Q2 2026</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Precision tools<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>calibrated and ready.</span></span>
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Infrared Therm.','$79.00','#2C4A7C','NEW'],['Sound Meter','$149.00','#3A6B4E',null],['pH Meter','$119.00','#4A5E2A','PRO'],['PID Controller','$122.00','#2A4A6B','SALE']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Thermometers','#2C4A7C'],['Light Meters','#3A6B4E'],['Water Quality','#4A5E2A'],['Sound','#2A4A6B']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'var(--uc-signal)' }}/>EST. 1988 · FAMILY-OWNED</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Trusted for<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>35 years of precision.</span></span>
            </div>
            ...
            {/* manifesto */}
            <div style={{ padding:'4px 13px 8px' }}><span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:10.5, lineHeight:1.45, color:'var(--fg-2)' }}>“35 years of precision instruments \u2014 same promise: calibrated, reliable, and ready to ship.”</span></div>
            {/* stat strip */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, padding:'8px 13px', borderTop:'1px solid var(--line-1)', background:'var(--uc-paper)' }}>{[['5k+','Instruments'],['35yr','In business'],['4.9★','Rated']].map((s,i)=>(<div key={i}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:14, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:5.5, color:'var(--fg-3)', letterSpacing:'0.08em', textTransform:'uppercase' }}>{s[1]}</div></div>))}</div>
          </div>
        )}
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'12px 13px 8px' }}><span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.04em', color:'var(--fg-1)' }}>Tech Notes</span></div>
            {/* featured story */}
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              ...
              <div style={{ padding:'9px 11px', background:'var(--uc-paper)', display:'flex', flexDirection:'column', gap:4 }}>
                ...
                <span style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:12.5, letterSpacing:'-0.018em', lineHeight:1.08, color:'var(--fg-1)' }}>How to choose the right thermometer.</span>
              </div>
            </div>
            {/* article list */}
            <div style={{ padding:'10px 13px', display:'flex', flexDirection:'column', gap:9 }}>
              {[['pH calibration explained','5 min','#2C4A7C'],['Sound meter accuracy guide','7 min','#3A6B4E'],['Infrared vs. contact temp.','6 min','#4A5E2A']].map((a,i)=>(
...
function BPContent() {
  const screens = [
    { kind: 'home',  label: 'Landing Pages', d: 'Short hero, brand wall, live product tabs, and a department grid — a clean surface that gets buyers to the right instrument fast.' },
...
function BPTechStack() {
  const rows = [
    { name: 'Shopify Plus',                    fn: 'Commerce platform + storefront',          pri: 'start' },
    { name: 'Shopify Search & Discovery',      fn: 'Native search, filters & merchandising',  pri: 'start' },
    { name: 'Shopify Checkout Blocks',         fn: 'Custom checkout experience',              pri: 'start' },
    { name: 'Shopify Flow',                    fn: 'Workflow automation',                     pri: 'start' },
    { name: 'Shopify Forms',                   fn: 'Lead capture & customer data',            pri: 'start' },
    { name: 'EPA – Easy Product Addons',        fn: 'Product add-ons and upsell options',       pri: 'start' },
    { name: 'Easy Tabs – Product Tabs',        fn: 'Tabbed product page content & specs',     pri: 'start' },
    { name: 'Rebuy Personalization Engine',    fn: 'AI-powered recommendations & upsells',    pri: 'start' },
    { name: 'Shopper Approved Reviews',        fn: 'Verified reviews & ratings platform',     pri: 'start' },
  ];
  const priMeta = {
  ...
  return (
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <p style={{ marginTop:'clamp(16px, 2vw, 22px)', maxWidth:760, fontFamily:'var(--font-serif)', fontSize:'clamp(15px, 1.3vw, 18px)', lineHeight:1.55, color:'var(--uc-stone-300)' }}>
        A lean, native Shopify Plus stack — purpose-built for this upgrade. Every tool below is part of the Shopify platform, keeping the app count low and the system clean.
      </p>
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:18, flexWrap:'wrap', gap:8 }}>
          ...
          <span style={{ fontFamily:'var(--font-mono)', fontSize:10, color:'var(--uc-stone-500)', letterSpacing:'0.04em' }}>All native Shopify · no third-party dependencies</span>
        </div>

        {(() => {
          const layers = [
            { tier:'01', name:'Commerce Core', note:'Foundation', tools:[['Shopify Plus','start'],['Search & Discovery','start'],['Checkout Blocks','start'],['Flow','start'],['Forms','start'],['Easy Product Addons','start'],['Easy Tabs','start'],['Rebuy','start'],['Shopper Approved','start']], core:true }
          ];
...
function BPMigration() {
  ...
  return (
    <BPSection id="migration" n="12" label="Migration" paper tail="MOVE WITHOUT LOSS" vec="bgVector2">
      ...
      {/* guarantee strip */}
      <div style={{ marginTop: 'clamp(28px, 3vw, 40px)', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0, borderTop: '1px solid var(--line-1)', borderBottom: '1px solid var(--line-1)' }}>
        {stats.map((s, i) => (
...
function BPDelivery() {
  const milestones = [
    ...
    { wk: 'WK 10', t: 'QA & testing', s: 'Full QA pass, redirects, pre-launch checks.' },
...
function BPTeam() {
  ...
  return (
    <BPSection id="team" n="12" label="Team" tail="WHO DOES THE WORK">
      <BPHeadline>
        Senior from{' '}
...
function BPInvestment() {
  ...
  const schedule = [
    { n: '01', amount: '$19,000', due: 'Due at signup · ½ of total' },
    { n: '02', amount: '$9,500',  due: 'Due at design approval · ¼ of total' },
    { n: '03', amount: '$9,500',  due: 'Due at project completion · ¼ of total' }
  ];
  return (
    <BPSection id="investment" n="10" label="Investment" dark tail="FIXED · NO SURPRISES">
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ marginTop: 6, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(34px, 3.6vw, 52px)', letterSpacing: '-0.045em', lineHeight: 0.9, color: 'var(--uc-paper)' }}>$5k</div>
        </div>
      </div>
    </BPSection>
  );
...
function BPGrowth() {
  ...
  return (
    <BPSection id="growth" n="11" label="Growth" paper tail="AFTER LAUNCH">
      <BPHeadline>
        Launch is the start.{' '}
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 15, color: 'var(--fg-1)' }}>
          Uncap Growth · Core Package · <span style={{ fontFamily: 'var(--font-hero)', fontWeight: 800 }}>$2,500 / mo</span>
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-3)', letterSpacing: '0.06em' }}>
          12 MONTH MANAGED SERVICE RETAINER
        </div>
      </div>
    </BPSection>
  );
...
function BPWhy() {
  ...
  return (
    <BPSection id="why" n="13" label="Uncap" dark tail="THE CASE">
      <BPHeadline dark>
        Uncap Commerce.{' '}
...
function BPProof() {
  ...
  const initials = (n) => n.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase();
  return (
    <BPSection id="proof" n="14" label="Proof" paper tail="ON THE RECORD">
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', gap: 'clamp(28px, 5vw, 80px)', alignItems: 'end' }}>
        <BPHeadline>
          Don&rsquo;t take{' '}
...
function BPApproveButton() {
  const APPROVED_KEY = 'sperscientific_approved_v1';
  ...
  const BRAND_NAME              = 'Sper Scientific';
...
function BPNav() {
  const items = [
    ...
    { id: 'delivery',    n: '09', l: 'Delivery' },
    { id: 'investment',  n: '10', l: 'Investment' },
    { id: 'growth',      n: '11', l: 'Growth' },
    { id: 'team',        n: '12', l: 'Team' },
    { id: 'why',         n: '13', l: 'Uncap' },
    { id: 'proof',       n: '14', l: 'Proof' },
    { id: 'next',        n: '15', l: 'Next' }
  ];
