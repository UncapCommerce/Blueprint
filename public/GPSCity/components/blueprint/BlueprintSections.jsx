...
function brandName() { return (window.__brand && window.__brand.name) || 'GPS City'; }
    ...
    <section id="intro" data-bp-section="intro" style={{
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <span>BLUEPRINT 008 · PREPARED JUNE 2026 · CONFIDENTIAL</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              A plan to replatform {brandName()}&rsquo;s commerce operations to Shopify, consolidate the tech stack, and build an exceptional experience for growth.
            </div>
          </div>
          ...
          <div style={{
            ...
            {[
              ...
              { k: 'Client lead',  v: 'Jordan Timinski',  s: 'Vice President of Operations' },
              { k: 'Company', v: 'GPS City', s: '6847 S Eastern Ave · Las Vegas, NV 89119' },
    ...
    <BPSection id="summary" n="01" label="Executive Summary" paper tail="TL;DR">
      ...
      <div style={{
        ...
        {[
          ...
          { v: 'Unified', l: 'Unified and integrated systems on Shopify' },
...
function BPWhere() {
  const items = [
    { n: '01', t: 'Aging storefront', tag: 'Storefront', impact: 'High', pct: 82, d: 'Custom-built, patched for years, difficult to maintain and costly support.' },
    { n: '02', t: 'Outdated experience', tag: 'Experience', impact: 'Critical', pct: 94, d: 'Unfriendly shopping experience and no mobile optimization.' },
    { n: '03', t: 'No optionality', tag: 'Platform', impact: 'Medium', pct: 64, d: 'Lack of ability to integrate and plug and play with modern advanced commerce tools and solutions.' },
...
function BPApproach() {
  const steps = [
    ...
    { n: 'Phase 2', t: 'Build', d: 'Storefront, B2B, ERP sync. Fixed scope, senior team, no surprises.', wk: '20 weeks' },
...
function BPScope() {
  const layers = [
    ...
    { code: '04', t: 'Unify Systems',      d: 'DTC, B2B, Retail, Omni-Channel all in one platform.' },
    { code: '05', t: 'ERP Integration Assistance', d: 'Bidirectional, real-time sync native to Shopify Plus.' },
...
function BPGantt() {
  const groups = [
    { name: 'Foundation', range: 'WK 01–07', tasks: [
      ...
      { n: '03', t: 'Experience Design',  tag: 'UX/UI Design · Web · Mobile',   s: 2, e: 6,  range: 'W2–6' },
    ...
    { name: 'Production', range: 'WK 07–17', tasks: [
      ...
      { n: '06', t: 'Data Migration',             tag: 'Migrate',  s: 7,  e: 15, range: 'W7–15' },
      { n: '07', t: 'Integration',                tag: 'Connect',  s: 11, e: 16, range: 'W11–16' },
      { n: '08', t: 'QA & Testing',               tag: 'Verify',   s: 15, e: 17, range: 'W15–17' }
    ] },
    { name: 'Launch', range: 'WK 17–20', tasks: [
      { n: '09', t: 'Client UAT & Go-live',       tag: 'Launch',   s: 17, e: 20, range: 'W17–20' }
    ] }
  ];
  return (
...
function GfxChrome({ children }) {
  return (
    <div style={{ background: 'var(--uc-paper)', border: '1px solid var(--line-1)', borderRadius: 5, overflow: 'hidden' }}>
      <div style={{ background:'#2B2B2B', color:'rgba(255,255,255,0.82)', textAlign:'center', padding:'5px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE FREIGHT OVER $999 · CONTRACTOR PRICING</div>
...
function GfxBrand() {
  return <img src="uploads/logo-gpscity-gpsusa.png" style={{ height:13, display:'block' }} alt="GPS City"/>;
}
function GfxBrandDark() {
  return <img src="uploads/logo-gpscity-gpsusa.png" style={{ height:13, display:'block' }} alt="GPS City"/>;
}

function GfxMegaNav() {
  const cols = [
    { h: 'Outdoor & Fitness', items: ['Hiking & Trekking','Trail Running','Cycling','Golf'] },
    { h: 'Automotive', items: ['Car GPS','Truck & RV','Motorcycle','OBD Trackers'] },
    { h: 'Marine', items: ['Chartplotters','Fish Finders','VHF Radios','Transducers'] },
    { h: 'Featured', items: ['New Arrivals','Garmin Summer Sale','Fleet Pricing','Certified Refurb'] }
  ];
  return (
    <GfxChrome>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:1, background:'var(--line-1)' }}>
        {cols.map((c,ci)=>(
          <div key={ci} style={{ background:'var(--uc-paper)', padding:'12px 13px 16px', display:'flex', flexDirection:'column', gap:7 }}>
            ...
            {c.items.map((it,ri)=>(<span key={ri} style={{ fontFamily:'var(--font-display)', fontWeight:ci===3&&ri===0?700:500, fontSize:10.5, letterSpacing:'-0.01em', color: ci===0&&ri===0?'var(--uc-black)':'var(--fg-2)', display:'flex', alignItems:'center', gap:5 }}>{ci===0&&ri===0 && <span style={{ width:14, height:3, background:'#FDCA2F' }}/>}{it}</span>))}
          </div>
        ))}
      </div>
    </GfxChrome>
...
function GfxCollection() {
  const prods = [
    ['Garmin fenix 8 Pro','$1,449','/51mm','#1B3A5C','SALE','4.9','512'],
    ['RAM Tough-Tray III','$112','/laptop','#7A6B54','NEW','4.8','204'],
    ['Garmin Striker Vivid 7sv','$519','/w transducer','#1E3D5C',null,'4.9','318'],
    ['Garmin dezl OTR720','$449','/truck GPS','#2B2B2B',null,'4.7','189'],
    ['Garmin Instinct Solar','$239','/tactical','#3F6B3A','DEAL','4.9','621'],
    ['Garmin GLO 2','$119','/receiver','#3A3A3A',null,'4.6','143']
  ];
  const facets = [
    { h: 'Category', items: [['Outdoor & Fitness', true],['Automotive', true],['Marine', false],['Mounts', false],['Maps', false]] },
    { h: 'Brand', items: [['Garmin', true],['RAM Mounts', false],['Lowrance', false]] },
    { h: 'Availability', items: [['In stock', true],['Ships today', false],['Will-call', false]] }
  ];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'#2B2B2B', color:'rgba(255,255,255,0.82)', textAlign:'center', padding:'5px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $25 · AUTHORIZED GARMIN DEALER</div>
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.08)', background:'var(--uc-black)' }}>
        <GfxBrand/>
        <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Outdoor','Automotive','Marine','Mounts'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===1?'#fff':'rgba(255,255,255,0.45)' }}>{x}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:160, height:24, borderRadius:999, border:'1px solid rgba(255,255,255,0.3)', display:'flex', alignItems:'center', gap:5, padding:'0 11px', fontFamily:'var(--font-mono)', fontSize:8, color:'rgba(255,255,255,0.65)', whiteSpace:'nowrap', overflow:'hidden', background:'rgba(255,255,255,0.06)' }}><span style={{ fontSize:9 }}>⌕</span>Search Garmin, RAM Mounts…</span>
          <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-black)', background:'#FDCA2F', padding:'3px 8px', borderRadius:3 }}><svg width="10" height="9" viewBox="0 0 20 18" fill="currentColor" style={{flexShrink:0}}><path d="M1 1h3.5l2.5 9h9l2-7H6.5" strokeWidth="0"/><circle cx="9" cy="16" r="1.8"/><circle cx="15" cy="16" r="1.8"/></svg> 0</span>
        </span>
      </div>
      ...
      <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:10 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.1em', color:'var(--fg-3)' }}>HOME / OUTDOOR & FITNESS</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:19, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Outdoor & Fitness</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--fg-3)' }}>3,840 results</span>
      ...
      <div style={{ padding:'0 14px 10px', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
        {['Outdoor ✕','Automotive ✕','In stock ✕'].map((c,i)=>(<span key={i} style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', background:'var(--uc-bone)', border:'1px solid var(--line-2)', borderRadius:999, padding:'2px 8px' }}>{c}</span>))}
...
function GfxCart() {
  const lines=[['Garmin fenix 8 Pro · 51mm','2+','$1,349','$2,698'],['RAM Tough-Tray III Laptop Mount','5+','$107','$535'],['Garmin Striker Vivid 7sv','3+','$494','$1,482']];
  const grid=[['Garmin fenix 8','$1,449','#1B3A5C','SALE'],['RAM Tough-Tray III','$112.99','#7A6B54','NEW'],['Striker Vivid 7sv','$519.99','#1E3D5C',null],['Garmin dezl OTR720','$449','#2B2B2B',null],['Instinct Solar','$239.99','#3F6B3A','DEAL'],['Garmin GLO 2','$119.99','#3A3A3A',null]];
  return (
    <div style={{ position:'relative', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.08)', background:'var(--uc-black)' }}>
          <GfxBrand/>
          <div style={{ display:'flex', gap:9, marginLeft:4 }}>{['Outdoor','Automotive','Marine','Mounts'].map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'#fff':'rgba(255,255,255,0.45)' }}>{x}</span>))}</div>
          <span style={{ marginLeft:'auto', display:'inline-flex', alignItems:'center', gap:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-black)', background:'#FDCA2F', padding:'3px 8px', borderRadius:3 }}><svg width="10" height="9" viewBox="0 0 20 18" fill="currentColor" style={{flexShrink:0}}><path d="M1 1h3.5l2.5 9h9l2-7H6.5" strokeWidth="0"/><circle cx="9" cy="16" r="1.8"/><circle cx="15" cy="16" r="1.8"/></svg> 3</span>
        </div>
      ...
      <div style={{
        ...
        <div style={{ padding:'10px 13px', background:'var(--uc-black)', color:'var(--uc-paper)', borderBottom:'1px solid var(--line-1)', display:'flex', gap:9, alignItems:'center' }}>
          <span style={{ width:18, height:18, borderRadius:999, border:'2px solid #FDCA2F', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}><span style={{ width:7, height:7, borderRadius:999, background:'#FDCA2F' }}/></span>
          <div style={{ flex:1 }}><div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:9.5, color:'var(--uc-paper)' }}>Upgrade to StorePro <span style={{ color:'#FDCA2F' }}>−15%</span></div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, color:'var(--uc-stone-300)' }}>$49/mo · free freight · priority support</div></div>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'var(--uc-black)', background:'#FDCA2F', padding:'4px 8px', borderRadius:3 }}>ADD</span>
        </div>
        {/* totals + checkout */}
  ...
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.08)', background:'var(--uc-black)' }}>
        ...
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:160, height:24, borderRadius:999, border:'1px solid rgba(255,255,255,0.3)', display:'flex', alignItems:'center', gap:5, padding:'0 11px', fontFamily:'var(--font-mono)', fontSize:8, color:'rgba(255,255,255,0.65)', whiteSpace:'nowrap', overflow:'hidden', background:'rgba(255,255,255,0.06)' }}><span style={{ fontSize:9 }}>⌕</span>Search Garmin, RAM Mounts…</span>
          <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-black)', background:'#FDCA2F', padding:'3px 8px', borderRadius:3 }}><svg width="10" height="9" viewBox="0 0 20 18" fill="currentColor" style={{flexShrink:0}}><path d="M1 1h3.5l2.5 9h9l2-7H6.5" strokeWidth="0"/><circle cx="9" cy="16" r="1.8"/><circle cx="15" cy="16" r="1.8"/></svg> 2</span>
        </span>
      </div>
      {/* breadcrumb */}
      <div style={{ padding:'8px 14px 0', fontFamily:'var(--font-mono)', fontSize:7, letterSpacing:'0.08em', color:'var(--fg-3)' }}>HOME / OUTDOOR / SMARTWATCHES</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'9px 14px 13px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
          <div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em', color:'#3F8B5D' }}>GARMIN · IN STOCK · SHIPS TODAY</div>
          <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.04em', lineHeight:0.9, color:'var(--fg-1)' }}>FENIX 8 PRO<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400, fontSize:16 }}>MicroLED Smartwatch</span></div>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--uc-brand)' }}>★★★★★</span><span style={{ color:'var(--fg-3)' }}>4.9 · 512 reviews</span></div>
          {/* swatches */}
          <div style={{ display:'flex', gap:5, marginTop:1 }}>{['#1B3A5C','#C8A14E','#1F1F1F','#8A8780'].map((c,i)=>(<span key={i} style={{ width:16, height:16, borderRadius:999, background:c, border:i===0?'1.5px solid var(--uc-black)':'1px solid var(--line-1)', boxShadow:i===0?'0 0 0 2px var(--uc-paper) inset':'none' }}/>))}</div>
          {/* edition */}
          <div style={{ marginTop:1 }}>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, letterSpacing:'0.1em', color:'var(--fg-3)', marginBottom:4 }}>EDITION</div>
            <div style={{ display:'flex', gap:4 }}>{[['Standard',false],['Solar+',true],['Sapphire',false]].map(([l,on],i)=>(<span key={i} style={{ flex:1, padding:'5px 2px', textAlign:'center', border:'1px solid', borderColor:on?'var(--uc-black)':'var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, color:on?'var(--uc-paper)':'var(--fg-2)', background:on?'var(--uc-black)':'transparent', whiteSpace:'nowrap' }}>{l}</span>))}</div>
          </div>
          {/* tier pricing — minimalist table */}
          {/* Protection plans */}
          <div style={{ marginTop:2, border:'1px solid var(--line-1)', borderRadius:3, overflow:'hidden' }}>
            <div style={{ padding:'5px 9px 4px', background:'var(--uc-bone)', borderBottom:'1px solid var(--line-1)', fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, letterSpacing:'0.08em', color:'var(--fg-3)' }}>PROTECTION PLAN</div>
            {[['2 Year Protection Plan','$9.99',false],['3 Year Protection Plan','$13.99',true]].map(([l,p,on],i)=>(
              <div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'6px 9px', borderTop:i?'1px solid var(--line-1)':'none' }}>
                <span style={{ width:10, height:10, borderRadius:2, flexShrink:0, background:on?'var(--uc-black)':'var(--uc-paper)', border:'1px solid var(--line-2)', display:'flex', alignItems:'center', justifyContent:'center' }}>{on && <svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="var(--uc-paper)" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg>}</span>
                <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:7.5, color:on?'var(--fg-1)':'var(--fg-2)', fontWeight:on?700:400 }}>{l}</span>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p}</span>
              </div>
            ))}
          </div>
          {/* CTAs */}
          <div style={{ display:'flex', flexDirection:'column', gap:6, marginTop:2 }}>
            <div style={{ display:'flex', gap:6 }}>
              ...
              <span style={{ flex:1, padding:'8px', textAlign:'center', background:'#FDCA2F', color:'var(--uc-black)', borderRadius:3, fontFamily:'var(--font-display)', fontSize:9.5, fontWeight:800 }}>Add to cart</span>
            </div>
          ...
          {/* trust row */}
          <div style={{ display:'flex', gap:8, marginTop:3, flexWrap:'wrap' }}>{['✓ 1-yr Garmin warranty','✓ Free returns','✓ Ships today'].map(t=>(<span key={t} style={{ fontFamily:'var(--font-mono)', fontSize:6.5, fontWeight:700, color:'var(--fg-3)' }}>{t}</span>))}</div>
          ...
          <div style={{ marginTop:7, paddingTop:9, borderTop:'1px solid var(--line-1)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
              {[['fenix 8 Pro','$1,449','#1B3A5C'],['HRM-Pro+ Monitor','$129','#1F1F1F'],['Nylon Band 26mm','$34','#8A8780']].map((p,i)=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-3)', whiteSpace:'nowrap' }}>Bundle <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:12, color:'var(--fg-1)' }}>$1,612</span></span>
              <span style={{ flex:1, padding:'6px', textAlign:'center', background:'#FDCA2F', color:'var(--uc-black)', borderRadius:3, fontFamily:'var(--font-display)', fontSize:8.5, fontWeight:700 }}>Add all 3</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--line-1)', borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'var(--uc-paper)', padding:'11px 14px' }}>
          ...
          {[['Display','MicroLED 1.4"'],['Battery','Up to 16 days'],['Weight','63 g'],['GPS','Multi-band']].map((r,i)=>(<div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderTop:i?'1px solid var(--line-1)':'none', fontFamily:'var(--font-mono)', fontSize:8 }}><span style={{ color:'var(--fg-3)' }}>{r[0]}</span><span style={{ color:'var(--fg-1)', fontWeight:700 }}>{r[1]}</span></div>))}
        </div>
        <div style={{ background:'var(--uc-cream)', padding:'11px 14px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{['Multi-band GPS — survey-grade accuracy on trail and road.','Fleet pricing and dealer terms unlock at 5+ units.','Same-day ship from Las Vegas, NV on all in-stock items.'].map((t,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'12px 1fr', gap:7, alignItems:'start' }}><span style={{ width:10, height:10, borderRadius:999, background:'#FDCA2F', marginTop:1, display:'flex', alignItems:'center', justifyContent:'center' }}><svg width="6" height="6" viewBox="0 0 12 12" fill="none"><path d="M2 6 L5 9 L10 3" stroke="var(--uc-black)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/></svg></span><span style={{ fontFamily:'var(--font-serif)', fontSize:9, lineHeight:1.35, color:'var(--fg-2)' }}>{t}</span></div>))}</div>
        </div>
      </div>
      ...
      <div style={{ padding:'11px 14px', borderTop:'1px solid var(--line-1)', background:'var(--uc-bone)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:7 }}>{[['HRM-Pro+ Monitor','$129','#1F1F1F'],['Nylon Band 26mm','$34','#8A8780'],['Carrying Case','$54','#3A3632'],['Charging Cable','$29','#1B3A5C']].map((p,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}><div style={{ aspectRatio:'1/1', borderRadius:3, background:`linear-gradient(140deg, ${p[2]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:7.5, color:'var(--fg-1)' }}>{p[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--fg-1)' }}>{p[1]}</span></div>))}</div>
      </div>
    </div>
  );
}
...
function GfxPortal() {
  const rows=[['#PO-7841','12 Units','Approved'],['#PO-7840','4 Units','Picking'],['#PO-7839','84 Units','Pending'],['#PO-7838','28 Units','Shipped'],['#PO-7837','6 Units','Delivered'],['#PO-7836','52 Units','Delivered']];
  const nav=[['◧','Dashboard',true],['▤','Orders',false],['❏','Quotes',false],['♡','Saved lists',false],['⚙','Team',false]];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* chrome */}
      <div style={{ background:'#2B2B2B', color:'rgba(255,255,255,0.82)', textAlign:'center', padding:'5px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FLEET DEALER PORTAL · NET-30 TERMS ACTIVE</div>
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.08)', background:'var(--uc-black)' }}>
        <GfxBrand/>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'rgba(255,255,255,0.55)' }}>WESTLAKE FLEET SERVICES</span>
          <span style={{ width:18, height:18, borderRadius:999, background:'linear-gradient(135deg,#5C8FA0,#1B3A5C)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800, color:'#fff' }}>M</span>
        </span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'112px 1fr', gap:1, background:'var(--line-1)' }}>
        ...
        <div style={{ background:'var(--uc-bone)', padding:'11px 10px', display:'flex', flexDirection:'column', gap:3 }}>
          {nav.map((n,i)=>(<div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'5px 7px', borderRadius:3, background:n[2]?'var(--uc-black)':'transparent' }}><span style={{ fontSize:9, color:n[2]?'#FDCA2F':'var(--fg-3)' }}>{n[0]}</span><span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:n[2]?'var(--uc-paper)':'var(--fg-2)' }}>{n[1]}</span></div>))}
          <div style={{ marginTop:'auto', padding:'8px 7px 0' }}><div style={{ fontFamily:'var(--font-mono)', fontSize:6.5, color:'var(--fg-3)', lineHeight:1.5 }}>CREDIT LINE<br/><span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:13, letterSpacing:'-0.02em', color:'var(--fg-1)' }}>$84k</span> open</div></div>
        </div>
        ...
        <div style={{ background:'var(--uc-paper)', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ padding:'11px 14px 9px', display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
            <div>
              ...
              <div style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:17, letterSpacing:'-0.035em', color:'var(--fg-1)' }}>Alex</div>
            </div>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, color:'var(--uc-black)', background:'#FDCA2F', padding:'4px 9px', borderRadius:3 }}>+ New order</span>
          </div>
          {/* stat tiles */}
          <div style={{ padding:'0 14px 10px', display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8 }}>
            {[['22','OPEN ORDERS'],['6','QUOTES'],['$318k','YTD SPEND']].map((s,i)=>(<div key={i} style={{ border:'1px solid var(--line-1)', borderRadius:4, padding:'8px 10px' }}><div style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.03em', color:'var(--fg-1)' }}>{s[0]}</div><div style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.08em', color:'var(--fg-3)', marginTop:2 }}>{s[1]}</div></div>))}
          </div>
          ...
          <div style={{ padding:'0 14px 12px' }}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{ display:'grid', gridTemplateColumns:'70px 1fr auto auto', gap:8, alignItems:'center', padding:'6px 0', borderTop:'1px solid var(--line-1)' }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:8, fontWeight:700, color:'var(--uc-black)', background:'#FDCA2F', padding:'2px 7px', borderRadius:2 }}>Reorder</span>
            </div>))}
          </div>
        </div>
      </div>
...
function GfxHomepage() {
  const nav = ['Shop','Brands','Deals','Fitment','Fleet'];
  const mega = [
    { h: 'By Category', items: ['Smartwatches','Outdoor GPS','Automotive','Marine','Mounts','Accessories'] },
    { h: 'By Brand', items: ['Garmin','RAM Mounts','Lowrance','TomTom'] },
    { h: 'Quick Links', items: ['New Arrivals','Clearance','Fleet Pricing','Certified Refurb'] }
  ];
  const brands = ['GARMIN','LOWRANCE','RAM','TOMTOM','MAGELLAN','BUSHNELL'];
  const feat = [['Garmin fenix 8 Pro','$1,449','#1B3A5C','SALE'],['RAM Tough-Tray III','$112.99','#7A6B54','NEW'],['Garmin Striker 7sv','$519.99','#1E3D5C',null],['Garmin dezl OTR720','$449','#2B2B2B','DEAL']];
  return (
    <div style={{ background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:5, overflow:'hidden' }}>
      {/* announcement */}
      <div style={{ background:'#2B2B2B', color:'rgba(255,255,255,0.82)', textAlign:'center', padding:'5px 0', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $25 · AUTHORIZED GARMIN DEALER</div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:11, padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.08)', background:'var(--uc-black)' }}>
        <GfxBrand/>
        <div style={{ display:'flex', gap:11, marginLeft:6 }}>{nav.map((x,i)=>(<span key={x} style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:i===0?'#fff':'rgba(255,255,255,0.45)', position:'relative' }}>{x}{i===0 && <span style={{ position:'absolute', left:0, right:0, bottom:-9, height:2, background:'#FDCA2F' }}/>}</span>))}</div>
        <span style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:7 }}>
          <span style={{ width:160, height:24, borderRadius:999, border:'1px solid rgba(255,255,255,0.3)', display:'flex', alignItems:'center', gap:5, padding:'0 11px', fontFamily:'var(--font-mono)', fontSize:8, color:'rgba(255,255,255,0.65)', whiteSpace:'nowrap', overflow:'hidden', background:'rgba(255,255,255,0.06)' }}><span style={{ fontSize:9 }}>⌕</span>Search Garmin, RAM Mounts & more…</span>
          <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-black)', background:'#FDCA2F', padding:'3px 8px', borderRadius:3 }}><svg width="10" height="9" viewBox="0 0 20 18" fill="currentColor" style={{flexShrink:0}}><path d="M1 1h3.5l2.5 9h9l2-7H6.5" strokeWidth="0"/><circle cx="9" cy="16" r="1.8"/><circle cx="15" cy="16" r="1.8"/></svg> 6</span>
        </span>
      </div>
      ...
      <div style={{ position:'relative', height:188, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #7A3B16 135%)', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:18, top:0, bottom:0, width:'34%', display:'flex', flexDirection:'column', justifyContent:'center', gap:7 }}>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, letterSpacing:'0.16em', color:'#FDCA2F' }}>SUMMER SALE · ENDS FRIDAY</span>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:24, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Navigate anywhere.<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>On sale.</span></span>
          <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 13px', background:'#FDCA2F', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800 }}>Shop GPS deals →</span>
        </div>
        {/* mega menu over 2/3 */}
      ...
      <div style={{ background:'var(--uc-black)', padding:'10px 14px', display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, letterSpacing:'0.08em', color:'#FDCA2F', whiteSpace:'nowrap' }}>FIND YOUR FIT</span>
        ...
        <span style={{ display:'inline-flex', alignItems:'center', height:22, padding:'0 12px', background:'#FDCA2F', color:'var(--uc-black)', borderRadius:4, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:800, whiteSpace:'nowrap' }}>Find Parts →</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, borderTop:'1px solid var(--line-1)' }}>
        <div style={{ background:'linear-gradient(150deg, #6E5235, #2A2018)', minHeight:96, position:'relative' }}><span style={{ position:'absolute', bottom:8, left:10, padding:'2px 7px', background:'var(--uc-paper)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)' }}>EST. 1996</span></div>
        <div style={{ padding:'16px 16px', display:'flex', flexDirection:'column', justifyContent:'center', gap:7, background:'var(--uc-bone)' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:14, lineHeight:1.3, letterSpacing:'-0.01em', color:'var(--fg-1)' }}>From a single storefront in Las Vegas to a global GPS authority — same obsession with navigation, since 1996.</span>
          <span style={{ alignSelf:'flex-start', fontFamily:'var(--font-mono)', fontSize:7, fontWeight:700, color:'var(--fg-1)', borderBottom:'1.5px solid #FDCA2F', paddingBottom:1 }}>Read our story →</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ background:'var(--uc-cream)', borderRadius:12, overflow:'hidden', height:460, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, borderBottom:'1px solid rgba(255,255,255,0.08)', background:'var(--uc-black)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px' }}>
            <span style={{ display:'inline-flex', alignItems:'center', gap:8 }}>
              <span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:22, height:22, border:'1px solid rgba(255,255,255,0.2)', borderRadius:5, fontSize:9, color:'rgba(255,255,255,0.7)' }}>☰</span>
              <GfxBrand/>
            </span>
            <span style={{ display:'inline-flex', gap:6, alignItems:'center' }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, height:22, padding:'0 9px', background:'#FDCA2F', borderRadius:5, fontFamily:'var(--font-mono)', fontSize:7.5, fontWeight:700, color:'var(--uc-black)' }}><svg width="10" height="9" viewBox="0 0 20 18" fill="currentColor" style={{flexShrink:0}}><path d="M1 1h3.5l2.5 9h9l2-7H6.5" strokeWidth="0"/><circle cx="9" cy="16" r="1.8"/><circle cx="15" cy="16" r="1.8"/></svg> Cart<span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', minWidth:11, height:11, padding:'0 2px', background:'var(--uc-black)', color:'#FDCA2F', borderRadius:999, fontSize:6, fontWeight:800 }}>6</span></span>
            </span>
          </div>
          {/* big open search */}
          <div style={{ padding:'0 12px 9px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, height:30, border:'1.5px solid rgba(255,255,255,0.35)', borderRadius:5, padding:'0 4px 0 10px', background:'rgba(255,255,255,0.06)' }}>
              <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)' }}>⌕</span>
              <span style={{ flex:1, fontFamily:'var(--font-mono)', fontSize:8, color:'rgba(255,255,255,0.5)' }}>Search Garmin, RAM Mounts & more…</span>
              <span style={{ alignSelf:'stretch', display:'flex', alignItems:'center', padding:'0 9px', margin:'3px 0', background:'#FDCA2F', color:'var(--uc-black)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>GO</span>
            </div>
          </div>
        </div>
        ...
        {kind === 'home' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--uc-black)', color:'var(--uc-paper)', padding:'4px 0', textAlign:'center', fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.12em' }}>FREE SHIPPING OVER $25 · AUTHORIZED GARMIN DEALER</div>
            {/* SHORT hero banner */}
            <div style={{ margin:'10px 12px 0', borderRadius:8, background:'linear-gradient(100deg, #1C1A17 0%, #3A2F25 55%, #7A3B16 135%)', position:'relative', overflow:'hidden', padding:'12px 13px', display:'flex', flexDirection:'column', gap:5, minHeight:96 }}>
              ...
              <span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'#FDCA2F' }}>SUMMER SALE · ENDS FRIDAY</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:18, letterSpacing:'-0.04em', lineHeight:0.9, color:'#fff' }}>Navigate anywhere.<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>On sale.</span></span>
              <span style={{ alignSelf:'flex-start', marginTop:2, padding:'5px 12px', background:'#FDCA2F', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:7, fontWeight:800 }}>Shop the sale →</span>
            </div>
            {/* brand strip */}
            <div style={{ display:'flex', alignItems:'center', gap:0, margin:'11px 0 0', padding:'8px 12px', borderTop:'1px solid var(--line-1)', borderBottom:'1px solid var(--line-1)', justifyContent:'space-between' }}>{['GARMIN','LOWRANCE','RAM','TOMTOM','BUSHNELL'].map(b=>(<span key={b} style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:9, letterSpacing:'-0.02em', color:'var(--fg-3)' }}>{b}</span>))}</div>
            {/* product tabs */}
            ...
            <div style={{ padding:'0 12px 11px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[['Garmin fenix 8','$1,449','#1B3A5C','SALE'],['RAM Tough-Tray','$112.99','#7A6B54','NEW'],['Striker Vivid','$519.99','#1E3D5C',null],['Instinct Solar','$239.99','#3F6B3A','DEAL']].map((p,i)=>(
                <div key={i} style={{ display:'flex', flexDirection:'column', gap:4 }}>
            ...
            <div style={{ padding:'0 12px 12px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>{[['Lumber','#C2A878'],['Concrete','#9A938A'],['Electrical','#C8A14E'],['Tools','#C2410C']].map((c,i)=>(<div key={i} style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ aspectRatio:'1/1', borderRadius:4, background:`linear-gradient(140deg, ${c[1]}, var(--uc-bone))` }}/><span style={{ fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, textAlign:'center', color:'var(--fg-2)' }}>{c[0]}</span></div>))}</div>
            </div>
          </div>
        )}
        {kind === 'about' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:'14px 13px 10px', display:'flex', flexDirection:'column', gap:6 }}>
              <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:700, letterSpacing:'0.16em', color:'var(--fg-3)' }}><span style={{ width:5, height:5, borderRadius:999, background:'#FDCA2F' }}/>EST. 1974 · FAMILY-OWNED</span>
              <span style={{ fontFamily:'var(--font-hero)', fontWeight:700, fontSize:21, letterSpacing:'-0.045em', lineHeight:0.9, color:'var(--fg-1)' }}>Supplying the<br/><span style={{ fontFamily:'var(--font-serif)', fontWeight:400 }}>trades for 50 years.</span></span>
            </div>
            ...
            <div style={{ position:'relative', height:112, margin:'2px 13px 8px' }}>
              ...
              <span style={{ position:'absolute', left:'33%', bottom:2, padding:'3px 8px', background:'var(--uc-paper)', border:'1px solid var(--line-1)', borderRadius:3, fontFamily:'var(--font-mono)', fontSize:5.5, fontWeight:800, letterSpacing:'0.1em', color:'var(--fg-3)', transform:'rotate(-3deg)' }}>BRANCH 01 · 1974</span>
            </div>
            {/* manifesto */}
        ...
        {kind === 'blog' && (
          <div style={{ display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ margin:'0 13px', borderRadius:6, overflow:'hidden', border:'1px solid var(--line-1)' }}>
              <div style={{ aspectRatio:'16/9', background:'radial-gradient(120% 100% at 30% 10%, #C2A878, #6E5235)', position:'relative' }}><span style={{ position:'absolute', top:7, left:7, padding:'2px 7px', background:'#FDCA2F', color:'var(--uc-black)', borderRadius:999, fontFamily:'var(--font-mono)', fontSize:6, fontWeight:800, letterSpacing:'0.06em' }}>FEATURED</span></div>
...
function BPTechStack() {
  const rows = [
    { name: 'Shopify Plus',                fn: 'Commerce Operation Platform',         pri: 'start',  cost: '$2,300/mo' },
    { name: 'NetSuite Connector',          fn: 'ERP ↔ Shopify sync',                  pri: 'start',  cost: '$250/mo' },
    { name: 'QuickBooks Online',            fn: 'Accounting & financials',              pri: 'start',  cost: '—' },
    { name: 'Shopify Search & Discovery',  fn: 'Native search + merchandising',        pri: 'start',  cost: 'Included' },
    { name: 'Shopify Checkout Blocks',     fn: 'Custom checkout extensions',           pri: 'start',  cost: 'Included' },
    { name: 'Shopify Flow',                fn: 'Automation & workflow engine',          pri: 'start',  cost: 'Included' },
    ...
    { name: 'Shopify Marketplace Connect', fn: 'Multi-channel marketplace sync',       pri: 'postlaunch', cost: 'Included' },
    { name: 'Matrixify',                   fn: 'Data import',                          pri: 'delivery',   cost: '$200/mo' }
  ];
  const priMeta = {
    start:  { l: 'Needed to start',  c: 'var(--uc-signal)', fg: 'var(--uc-black)' },
    launch: { l: 'Needed on launch', c: '#FF8B37',          fg: 'var(--uc-black)' },
    postlaunch: { l: 'Post launch',   c: '#6B7280',          fg: '#fff' },
    delivery: { l: 'During delivery', c: '#8B5CF6',          fg: '#fff' },
    ...
    <BPSection id="techstack" n="08" label="Architecture" dark grid tail="WHAT IT RUNS ON">
      ...
      <div style={{ marginTop:'clamp(28px, 3.5vw, 44px)' }}>
        ...
        {(() => {
          ...
          const chip = { start:{ bg:'var(--uc-signal)', fg:'var(--uc-black)' }, launch:{ bg:'#FF8B37', fg:'var(--uc-black)' }, postlaunch:{ bg:'#6B7280', fg:'#fff' }, future:{ bg:'transparent', fg:'var(--uc-stone-300)', outline:true } };
          return (
      ...
      <div style={{ border:'1px solid #1F1F1F', borderRadius:6, overflow:'hidden' }}>
        {/* Header */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.8fr) minmax(0,1.6fr) minmax(0,1fr) minmax(0,130px)', gap:16, padding:'12px 18px', background:'#0F0F0F', borderBottom:'1px solid #1F1F1F', fontFamily:'var(--font-mono)', fontSize:10, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--uc-stone-500)' }}>
        ...
        {rows.map((r, i) => {
          ...
          return (
            <div key={i} style={{ display:'grid', gridTemplateColumns:'minmax(0,1.8fr) minmax(0,1.6fr) minmax(0,1fr) minmax(0,130px)', gap:16, padding:'14px 18px', borderBottom: i < rows.length-1 ? '1px solid #1F1F1F' : 'none', alignItems:'center' }}>
        ...
        {/* Footer total */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.8fr) minmax(0,1.6fr) minmax(0,1fr) minmax(0,130px)', gap:16, padding:'14px 18px', borderTop:'1px solid #2B2B2B', background:'#0F0F0F', alignItems:'center' }}>
          ...
          <span style={{ fontFamily:'var(--font-serif)', fontSize:13, color:'var(--uc-stone-500)' }}>Start stack · launch apps usage-based</span>
          <span/>
          <span style={{ fontFamily:'var(--font-hero)', fontWeight:800, fontSize:'clamp(18px,1.8vw,24px)', letterSpacing:'-0.03em', color:'var(--uc-paper)', textAlign:'right' }}>~$2,650/mo</span>
        </div>
      </div>
      <div style={{ marginTop:16, display:'flex', gap:18, flexWrap:'wrap', fontFamily:'var(--font-mono)', fontSize:10, color:'var(--uc-stone-500)', letterSpacing:'0.06em' }}>
        ...
        <span style={{ display:'inline-flex', alignItems:'center', gap:6 }}><span style={{ width:8,height:8,borderRadius:999,background:'#8B5CF6' }}/>DURING DELIVERY</span>
        <span style={{ display:'inline-flex', alignItems:'center', gap:6 }}><span style={{ width:8,height:8,borderRadius:999,background:'#6B7280' }}/>POST LAUNCH</span>
  ...
  return (
    <BPSection id="integrations" n="11" label="Integrated" tail="HOW IT CONNECTS">
      ...
      <p style={{ marginTop:'clamp(18px, 2.2vw, 26px)', maxWidth:640, fontFamily:'var(--font-serif)', fontSize:'clamp(15px, 1.3vw, 18px)', lineHeight:1.5, color:'var(--fg-2)' }}>
        Uncap will assist the NetSuite integration through the native NetSuite Connector — a real-time, bidirectional sync with field-level mapping for orders, inventory, customers, fulfillments, and B2B flows, managed directly inside the connector dashboard.
      </p>

      {/* NetSuite Connector screenshot */}
      <div style={{ marginTop:'clamp(30px, 4vw, 52px)', border:'1px solid var(--line-2)', borderRadius:12, overflow:'hidden', boxShadow:'0 24px 60px -32px rgba(10,10,10,0.45)' }}>
        <img src="uploads/CI2H9pP7yIwDEAE=.webp" alt="NetSuite Connector dashboard" style={{ width:'100%', display:'block' }}/>
      </div>
        {/* Shopify admin top bar */}
  ...
  return (
    <BPSection id="migration" n="12" label="Migration" paper tail="MOVE WITHOUT LOSS" vec="bgVector2">
      ...
      <p style={{ marginTop: 18, maxWidth: 600, fontFamily: 'var(--font-serif)', fontSize: 'clamp(15px, 1.3vw, 18px)', lineHeight: 1.5, color: 'var(--fg-2)' }}>
        We migrate from your Custom Cart — Uncap provides templated structured data for your team to export from the current custom cart. Our team handles the attribution, mapping, and importing into Shopify, ensuring every product, customer, and order lands correctly with zero data loss.
      </p>
      ...
      <div style={{ marginTop: 'clamp(28px, 3vw, 40px)', display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 0, borderTop: '1px solid var(--line-1)', borderBottom: '1px solid var(--line-1)' }}>
        {stats.map((s, i) => (
          <div key={i} style={{ padding: '24px 20px', borderLeft: i > 0 ? '1px solid var(--line-1)' : 'none', position: 'relative' }}>
            <span aria-hidden="true" style={{ position: 'absolute', top: 0, left: i > 0 ? 20 : 0, width: 16, height: 2, background: 'var(--uc-signal)' }}/>
            <div style={{ marginTop: 6, fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--fg-1)' }}>{s.l}</div>
          </div>
        ))}
      </div>
    </BPSection>
...
function BPRiskAssessment() {
  ...
  const risks = [
    ...
    { t: 'Third party solution partners', sev: 'High',
      d: 'When third party partners are involved they can delay our project.',
      m: 'Set the stage early in the process and constant communication to unlock progress.' },
      ...
      <div style={{
        ...
        <div style={{ padding: 'clamp(28px, 3.4vw, 52px)', display: 'flex', flexDirection: 'column', gap: 'clamp(18px, 2vw, 26px)' }}>
          ...
          <div style={{ padding: 'clamp(18px, 2vw, 26px) clamp(20px, 2.2vw, 28px)', background: 'var(--uc-signal)', borderRadius: 12, boxShadow: '0 22px 60px -26px rgba(232,255,82,0.45)' }}>
            ...
            <div style={{ marginTop: 4, fontFamily: 'var(--font-hero)', fontWeight: 800, fontSize: 'clamp(52px, 6vw, 88px)', letterSpacing: '-0.05em', lineHeight: 0.85, color: 'var(--uc-black)' }}>$94k</div>
          </div>
...
function BPApproveButton() {
  const APPROVED_KEY = 'gpscity_approved_v1';
